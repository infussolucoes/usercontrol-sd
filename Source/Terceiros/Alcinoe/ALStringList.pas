...
{$IFEND}
  ...
  public
    ...
    function GetCurrent: AnsiString; {$IF CompilerVersion >= 17.0}inline;
    {$IFEND}
  ...
  private
    ...
    procedure SetPersistentValue(const Name, Value: AnsiString);
    // [added from Tstrings]
    ...
    procedure SetPersistentValueFromIndex(Index: Integer;
      const Value: AnsiString); // [added from Tstrings]
  protected
...
{$IF CompilerVersion >= 23} { Delphi XE2 }
    ...
    procedure AddStrings(const Strings: TArray<AnsiString>;
      const Objects: TArray<TObject>); overload;
{$IFEND}
...
{$IFEND}
    ...
    property StrictNames[Index: Integer]: AnsiString read GetStrictName;
    // [added from Tstrings]
    property Objects[Index: Integer]: TObject read GetObject write PutObject;
    property QuoteChar: AnsiChar read GetQuoteChar write SetQuoteChar;
    property Values[const Name: AnsiString]: AnsiString read GetValue
      write SetValue;
    property ValueFromIndex[Index: Integer]: AnsiString read GetValueFromIndex
      write SetValueFromIndex;
    property PersistentValues[const Name: AnsiString]: AnsiString read GetValue
      write SetPersistentValue; // [added from Tstrings]
    property PersistentValueFromIndex[Index: Integer]: AnsiString
      read GetValueFromIndex write SetPersistentValueFromIndex;
    // [added from Tstrings]
    property NameValueSeparator: AnsiChar read GetNameValueSeparator
...
{$IF CompilerVersion >= 23} { Delphi XE2 }
  TALStringListSortCompare = reference to function(List: TALStringList;
    Index1, Index2: Integer): Integer;
{$ELSE}
  TALStringListSortCompare = function(List: TALStringList;
    Index1, Index2: Integer): Integer;
{$IFEND}
  ...
  public
    ...
    function FindName(const S: AnsiString; var Index: Integer): Boolean;
    // [added from TStringList]
  ...
  TALAVLStringListSortCompare = function(List: TALAVLStringList;
    Index1, Index2: Integer): Integer;
  ...
  private
    ...
    Function ExtractNameValue(const S: AnsiString;
      var Name, Value: AnsiString): Boolean;
...
begin
  ...
  // doesn't matter what we set here because of fDefined
...
begin
  if Length(Strings) <> Length(Objects) then
    raise EArgumentOutOfRangeException.CreateRes
      (@System.RTLConsts.sInvalidStringAndObjectArrays);
  BeginUpdate;
  try
    for I := Low(Strings) to High(Strings) do
...
begin
  I := IndexOfName(Name);
  if I >= 0 then
    Result := ALCopyStr(Get(I), Length(Name) + 2, MaxInt)
  else
    Result := '';
end;
...
begin
  ...
  begin
    ...
    if ((P <> 0) and (CompareStrings(ALCopyStr(S, 1, P - 1), Name) = 0)) or
      ((P = 0) and (CompareStrings(S, Name) = 0)) then
      Exit;
  end;
  Result := -1;
end;
...
{ *************************************************************************************** }
procedure TALStrings.InsertObject(Index: Integer; const S: AnsiString;
  AObject: TObject);
begin
  Insert(Index, S);
...
{ **************************************************************************************** }
procedure TALStrings.SetPersistentValueFromIndex(Index: Integer;
...
{ ******************************************************************************* }
function TALStringList.AddObject(const S: AnsiString; AObject: TObject)
  : Integer;
begin
  if not Sorted then
    Result := FCount
...
{ ******************************************************************************** }
function TALStringList.FindName(const S: AnsiString;
...
begin
  ...
  else if not FindName(Name, Result) then
    Result := -1;
end;
...
{ ****************************************************************************************** }
procedure TALStringList.InsertObject(Index: Integer; const S: AnsiString;
  AObject: TObject);
begin
  if Sorted then
...
{ **************************************************************************************** }
procedure TALStringList.InsertItem(Index: Integer; const S: AnsiString;
  AObject: TObject);
begin
  Changing;
...
{ *********************************************************************************** }
procedure TALStringList.QuickSort(L, R: Integer;
  SCompare: TALStringListSortCompare);
var
  I, J, P: Integer;
begin
  repeat
...
{ ***************************************************************************************** }
function ALStringListCompareStrings(List: TALStringList;
  Index1, Index2: Integer): Integer;
begin
...
{ ------------------------------------------------------------- }
  ...
  begin
    ...
    begin
      if (P1^ <> P2^) then
      begin
        if (P1^ = NameValueSeparator) then
          Result := -1
        else if (P2^ = NameValueSeparator) then
          Result := 1
        else
...
{ -------------------------------------------------------------- }
  ...
  begin
    ...
    begin
      ...
      if (C1 <> C2) then
      begin
        if (C1 = NameValueSeparator) then
          Result := -1
        else if (C2 = NameValueSeparator) then
          Result := 1
        else
...
{ ********************************************************************************** }
function TALAVLStringList.AddObject(const S: AnsiString;
...
begin
  if (Index < 0) or (Index >= Count) then
    Error(@SListIndexError, Index);
  with TALAVLStringListBinaryTreeNode(FNodeList[Index]) do
  begin
    Result := Obj;
    Obj := nil;
  end;
...
var
  Item1, Item2: TALAVLStringListBinaryTreeNode;
begin
...
begin
  ...
  begin
    if Nvs then
      Result := ID + NameValueSeparator + Val
    else
      Result := ID;
  end;
end;
...
begin
  if ExtractNameValue(S, aName, aValue) then
  begin
    aNode := TALAVLStringListBinaryTreeNode(FAVLBinTree.FindNode(aName));
    if (not Assigned(aNode)) or ((CaseSensitive) and (aNode.Val <> aValue)) or
      ((not CaseSensitive) and (not ALSametext(aNode.Val, aValue))) then
      Result := -1
    else
      Result := aNode.Idx;
  end
  ...
  begin
    ...
    if (not Assigned(aNode)) or (aNode.Nvs) then
      Result := -1
    else
      Result := aNode.Idx;
  end;
...
{ ********************************************************************************************* }
procedure TALAVLStringList.InsertObject(Index: Integer; const S: AnsiString;
  AObject: TObject);
begin
  if (Index < 0) or (Index > Count) then
    Error(@SListIndexError, Index);
...
{ ******************************************************************************************* }
procedure TALAVLStringList.InsertItem(Index: Integer; const S: AnsiString;
  AObject: TObject);
Var
  aName, aValue: AnsiString;
  aNvs: Boolean;
...
begin
  ...
  if (CaseSensitive and (aOldNode.ID <> aNewName)) or
    ((not CaseSensitive) and (not ALSametext(aOldNode.ID, aNewName))) then
  begin
    ...
    if not FAVLBinTree.AddNode(aNewNode) then
    begin
      aNewNode.Free;
      case Duplicates of
        dupIgnore:
...
{ ***************************************************************************************** }
procedure TALAVLStringList.QuickSort(L, R: Integer;
  SCompare: TALAVLStringListSortCompare);
var
  I, J, P: Integer;
begin
  repeat
...
begin
  if Value = dupAccept then
    raise exception.Create('TALAVLStringList does not support duplicate Names');
  FDuplicates := Value;
end;

{ **************************************************************************************************** }
Function TALAVLStringList.ExtractNameValue(const S: AnsiString;
  var Name, Value: AnsiString): Boolean;
Var
  P1: Integer;
begin
