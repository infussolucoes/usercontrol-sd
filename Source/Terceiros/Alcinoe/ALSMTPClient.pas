...
type
  ...
  public
    ...
    Function Auth(AUserName, APassword: AnsiString;
      aAuthType: TAlSmtpClientAuthType): AnsiString; virtual;
    ...
    Function Data(aHeader: TALEmailHeader; aBody: AnsiString): AnsiString;
      overload; virtual;
    ...
    procedure SendMail(aHost: AnsiString; APort: Integer;
      aSenderEmail: AnsiString; aRcptNameLst: TALStrings;
      AUserName, APassword: AnsiString; aAuthType: TAlSmtpClientAuthType;
      aMailData: AnsiString); overload; virtual;
    procedure SendMail(aHost: AnsiString; APort: Integer;
      aSenderEmail: AnsiString; aRcptNameLst: TALStrings;
      AUserName, APassword: AnsiString; aAuthType: TAlSmtpClientAuthType;
      aHeader, aBody: AnsiString); overload; virtual;
    procedure SendMailMultipartMixed(aHost: AnsiString; APort: Integer;
      aSenderEmail: AnsiString; aRcptNameLst: TALStrings;
      AUserName, APassword: AnsiString; aAuthType: TAlSmtpClientAuthType;
      aHeader: TALEmailHeader; aInlineText, aInlineTextContentType: AnsiString;
      aAttachments: TALMultiPartMixedContents); virtual;
    Procedure Disconnect; virtual;
    Function GetAuthTypeFromEhloResponse(EhloResponse: AnsiString)
...
{ **************************************************************************************** }
Function TAlSmtpClient.Connect(const aHost: AnsiString; const APort: Integer)
  : AnsiString;
...
begin
  ...
  if Fconnected then
    raise EALException.Create('SMTP component already connected');
...
{ EhloResponse is like:
  ...
  250-8BITMIME
  250 SIZE 0 }
Function TAlSmtpClient.GetAuthTypeFromEhloResponse(EhloResponse: AnsiString)
...
begin
  ...
  If aSenderEmail = '' then
    raise EALException.Create('Sender email is empty');
  If AlPos(#13#10, aSenderEmail) > 0 then
    raise EALException.Create('Sender email is invalid');
  Result := SendCmd('MAIL From:<' + aSenderEmail + '>', [250]);
end;

{ ********************************************************************************************************** }
Function TAlSmtpClient.Auth(AUserName, APassword: AnsiString;
  aAuthType: TAlSmtpClientAuthType): AnsiString;

{ --------------------------------------- }
  Function InternalDoAuthPlain: AnsiString;
  var
    aAuthPlain: AnsiString;
  begin
    If AUserName = '' then
      raise EALException.Create('UserName is empty');
    If APassword = '' then
      raise EALException.Create('Password is empty');
  ...
  case tmpAuthType of
    ...
    AlsmtpClientAuthCramMD5:
      raise EALException.Create
        ('CRAM-MD5 Authentication is not supported yet!');
    AlsmtpClientAuthCramSha1:
      raise EALException.Create
        ('CRAM-SHA1 Authentication is not supported yet!');
  else
    raise EALException.Create('No Authentication scheme found');
  end;
...
begin
  ...
  if aRcptNameLst.Count <= 0 then
    raise EALException.Create('RcptName list is empty');
  ...
  begin
    ...
    If (aRcptNameValue = '') or (AlPos(#13#10, aRcptNameValue) > 0) then
      raise EALException.Create('Bad entry in RcptName list');
...
{ ********************************************************************************* }
Function TAlSmtpClient.Data(aHeader: TALEmailHeader; aBody: AnsiString)
  : AnsiString;
begin
...
begin
  ...
  try
    aMultipartMixedEncoder.Encode(aInlineText, aInlineTextContentType,
      aAttachments);
    with aMultipartMixedEncoder do
    begin
      aHeader.ContentType := 'multipart/mixed; boundary="' +
        DataStream.Boundary + '"';
...
{ ************************************************* }
procedure TAlSmtpClient.SendMail(aHost: AnsiString; APort: Integer;
  aSenderEmail: AnsiString; aRcptNameLst: TALStrings;
  AUserName, APassword: AnsiString; aAuthType: TAlSmtpClientAuthType;
...
procedure TAlSmtpClient.SendMailMultipartMixed(aHost: AnsiString;
  APort: Integer; aSenderEmail: AnsiString; aRcptNameLst: TALStrings;
  AUserName, APassword: AnsiString; aAuthType: TAlSmtpClientAuthType;
  aHeader: TALEmailHeader; aInlineText, aInlineTextContentType: AnsiString;
...
begin
  ...
  Try
    ...
    MailFrom(aSenderEmail);
    RcptTo(aRcptNameLst);
    DataMultipartMixed(aHeader, aInlineText, aInlineTextContentType,
      aAttachments);
    Quit;
...
{ commands consist of a command code followed by an argument field. Command codes
  ...
  QUIT <CRLF>
  TURN <CRLF> }
function TAlSmtpClient.SendCmd(aCmd: AnsiString; OkResponses: array of Word)
  : AnsiString;
Var
  P: PAnsiChar;
  L: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      If (aResponseLength > 1) and (aResponse[aResponseLength] = #10) and
        (aResponse[aResponseLength - 1] = #13) then
        Break;
    end;
    ...
    Try
      ...
      If ALst.Count = 0 then
        raise EALException.Create('Emtpy response');
      ...
      begin
        ...
        If not aGoodResponse then
          Raise EALException.Create(aResponse);
        if P^ <> '-' then
        Begin
...
{ ****************************************************************************************************************** }
Function TAlSmtpClient.SocketWrite({$IF CompilerVersion >= 23}const {$ELSE}var
