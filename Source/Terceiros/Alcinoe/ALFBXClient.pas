...
{$IF CompilerVersion >= 23} { Delphi XE2 }
  TALFBXClientSelectDataOnNewRowFunct = reference to Procedure
...
{$IFEND}
  ...
  TALFBXClientSQLParam = record
    Value: AnsiString;
    IsNull: Boolean;
    class function Create: TALFBXClientSQLParam; static;
  ...
  TALFBXClientSelectDataQUERY = record
    ...
    class function Create: TALFBXClientSelectDataQUERY; static;
    {$IF CompilerVersion >= 17.0}inline; {$IFEND}
  end;
  ...
  TALFBXClientUpdateDataQUERY = record
    ...
    class function Create: TALFBXClientUpdateDataQUERY; static;
    {$IF CompilerVersion >= 17.0}inline; {$IFEND}
  end;
  ...
  TALFBXClientMonitoringIOStats = record
    page_reads, page_writes, page_fetches, page_marks: int64;
  end;

  { ---------------------------------------- }
  TALFBXClientMonitoringRecordStats = record
    record_idx_reads, record_seq_reads, record_inserts, record_updates,
      record_deletes, record_backouts, record_purges, record_expunges: int64;
  end;

  { ---------------------------------------- }
  TALFBXClientMonitoringMemoryUsage = record
    memory_used, memory_allocated, max_memory_used, max_memory_allocated: int64;
  end;
  ...
  Protected
    function GetFieldValue(aSQLDA: TALFBXSQLResult; aDBHandle: IscDbHandle;
      aTraHandle: IscTrHandle; aIndex: integer;
    ...
    procedure OnUpdateDataDone(Query: TALFBXClientUpdateDataQUERY;
      TimeTaken: integer); virtual;
  Public
    Constructor Create(ApiVer: TALFBXVersion_API;
      const lib: AnsiString = GDS32DLL); overload; virtual;
    Constructor Create(lib: TALFBXLibrary); overload; virtual;
    Destructor Destroy; Override;
    procedure GetMonitoringInfos(ConnectionID, TransactionID: int64;
      StatementSQL: AnsiString; Var IOStats: TALFBXClientMonitoringIOStats;
    ...
    Procedure Connect(DataBaseName, Login, Password, CharSet: AnsiString;
    ...
    Procedure SelectData(Queries: TALFBXClientSelectDataQUERIES;
      XMLDATA: TalXmlNode; OnNewRowFunct: TALFBXClientSelectDataOnNewRowFunct;
      ExtData: Pointer; FormatSettings: TALFormatSettings); overload; virtual;
    Procedure SelectData(Query: TALFBXClientSelectDataQUERY;
      OnNewRowFunct: TALFBXClientSelectDataOnNewRowFunct; ExtData: Pointer;
    ...
    Procedure SelectData(SQL: AnsiString; XMLDATA: TalXmlNode;
      FormatSettings: TALFormatSettings); overload; virtual;
    Procedure SelectData(SQL: AnsiString; Params: Array of AnsiString;
      RowTag: AnsiString; Skip: integer; First: integer; XMLDATA: TalXmlNode;
      FormatSettings: TALFormatSettings); overload; virtual;
    Procedure SelectData(SQL: AnsiString; Params: Array of AnsiString;
      RowTag: AnsiString; XMLDATA: TalXmlNode;
      FormatSettings: TALFormatSettings); overload; virtual;
    Procedure SelectData(SQL: AnsiString; Params: Array of AnsiString;
      XMLDATA: TalXmlNode; FormatSettings: TALFormatSettings);
      overload; virtual;
    ...
    procedure UpdateData(SQL: AnsiString; Params: Array of AnsiString);
      overload; virtual;
    ...
    property DefaultReadTPB: AnsiString read fDefaultReadTPB
      Write fDefaultReadTPB;
    property DefaultWriteTPB: AnsiString read fDefaultWriteTPB
      write fDefaultWriteTPB;
    property TransactionID: Cardinal read GetTransactionID;
    property ConnectionID: integer read GetConnectionID;
  end;

  { ----------------------------------------------------------------------------- }
  TALFBXConnectionStatementPoolBinTreeNode = class
  ...
  Private
    ...
    FConnectionWithStmtPool: TObjectList;
    // pool of connection With list of statements
    ...
    FConnectionWithoutStmtPool: TObjectList;
    // pool of connection WITHOUT list of statements
    ...
    FReadTransactionPool: TALStringKeyAVLBinaryTree;
    // pool of READ ONLY READ COMMITED TRANSACTION
    ...
    // --READ STATEMENT POOL--
    FReadStatementPool: TALStringKeyAVLBinaryTree;
    // pool of STATEMENT with READ ONLY READ COMMITED TRANSACTION
  ...
  Protected
    ...
    Procedure AcquireReadTransaction(var DBHandle: IscDbHandle;
      var TraHandle: IscTrHandle; TPB: AnsiString); virtual;
    Procedure ReleaseReadTransaction(var DBHandle: IscDbHandle;
      var TraHandle: IscTrHandle; TPB: AnsiString;
      const CloseConnection: Boolean = False); virtual;
    Procedure AcquireReadStatement(SQL: AnsiString; var DBHandle: IscDbHandle;
      var TraHandle: IscTrHandle; var StmtHandle: IscStmtHandle;
      var Sqlda: TALFBXSQLResult; TPB: AnsiString); virtual;
    Procedure ReleaseReadStatement(SQL: AnsiString; var DBHandle: IscDbHandle;
      var TraHandle: IscTrHandle; var StmtHandle: IscStmtHandle;
      var Sqlda: TALFBXSQLResult; TPB: AnsiString;
      const CloseConnection: Boolean = False); virtual;
    procedure initObject(aDataBaseName, aLogin, aPassword, aCharSet: AnsiString;
    ...
    procedure OnUpdateDataDone(Query: TALFBXClientUpdateDataQUERY;
      TimeTaken: integer); virtual;
  Public
    Constructor Create(aDataBaseName, aLogin, aPassword, aCharSet: AnsiString;
      aApiVer: TALFBXVersion_API; const alib: AnsiString = GDS32DLL;
      const aNumbuffers: integer = -1;
      const aOpenConnectionExtraParams: AnsiString = ''); overload; virtual;
    Constructor Create(aDataBaseName, aLogin, aPassword, aCharSet: AnsiString;
      alib: TALFBXLibrary; const aNumbuffers: integer = -1;
      const aOpenConnectionExtraParams: AnsiString = ''); overload; virtual;
    Destructor Destroy; Override;
    ...
    Procedure TransactionStart(Var DBHandle: IscDbHandle;
      ...
      var StatementPool: TALFBXConnectionStatementPoolBinTree; TPB: AnsiString);
      overload; virtual;
    ...
    Procedure TransactionRollback(var DBHandle: IscDbHandle;
      ...
      const CloseConnection: Boolean = False); overload; virtual;
    Procedure TransactionStart(Var DBHandle: IscDbHandle;
      var TraHandle: IscTrHandle; TPB: AnsiString); overload; virtual;
    Procedure TransactionCommit(var DBHandle: IscDbHandle;
      var TraHandle: IscTrHandle; const CloseConnection: Boolean = False);
      overload; virtual;
    Procedure TransactionRollback(var DBHandle: IscDbHandle;
      var TraHandle: IscTrHandle; const CloseConnection: Boolean = False);
      overload; virtual;
    ...
    Function Prepare(SQL: AnsiString; Var DBHandle: IscDbHandle;
      var TraHandle: IscTrHandle; var StmtHandle: IscStmtHandle;
      var Sqlda: TALFBXSQLResult; const TPB: AnsiString = '')
      : TALFBXStatementType;
    Procedure SelectData(Queries: TALFBXClientSelectDataQUERIES;
      XMLDATA: TalXmlNode; OnNewRowFunct: TALFBXClientSelectDataOnNewRowFunct;
      ExtData: Pointer; FormatSettings: TALFormatSettings;
      const DBHandle: IscDbHandle = nil; const TraHandle: IscTrHandle = nil;
      const StatementPool: TALFBXConnectionStatementPoolBinTree = nil;
      const TPB: AnsiString = ''); overload; virtual;
    Procedure SelectData(Query: TALFBXClientSelectDataQUERY;
      OnNewRowFunct: TALFBXClientSelectDataOnNewRowFunct; ExtData: Pointer;
      FormatSettings: TALFormatSettings; const DBHandle: IscDbHandle = nil;
    ...
    procedure UpdateData(Query: TALFBXClientUpdateDataQUERY;
      ...
      const StatementPool: TALFBXConnectionStatementPoolBinTree = nil;
      const TPB: AnsiString = ''); overload; virtual;
    procedure UpdateData(SQLs: TALStrings; const DBHandle: IscDbHandle = nil;
      const TraHandle: IscTrHandle = nil;
      const StatementPool: TALFBXConnectionStatementPoolBinTree = nil;
      const TPB: AnsiString = ''); overload; virtual;
    procedure UpdateData(SQL: AnsiString; const DBHandle: IscDbHandle = nil;
      const TraHandle: IscTrHandle = nil;
      const StatementPool: TALFBXConnectionStatementPoolBinTree = nil;
      const TPB: AnsiString = ''); overload; virtual;
    procedure UpdateData(SQL: AnsiString; Params: Array of AnsiString;
    ...
    property TransactionMaxIdleTime: integer read FTransactionMaxIdleTime
      write FTransactionMaxIdleTime;
    property StatementMaxIdleTime: integer read FStatementMaxIdleTime
      write FStatementMaxIdleTime;
    Property NullString: AnsiString Read fNullString Write fNullString;
    property lib: TALFBXLibrary read FLibrary;
    property CharSet: TALFBXCharacterSet read fCharSet;
    property DefaultReadTPB: AnsiString read fDefaultReadTPB
...
{$IF CompilerVersion >= 23}
  { Delphi XE2 }
  TALFBXEventThreadEvent = reference to Procedure(Sender: Tobject;
    const EventName: AnsiString; Count: integer);
  TALFBXEventThreadException = reference to procedure(Sender: Tobject;
    Error: Exception);
{$ELSE}
  TALFBXEventThreadEvent = Procedure(Sender: Tobject;
    const EventName: AnsiString; Count: integer) of object;
  TALFBXEventThreadException = procedure(Sender: Tobject; Error: Exception)
    of object;
{$IFEND}
  ...
  protected
    procedure initObject(aDataBaseName, aLogin, aPassword, aCharSet: AnsiString;
      aEventNames: AnsiString; aConnectionMaxIdleTime: integer;
      aNumbuffers: integer; aOpenConnectionExtraParams: AnsiString;
  ...
  public
    constructor Create(aDataBaseName, aLogin, aPassword, aCharSet: AnsiString;
      ...
      aApiVer: TALFBXVersion_API; aOnEvent: TALFBXEventThreadEvent;
    ...
    Constructor Create(aDataBaseName, aLogin, aPassword, aCharSet: AnsiString;
      aEventNames: AnsiString; // ; separated value like EVENT1;EVENT2; etc...
      alib: TALFBXLibrary; aOnEvent: TALFBXEventThreadEvent;
      aOnException: TALFBXEventThreadException;
      const aConnectionMaxIdleTime: integer = -1;
      const aNumbuffers: integer = -1;
      const aOpenConnectionExtraParams: AnsiString = ''); overload; virtual;
    ...
    Constructor Create(aDataBaseName, aLogin, aPassword, aCharSet: AnsiString;
      aEventNames: AnsiString; // ; separated value like EVENT1;EVENT2; etc...
      alib: TALFBXLibrary; const aConnectionMaxIdleTime: integer = -1;
      const aNumbuffers: integer = -1;
      const aOpenConnectionExtraParams: AnsiString = ''); overload; virtual;
    Destructor Destroy; override;
    ...
    property OnException: TALFBXEventThreadException read fOnException
      write fOnException;
  end;

Const
  cALFbxClientDefaultReadNOWaitTPB = isc_tpb_version3 +
  // Transaction version number is used internally by the InterBase engine. It is always be
  ...
  // isc_tpb_version3 = InterBase version 3 transaction
    ...
    isc_tpb_read +
  // Access mode describes the actions that can be performed by the functions associated with
  ...
  // not yet committed, the row cannot be read.
    ...
    isc_tpb_nowait;
  // Lock resolution describes how a transaction should react if a lock conflict occurs. Valid
  ...
  cALFbxClientDefaultWriteNOWaitTPB = isc_tpb_version3 +
  // Transaction version number is used internally by the InterBase engine. It is always be
  // the first attribute specified in the TPB, and must always be set to isc_tpb_version3.
  // isc_tpb_version3 = InterBase version 3 transaction
  ...
  cALFbxClientDefaultReadWaitTPB = isc_tpb_version3 + isc_tpb_read +
    isc_tpb_read_committed + isc_tpb_rec_version + isc_tpb_wait;

  cALFbxClientDefaultWriteWaitTPB = isc_tpb_version3 + isc_tpb_write +
    isc_tpb_read_committed + isc_tpb_no_rec_version + isc_tpb_wait;
...
{ ***************************************************** }
procedure TALFBXClient.GetMonitoringInfos(ConnectionID, TransactionID: int64;
  StatementSQL: AnsiString; Var IOStats: TALFBXClientMonitoringIOStats;
  Var RecordStats: TALFBXClientMonitoringRecordStats;
  Var MemoryUsage: TALFBXClientMonitoringMemoryUsage;
  Const SkipIOStats: Boolean = False; Const SkipRecordStats: Boolean = False;
  Const SkipMemoryUsage: Boolean = False);
...
Begin
  ...
  aSelectPart := ALIfThen(not SkipIOStats, 'MON$PAGE_READS as PAGE_READS, ' +
    'MON$PAGE_WRITES as PAGE_WRITES, ' + 'MON$PAGE_FETCHES as PAGE_FETCHES, ' +
    'MON$PAGE_MARKS as PAGE_MARKS, ') + ALIfThen(not SkipRecordStats,
    ...
    'MON$RECORD_EXPUNGES as RECORD_EXPUNGES, ') + ALIfThen(not SkipMemoryUsage,
  ...
  if aSelectPart <> '' then
    delete(aSelectPart, length(aSelectPart) - 1, 1)
  else
    Exit;

  // build the aFromPart
  if StatementSQL <> '' then
    aFromPart := 'MON$STATEMENTS'
  else if TransactionID <> -1 then
    aFromPart := 'MON$TRANSACTIONS'
  else if ConnectionID <> -1 then
    aFromPart := 'MON$ATTACHMENTS'
  else
    aFromPart := 'MON$DATABASE';

  // build the aJoinPart
  aJoinPart := ALIfThen(not SkipIOStats,
    'JOIN MON$IO_STATS ON MON$IO_STATS.MON$STAT_ID = ' + aFromPart +
    '.MON$STAT_ID ') + ALIfThen(not SkipRecordStats,
    'JOIN MON$RECORD_STATS ON MON$RECORD_STATS.MON$STAT_ID = ' + aFromPart +
    '.MON$STAT_ID ') + ALIfThen(not SkipMemoryUsage,
    'JOIN MON$MEMORY_USAGE ON MON$MEMORY_USAGE.MON$STAT_ID = ' + aFromPart +
    '.MON$STAT_ID ');

  // build the aWherePart
  if StatementSQL <> '' then
    aWherePart := 'WHERE ' + 'MON$SQL_TEXT = ' + ALQuotedStr(StatementSQL) +
      ALIfThen(TransactionID <> -1, ' AND MON$TRANSACTION_ID = ' +
      ALIntToStr(TransactionID)) + ALIfThen(ConnectionID <> -1,
      ' AND MON$ATTACHMENT_ID = ' + ALIntToStr(ConnectionID))
  else if TransactionID <> -1 then
    aWherePart := 'WHERE ' + 'MON$TRANSACTION_ID = ' + ALIntToStr(TransactionID)
      + ALIfThen(ConnectionID <> -1, ' AND MON$ATTACHMENT_ID = ' +
      ALIntToStr(ConnectionID))
  else if ConnectionID <> -1 then
    aWherePart := 'WHERE ' + 'MON$ATTACHMENT_ID = ' + ALIntToStr(ConnectionID)
  else
    aWherePart := '';
  ...
  Try
    ...
    Try
      ...
      try
        ...
        SelectData('SELECT ' + aSelectPart + 'FROM ' + aFromPart + ' ' +
          aJoinPart + aWherePart, 'rec', aXMLDATA.DocumentElement,
          aFormatSettings);

        if aXMLDATA.DocumentElement.ChildNodes.Count <> 1 then
          raise Exception.Create('Can not get the monitoring stats');
        with aXMLDATA.DocumentElement.ChildNodes[0] do
        begin

          if not SkipIOStats then
          begin
            with IOStats do
            begin
          ...
          if not SkipRecordStats then
          begin
            with RecordStats do
            begin
              record_seq_reads :=
                ALStrToInt64(ChildNodes['record_seq_reads'].text);
              record_idx_reads :=
                ALStrToInt64(ChildNodes['record_idx_reads'].text);
              ...
              record_backouts :=
                ALStrToInt64(ChildNodes['record_backouts'].text);
              record_purges := ALStrToInt64(ChildNodes['record_purges'].text);
              record_expunges :=
                ALStrToInt64(ChildNodes['record_expunges'].text);
            end;
          end;

          if not SkipMemoryUsage then
          begin
            with MemoryUsage do
            begin
              memory_used := ALStrToInt64(ChildNodes['memory_used'].text);
              memory_allocated :=
                ALStrToInt64(ChildNodes['memory_allocated'].text);
              max_memory_used :=
                ALStrToInt64(ChildNodes['max_memory_used'].text);
              max_memory_allocated :=
                ALStrToInt64(ChildNodes['max_memory_allocated'].text);
            end;
          end;
...
function TALFBXClient.GetFieldValue(aSQLDA: TALFBXSQLResult;
  aDBHandle: IscDbHandle; aTraHandle: IscTrHandle; aIndex: integer;
  aFormatSettings: TALFormatSettings): AnsiString;
{ ------------------------- }
...
Begin
  If not aSQLDA.IsNull[aIndex] then
    Case aSQLDA.SQLType[aIndex] of
      SQL_TIMESTAMP:
        result := ALDatetimetostr(aSQLDA.AsDateTime[aIndex], aFormatSettings);
      SQL_TYPE_TIME:
        result := ALTimetostr(aSQLDA.AsTime[aIndex], aFormatSettings);
      SQL_TYPE_DATE:
        result := ALDatetostr(aSQLDA.AsDate[aIndex], aFormatSettings);
      SQL_DOUBLE:
        result := ALFloatToStr(aSQLDA.AsDouble[aIndex], aFormatSettings);
      SQL_FLOAT, SQL_D_FLOAT:
        result := ALFloatToStr(aSQLDA.AsSingle[aIndex], aFormatSettings);
      SQL_INT64, SQL_LONG, SQL_SHORT:
        begin
          if aSQLDA.SQLScale[aIndex] < 0 then
            result := ALFloatToStr(aSQLDA.AsDouble[aIndex], aFormatSettings)
          else
            result := aSQLDA.AsAnsiString[aIndex];
        end;
      SQL_BLOB:
        InternalReadBlob;
    else
...
{ ******************************************************************************************* }
constructor TALFBXClient.Create(ApiVer: TALFBXVersion_API;
  const lib: AnsiString = GDS32DLL);
begin
...
begin
  If not Connected then
    raise Exception.Create('Not connected');
  case item of
    isc_info_implementation, isc_info_base_level:
...
begin
  ...
  p := PAnsiChar(data);
  while byte(p^) = isc_info_user_names do
  begin
    len := byte(p[3]);
    inc(p, 4);
...
begin
  if Connected then
    raise Exception.Create('You must disconnect first');
...
{ ****************************************** }
procedure TALFBXClient.Connect(DataBaseName, Login, Password,
  CharSet: AnsiString; const ExtraParams: AnsiString = '');
Var
  aParams: AnsiString;
begin
  if Connected then
  ...
  aParams := 'user_name = ' + Login + '; ' + 'password = ' + Password + '; ' +
    'lc_ctype = ' + CharSet;
  if ExtraParams <> '' then
    aParams := aParams + '; ' + ExtraParams;
  Try
    FLibrary.AttachDatabase(DataBaseName, fDBHandle, aParams);
  Except
    fDBHandle := nil;
    raise;
  End;
...
{ ****************************************** }
procedure TALFBXClient.Connect(DataBaseName, Login, Password,
  CharSet: AnsiString; Numbuffers: integer);
begin
  Connect(DataBaseName, Login, Password, CharSet,
...
begin
  ...
  Try
    FLibrary.TransactionStart(fTraHandle, fDBHandle, TPB);
  Except
    fTraHandle := nil;
    raise;
  End;
...
begin
  ...
  Try
    ...
    try
      ...
      // prepare the SQL
      result := FLibrary.DSQLPrepare(fDBHandle, fTraHandle, fStmtHandle, SQL,
        FSQLDIALECT, fSqlda);
...
Var
  ...
  Begin
    ...
    // init the aViewRec
    if (Queries[aQueriesIndex].ViewTag <> '') and (not assigned(aXmlDocument))
    then
    ...
    aRecIndex := 0;
    aRecAdded := 0;
    while FLibrary.DSQLFetch(fDBHandle, fTraHandle, fStmtHandle, FSQLDIALECT,
      fSqlda) do
    begin
      ...
      begin
        ...
        // init NewRec
        if (Queries[aQueriesIndex].RowTag <> '') and (not assigned(aXmlDocument))
        then
        ...
        For aColumnIndex := 0 to fSqlda.FieldCount - 1 do
        begin
          aValueRec := aNewRec.AddChild
            (ALlowercase(fSqlda.AliasName[aColumnIndex]));
          if (fSqlda.SQLType[aColumnIndex] = SQL_BLOB) then
            aValueRec.ChildNodes.Add
              (aValueRec.OwnerDocument.CreateNode(GetFieldValue(fSqlda,
              fDBHandle, fTraHandle, aColumnIndex, FormatSettings), ntCData))
          else
            aValueRec.text := GetFieldValue(fSqlda, fDBHandle, fTraHandle,
              aColumnIndex, FormatSettings);
...
begin
  ...
  Try
    ...
    begin
      ...
      if (not assigned(fSqlda)) or (not assigned(fStmtHandle)) or
        (Queries[aQueriesIndex].SQL <> fStmtSQL) then
      begin
      ...
      try
        ...
        // if their is params
        if length(Queries[aQueriesIndex].Params) > 0 then
        begin
          ...
          try
            ...
            for aParamsIndex :=
              0 to length(Queries[aQueriesIndex].Params) - 1 do
            begin

              // with current Params Fields
              with Queries[aQueriesIndex].Params[aParamsIndex] do
              begin
                ...
                // IsBlob
                else if length(Value) > high(smallint) then
                begin
                  aSqlpa.AddFieldType('', uftBlob);
                  aBlobhandle := nil;
                  aSqlpa.AsQuad[aParamsIndex] := FLibrary.BlobCreate(fDBHandle,
                    fTraHandle, aBlobhandle);
                  Try
...
{ ************************************************ }
procedure TALFBXClient.SelectData(SQL: AnsiString; Skip: integer;
  First: integer; OnNewRowFunct: TALFBXClientSelectDataOnNewRowFunct;
  ExtData: Pointer; FormatSettings: TALFormatSettings);
var
  aSelectDataQUERIES: TALFBXClientSelectDataQUERIES;
begin
...
{ ************************************************ }
procedure TALFBXClient.SelectData(SQL: AnsiString; RowTag: AnsiString;
...
{ ************************************************ }
procedure TALFBXClient.SelectData(SQL: AnsiString; XMLDATA: TalXmlNode;
  FormatSettings: TALFormatSettings);
var
  aSelectDataQUERIES: TALFBXClientSelectDataQUERIES;
begin
...
{ ************************************************ }
procedure TALFBXClient.SelectData(SQL: AnsiString; Params: Array of AnsiString;
  RowTag: AnsiString; Skip: integer; First: integer; XMLDATA: TalXmlNode;
  FormatSettings: TALFormatSettings);
var
  aSelectDataQUERIES: TALFBXClientSelectDataQUERIES;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        try
          ...
          // loop throught all Params Fields
          for aParamsIndex := 0 to length(Queries[aQueriesIndex].Params) - 1 do
          begin
    ...
    begin
      ...
      begin
        ...
        FLibrary.DSQLExecuteImmediate(fDBHandle, fTraHandle,
          Queries[aQueriesIndex].SQL, FSQLDIALECT, nil);
...
{ ************************************************ }
procedure TALFBXClient.UpdateData(SQL: AnsiString; Params: Array of AnsiString);
Var
  aUpdateDataQUERIES: TALFBXClientUpdateDataQUERIES;
  i: integer;
begin
...
procedure TALFBXConnectionPoolClient.GetMonitoringInfos(ConnectionID,
  TransactionID: int64; StatementSQL: AnsiString;
...
Begin
  ...
  // start the TMP transaction
  TransactionStart(aDBHandle, aTraHandle, DefaultReadTPB);
  Try
    ...
    try
      ...
      SelectData('SELECT ' + aSelectPart + 'FROM ' + aFromPart + ' ' + aJoinPart
        + aWherePart, 'rec', aXMLDATA.DocumentElement, aFormatSettings,
        aDBHandle, aTraHandle);
    ...
    // commit the TMP transaction
    TransactionCommit(aDBHandle, aTraHandle);

  Except

    // roolback the TMP transaction
    TransactionRollback(aDBHandle, aTraHandle);
    Raise;
...
{ ************************************************************ }
procedure TALFBXConnectionPoolClient.initObject(aDataBaseName, aLogin,
  aPassword, aCharSet: AnsiString; const aNumbuffers: integer = -1;
...
begin
  ...
  FLogin := aLogin;
  FPassword := aPassword;
  fOpenConnectionParams := 'user_name = ' + aLogin + '; ' + 'password = ' +
    aPassword + '; ' + 'lc_ctype = ' + aCharSet;
  if aNumbuffers > -1 then
    fOpenConnectionParams := fOpenConnectionParams + '; num_buffers = ' +
      ALIntToStr(aNumbuffers);
  if aOpenConnectionExtraParams <> '' then
    fOpenConnectionParams := fOpenConnectionParams + '; ' +
...
{ ********************************************************** }
constructor TALFBXConnectionPoolClient.Create(aDataBaseName, aLogin, aPassword,
  aCharSet: AnsiString; aApiVer: TALFBXVersion_API;
  const alib: AnsiString = GDS32DLL; const aNumbuffers: integer = -1;
  const aOpenConnectionExtraParams: AnsiString = '');
begin
  FLibrary := TALFBXLibrary.Create(aApiVer);
  FLibrary.Load(alib);
  FownLibrary := True;
  initObject(aDataBaseName, aLogin, aPassword, aCharSet, aNumbuffers,
    aOpenConnectionExtraParams);
end;

{ ********************************************************** }
constructor TALFBXConnectionPoolClient.Create(aDataBaseName, aLogin, aPassword,
  aCharSet: AnsiString; alib: TALFBXLibrary; const aNumbuffers: integer = -1;
  const aOpenConnectionExtraParams: AnsiString = '');
begin
  FLibrary := alib;
  FownLibrary := False;
...
begin
  ...
  if not assigned(aTmpDBHandle) then
    aTmpDBHandle := AcquireConnectionWithoutStmt;
  try
    case item of
      isc_info_implementation, isc_info_base_level:
        result := byte(FLibrary.DatabaseInfoString(aTmpDBHandle, item, 8)[5]);
    else
      result := FLibrary.DatabaseInfoIntValue(aTmpDBHandle, AnsiChar(item));
    end;
  finally
    if not assigned(DBHandle) then
      ReleaseConnectionWithoutStmt(aTmpDBHandle);
  end;
end;
...
{ ********************************************************************************** }
function TALFBXConnectionPoolClient.GetConnectionID
  (DBHandle: IscDbHandle): integer;
begin
  if not assigned(DBHandle) then
...
{ ************************************************************************************* }
function TALFBXConnectionPoolClient.GetTransactionID(TraHandle: IscTrHandle)
  : Cardinal;
begin
  if not assigned(TraHandle) then
    raise Exception.Create('Transaction handle can not be null');
...
Type
  ...
  { ************************************************************************************************** }
procedure ALFBXConnectionPoolClient_ConnectionStatementPoolIterateFunct
  (aTree: TALBaseAVLBinaryTree; aNode: TALBaseAVLBinaryTreeNode;
  aExtData: Pointer; Var aContinue: Boolean);
begin

  // typecast the aExtData
  With TALFBXConnectionPoolClient_ConnectionStatementPoolIterateExtData
    (aExtData^) do

    // if the Statement is expired
    if TickCountCurrentdate - TALFBXConnectionStatementPoolBinTreeNode(aNode)
      .LastAccessDate > ConnectionPoolClient.StatementMaxIdleTime then
      LstBinaryTreeNodeToDelete.Add(aNode)

end;

{ *************************************************************************************** }
Procedure TALFBXConnectionPoolClient.AcquireConnectionWithStmt
  (var DBHandle: IscDbHandle;
  var StatementPool: TALFBXConnectionStatementPoolBinTree);
Var
  aConnectionStatementPoolIterateExtData
    : TALFBXConnectionPoolClient_ConnectionStatementPoolIterateExtData;
...
Begin
  ...
  Try
    ...
    if aTickCount - FLastConnectionWithStmtGarbage > (60000 { every minutes } )
    then
    begin
      while FConnectionWithStmtPool.Count > 0 do
      begin
        aConnectionWithStmtPoolContainer :=
          TALFBXConnectionWithStmtPoolContainer(FConnectionWithStmtPool[0]);
        if aTickCount - aConnectionWithStmtPoolContainer.LastAccessDate > FConnectionMaxIdleTime
        then
        begin
    ...
    If FConnectionWithStmtPool.Count > 0 then
    begin
      aConnectionWithStmtPoolContainer := TALFBXConnectionWithStmtPoolContainer
        (FConnectionWithStmtPool[FConnectionWithStmtPool.Count - 1]);
    ...
    begin
      DBHandle := nil;
      StatementPool := nil;
      FLibrary.AttachDatabase(DataBaseName, DBHandle, fOpenConnectionParams);
  ...
  if ALGettickCount64 - StatementPool.LastGarbage > (60000 { every minutes } )
  then
  begin

    // create aExtData.LstNodeToDelete
    aConnectionStatementPoolIterateExtData.LstBinaryTreeNodeToDelete :=
    ...
    Try
      ...
      aConnectionStatementPoolIterateExtData.TickCountCurrentdate :=
        ALGettickCount64;

      // iterate all StatementPool node
      StatementPool.Iterate
        (ALFBXConnectionPoolClient_ConnectionStatementPoolIterateFunct, True,
        @aConnectionStatementPoolIterateExtData);

      // delete all StatementPool node that have an empty pool
      for i := 0 to aConnectionStatementPoolIterateExtData.
        LstBinaryTreeNodeToDelete.Count - 1 do
        StatementPool.DeleteNode
          (TALFBXStringKeyPoolBinTreeNode
          (aConnectionStatementPoolIterateExtData.
          LstBinaryTreeNodeToDelete[i]).id);
...
{ *************************************************************************************** }
Procedure TALFBXConnectionPoolClient.ReleaseConnectionWithStmt
  (var DBHandle: IscDbHandle;
  var StatementPool: TALFBXConnectionStatementPoolBinTree;
...
begin
  ...
  if not assigned(DBHandle) then
    raise Exception.Create('Connection handle can not be null');
  if not assigned(StatementPool) then
    raise Exception.Create('StatementPool can not be null');
  ...
  Try
    ...
    begin
      aConnectionWithStmtPoolContainer :=
        TALFBXConnectionWithStmtPoolContainer.Create;
...
Begin
  ...
  Try
    ...
    // delete the old unused connection
    aTickCount := ALGettickCount64;
    if aTickCount - FLastConnectionWithoutStmtGarbage > (60000 { every minutes } )
    then
    begin
      while FConnectionWithoutStmtPool.Count > 0 do
      begin
        aConnectionWithoutStmtPoolContainer :=
          TALFBXConnectionWithoutStmtPoolContainer
          (FConnectionWithoutStmtPool[0]);
        if aTickCount - aConnectionWithoutStmtPoolContainer.LastAccessDate > FConnectionMaxIdleTime
        then
        begin

          // drop the connection
          Try
            FLibrary.DetachDatabase
              (aConnectionWithoutStmtPoolContainer.DBHandle);
          Except
            // Disconnect must be a "safe" procedure because it's mostly called in
            // finalization part of the code that it is not protected
          End;
    ...
    // acquire the new connection from the pool
    If FConnectionWithoutStmtPool.Count > 0 then
    begin
      aConnectionWithoutStmtPoolContainer :=
        TALFBXConnectionWithoutStmtPoolContainer
        (FConnectionWithoutStmtPool[FConnectionWithoutStmtPool.Count - 1]);
    ...
    begin
      result := nil;
      FLibrary.AttachDatabase(DataBaseName, result, fOpenConnectionParams);
    end;
...
{ ****************************************************************************************** }
procedure TALFBXConnectionPoolClient.ReleaseConnectionWithoutStmt
  (var DBHandle: IscDbHandle; const CloseConnection: Boolean = False);
Var
  aConnectionWithoutStmtPoolContainer: TALFBXConnectionWithoutStmtPoolContainer;
begin
  ...
  Try
    ...
    begin
      aConnectionWithoutStmtPoolContainer :=
        TALFBXConnectionWithoutStmtPoolContainer.Create;
...
Type
  ...
  { ********************************************************************************************** }
procedure ALFBXConnectionPoolClient_ReadTransactionPoolIterateFunct
  (aTree: TALBaseAVLBinaryTree; aNode: TALBaseAVLBinaryTreeNode;
  aExtData: Pointer; Var aContinue: Boolean);

Var
  aReadTransactionPool: TObjectList;
  aReadTransactionPoolContainer: TALFBXReadTransactionPoolContainer;

begin

  // typecast the aExtData
  With TALFBXConnectionPoolClient_ReadTransactionPoolIterateExtData
    (aExtData^) do
  begin
    ...
    while aReadTransactionPool.Count > 0 do
    begin

      // init aReadTransactionPoolContainer
      aReadTransactionPoolContainer := TALFBXReadTransactionPoolContainer
        (aReadTransactionPool[0]);

      // if the aReadTransactionPoolContainer is expired
      if TickCountCurrentdate - aReadTransactionPoolContainer.LastAccessDate >
        ConnectionPoolClient.TransactionMaxIdleTime then
      begin

        Try

          // commit the transaction and release the connection
          ConnectionPoolClient.TransactionCommit
            (aReadTransactionPoolContainer.DBHandle,
            aReadTransactionPoolContainer.TraHandle);

        Except

          // roolback the transaction (and free the connection)
          if assigned(aReadTransactionPoolContainer.TraHandle) then
            ConnectionPoolClient.TransactionRollback
              (aReadTransactionPoolContainer.DBHandle,
              aReadTransactionPoolContainer.TraHandle, True)
            // const CloseConnection: Boolean = False

            // simply free the connection
          else if assigned(aReadTransactionPoolContainer.DBHandle) then
            ConnectionPoolClient.ReleaseConnectionWithoutStmt
              (aReadTransactionPoolContainer.DBHandle, True);
          // const CloseConnection: Boolean = False
    ...
    if aReadTransactionPool.Count = 0 then
      LstBinaryTreeNodeToDelete.Add(aNode);
...
{ ************************************************************************************ }
procedure TALFBXConnectionPoolClient.AcquireReadTransaction
  (var DBHandle: IscDbHandle; var TraHandle: IscTrHandle; TPB: AnsiString);

Var
  aReadTransactionPoolIterateExtData
    : TALFBXConnectionPoolClient_ReadTransactionPoolIterateExtData;
  aReadTransactionPoolContainer: TALFBXReadTransactionPoolContainer;
...
Begin
  ...
  if (TPB = '') or ((AlPos(isc_tpb_read_committed, TPB) <= 0) or
    (AlPos(isc_tpb_read, TPB) <= 0) or (AlPos(isc_tpb_write, TPB) > 0)) then
    raise Exception.Create('Wrong Params');
  if assigned(DBHandle) then
    raise Exception.Create('Connection handle must be null');
  if assigned(TraHandle) then
    raise Exception.Create('Transaction handle must be null');
  ...
  Try
    ...
    if FReleasingAllconnections then
      raise Exception.Create
        ('Can not acquire transaction: currently releasing all connections');

    // delete the old unused Transaction
    if ALGettickCount64 - FLastReadTransactionGarbage > (60000 { every minutes } )
    then
    begin

      // create aExtData.LstNodeToDelete
      aReadTransactionPoolIterateExtData.LstBinaryTreeNodeToDelete :=
        TObjectList.Create(False);
      Try

        // init aExtData.ConnectionPoolClient and aExtData.TickCount
        aReadTransactionPoolIterateExtData.ConnectionPoolClient := Self;
        aReadTransactionPoolIterateExtData.TickCountCurrentdate :=
          ALGettickCount64;

        // iterate all FReadTransactionPool node
        FReadTransactionPool.Iterate
          (ALFBXConnectionPoolClient_ReadTransactionPoolIterateFunct, True,
          @aReadTransactionPoolIterateExtData);

        // delete all FReadTransactionPool node that have an empty pool
        for i := 0 to aReadTransactionPoolIterateExtData.
          LstBinaryTreeNodeToDelete.Count - 1 do
          FReadTransactionPool.DeleteNode
            (TALFBXStringKeyPoolBinTreeNode
            (aReadTransactionPoolIterateExtData.
            LstBinaryTreeNodeToDelete[i]).id);
    ...
    if not assigned(aStringKeyPoolBinTreeNode) then
    begin
      aStringKeyPoolBinTreeNode := TALFBXStringKeyPoolBinTreeNode.Create;
      aStringKeyPoolBinTreeNode.id := TPB;
      if not FReadTransactionPool.AddNode(aStringKeyPoolBinTreeNode) then
      begin
        aStringKeyPoolBinTreeNode.free;
        Raise Exception.Create
          ('Can not add any more node in transaction pool binary tree');
      end;
    end;

    // if the if still some transaction in the queue
    If TALFBXStringKeyPoolBinTreeNode(aStringKeyPoolBinTreeNode).Pool.Count > 0
    then
    begin
      with TALFBXStringKeyPoolBinTreeNode(aStringKeyPoolBinTreeNode) do
      begin
        aReadTransactionPoolContainer := TALFBXReadTransactionPoolContainer
          (Pool[Pool.Count - 1]);
    ...
    begin
      DBHandle := nil;
      TraHandle := nil;
      TransactionStart(DBHandle, TraHandle, TPB);
    end;
...
{ ************************************************************************************ }
procedure TALFBXConnectionPoolClient.ReleaseReadTransaction
  (var DBHandle: IscDbHandle; var TraHandle: IscTrHandle; TPB: AnsiString;
  const CloseConnection: Boolean = False);
...
begin
  ...
  Try
    ...
    begin
      ...
      aReadTransactionPoolContainer :=
        TALFBXReadTransactionPoolContainer.Create;
      ...
      TALFBXStringKeyPoolBinTreeNode(aStringKeyPoolBinTreeNode)
        .Pool.Add(aReadTransactionPoolContainer);
    ...
    begin
      ...
      Try
        ...
        // commit the transaction and release the connection
        TransactionCommit(DBHandle, TraHandle, CloseConnection);

      Except

        // roolback the transaction (and free the connection)
        if assigned(TraHandle) then
          TransactionRollback(DBHandle, TraHandle, True)
          // const CloseConnection: Boolean = False

          // simply free the connection
        else if assigned(DBHandle) then
...
Type
  ...
  { ******************************************************************************************** }
procedure ALFBXConnectionPoolClient_ReadStatementPoolIterateFunct
  (aTree: TALBaseAVLBinaryTree; aNode: TALBaseAVLBinaryTreeNode;
  aExtData: Pointer; Var aContinue: Boolean);

Var
  aReadStatementPool: TObjectList;
  aReadStatementPoolContainer: TALFBXReadStatementPoolContainer;

begin

  // typecast the aExtData
  With TALFBXConnectionPoolClient_ReadStatementPoolIterateExtData(aExtData^) do
  begin
    ...
    while aReadStatementPool.Count > 0 do
    begin

      // init aReadStatementPoolContainer
      aReadStatementPoolContainer := TALFBXReadStatementPoolContainer
        (aReadStatementPool[0]);

      // if the aReadStatementPoolContainer is expired
      if TickCountCurrentdate - aReadStatementPoolContainer.LastAccessDate >
        ConnectionPoolClient.StatementMaxIdleTime then
      begin
        ...
        Try
          ...
          // drop the statement
          try
            ConnectionPoolClient.lib.DSQLFreeStatement
              (aReadStatementPoolContainer.StmtHandle, DSQL_drop);
          Except
            // what else we can do here ?
            // this can happen if connection lost for exemple
          end;
          ...
          ConnectionPoolClient.TransactionCommit
            (aReadStatementPoolContainer.DBHandle,
            aReadStatementPoolContainer.TraHandle);

        Except

          // roolback the transaction (and free the connection)
          if assigned(aReadStatementPoolContainer.TraHandle) then
            ConnectionPoolClient.TransactionRollback
              (aReadStatementPoolContainer.DBHandle,
              aReadStatementPoolContainer.TraHandle, True)
            // const CloseConnection: Boolean = False

            // simply free the connection
          else if assigned(aReadStatementPoolContainer.DBHandle) then
            ConnectionPoolClient.ReleaseConnectionWithoutStmt
              (aReadStatementPoolContainer.DBHandle, True);
          // const CloseConnection: Boolean = False
    ...
    // update LstNodeToDelete
    if aReadStatementPool.Count = 0 then
      LstBinaryTreeNodeToDelete.Add(aNode);
...
procedure TALFBXConnectionPoolClient.AcquireReadStatement(SQL: AnsiString;
  var DBHandle: IscDbHandle; var TraHandle: IscTrHandle;
  var StmtHandle: IscStmtHandle; var Sqlda: TALFBXSQLResult; TPB: AnsiString);

Var
  aReadStatementPoolIterateExtData
    : TALFBXConnectionPoolClient_ReadStatementPoolIterateExtData;
  aReadStatementPoolContainer: TALFBXReadStatementPoolContainer;
  aStringKeyPoolBinTreeNode: TALStringKeyAVLBinaryTreeNode;
  i: integer;
...
Begin
  ...
  // must be only read commited read only
  if (SQL = '') or (TPB = '') or ((AlPos(isc_tpb_read_committed, TPB) <= 0) or
    (AlPos(isc_tpb_read, TPB) <= 0) or (AlPos(isc_tpb_write, TPB) > 0)) then
    raise Exception.Create('Wrong Params');
  if assigned(DBHandle) then
    raise Exception.Create('Connection handle must be null');
  ...
  if assigned(StmtHandle) then
    raise Exception.Create('Statement handle must be null');
  if assigned(Sqlda) then
    raise Exception.Create('Statement descriptor must be null');
  ...
  Try
    ...
    if FReleasingAllconnections then
      raise Exception.Create
        ('Can not acquire Statement: currently releasing all connections');

    // delete the old unused Statement
    if ALGettickCount64 - FLastReadStatementGarbage > (60000 { every minutes } )
    then
    begin

      // create aExtData.LstNodeToDelete
      aReadStatementPoolIterateExtData.LstBinaryTreeNodeToDelete :=
        TObjectList.Create(False);
      Try

        // init aExtData.ConnectionPoolClient and aExtData.TickCount
        aReadStatementPoolIterateExtData.ConnectionPoolClient := Self;
        aReadStatementPoolIterateExtData.TickCountCurrentdate :=
          ALGettickCount64;

        // iterate all FReadStatementPool node
        FReadStatementPool.Iterate
          (ALFBXConnectionPoolClient_ReadStatementPoolIterateFunct, True,
          @aReadStatementPoolIterateExtData);

        // delete all FReadStatementPool node that have an empty pool
        for i := 0 to aReadStatementPoolIterateExtData.
          LstBinaryTreeNodeToDelete.Count - 1 do
          FReadStatementPool.DeleteNode
            (TALFBXStringKeyPoolBinTreeNode
            (aReadStatementPoolIterateExtData.LstBinaryTreeNodeToDelete[i]).id);
    ...
    begin
      ...
      if not FReadStatementPool.AddNode(aStringKeyPoolBinTreeNode) then
      begin
        aStringKeyPoolBinTreeNode.free;
        Raise Exception.Create
          ('Can not add any more node in statement pool binary tree');
      end;
    end;
    ...
    begin
      ...
      StmtHandle := nil;
      Sqlda := nil;
      Prepare(SQL, DBHandle, TraHandle, StmtHandle, Sqlda, TPB);
    end;
...
begin
  ...
  if not assigned(TraHandle) then
    raise Exception.Create('Transaction handle can not be null');
  if not assigned(StmtHandle) then
    raise Exception.Create('Statement handle can not be null');
  if not assigned(Sqlda) then
    raise Exception.Create('Statement descriptor can not be null');
  ...
  Try
    ...
    begin
      ...
      TALFBXStringKeyPoolBinTreeNode(aStringKeyPoolBinTreeNode)
        .Pool.Add(aReadStatementPoolContainer);
    ...
    begin
      ...
      Try
        ...
        // commit and release the connection
        TransactionCommit(DBHandle, TraHandle);
...
{ ******************************************************************************************************* }
procedure TALFBXConnectionPoolClient.ReleaseAllConnections
  ...
  Try
    ...
    begin
      ...
      begin
        FReadStatementPoolCS.Acquire;
        Try
          if FWorkingReadStatementCount <= 0 then
            Break;
        finally
          FReadStatementPoolCS.Release;
        end;
    ...
    Try
      ...
      Try
        ...
        // init aExtData.ConnectionPoolClient and aExtData.TickCount
        aReadStatementPoolIterateExtData.ConnectionPoolClient := Self;
        aReadStatementPoolIterateExtData.TickCountCurrentdate := high(int64);
        // this will cause to delete all node
    ...
    begin
      ...
      begin
        FReadTransactionPoolCS.Acquire;
        Try
          if FWorkingReadTransactionCount <= 0 then
            Break;
        finally
          FReadTransactionPoolCS.Release;
        end;
    ...
    Try
      ...
      Try
        ...
        // init aExtData.ConnectionPoolClient and aExtData.TickCount
        aReadTransactionPoolIterateExtData.ConnectionPoolClient := Self;
        aReadTransactionPoolIterateExtData.TickCountCurrentdate := high(int64);
        // this will cause to delete all node
    ...
    begin
      ...
      begin
        FConnectionWithStmtPoolCS.Acquire;
        Try
          if FWorkingConnectionWithStmtCount <= 0 then
            Break;
        finally
          FConnectionWithStmtPoolCS.Release;
        end;
    ...
    Try
      ...
      begin
        ...
        aConnectionWithStmtPoolContainer :=
          TALFBXConnectionWithStmtPoolContainer
          (FConnectionWithStmtPool[FConnectionWithStmtPool.Count - 1]);
    ...
    begin
      ...
      begin
        FConnectionWithoutStmtPoolCS.Acquire;
        Try
          if FWorkingConnectionWithoutStmtCount <= 0 then
            Break;
        finally
          FConnectionWithoutStmtPoolCS.Release;
        end;
...
begin
  ...
  if assigned(TraHandle) then
    raise Exception.Create('Transaction handle must be null');
  if assigned(StatementPool) then
    raise Exception.Create('Statement pool object must be null');
  ...
  try
    ...
    // Start the transaction
    FLibrary.TransactionStart(TraHandle, DBHandle, TPB);
...
{ ******************************************************************************* }
procedure TALFBXConnectionPoolClient.TransactionCommit
  (var DBHandle: IscDbHandle; var TraHandle: IscTrHandle;
  var StatementPool: TALFBXConnectionStatementPoolBinTree;
  const CloseConnection: Boolean = False);
begin
  ...
  if not assigned(TraHandle) then
    raise Exception.Create('Transaction handle can not be null');
  if not assigned(StatementPool) then
    raise Exception.Create('Statement pool object can not be null');
  ...
  // release the connection
  ReleaseConnectionWithStmt(DBHandle, StatementPool, CloseConnection);

end;

{ ********************************************************************************* }
procedure TALFBXConnectionPoolClient.TransactionRollback
  (var DBHandle: IscDbHandle; var TraHandle: IscTrHandle;
  var StatementPool: TALFBXConnectionStatementPoolBinTree;
  const CloseConnection: Boolean = False);
var
...
begin
  ...
  Finally
    ...
    // release the connection
    TraHandle := nil;
    ReleaseConnectionWithStmt(DBHandle, StatementPool, aTmpCloseConnection);
...
{ ************************************************************************************** }
procedure TALFBXConnectionPoolClient.TransactionCommitRetaining
  (TraHandle: IscTrHandle);
begin
...
{ **************************************************************************************** }
procedure TALFBXConnectionPoolClient.TransactionRollbackRetaining
  (TraHandle: IscTrHandle);
begin
...
function TALFBXConnectionPoolClient.Prepare(SQL: AnsiString;
  Var DBHandle: IscDbHandle; var TraHandle: IscTrHandle;
  var StmtHandle: IscStmtHandle; var Sqlda: TALFBXSQLResult;
  const TPB: AnsiString = ''): TALFBXStatementType;
Var
  aReleaseConnectionHandleonError: Boolean;
begin

  // check the params
  if ((not assigned(DBHandle)) and (assigned(TraHandle))) or
    ((assigned(DBHandle)) and (not assigned(TraHandle))) or
    (assigned(StmtHandle)) or (assigned(Sqlda)) or (TPB = '') then
    raise Exception.Create('Wrong Params');
  ...
  begin
    ...
    aReleaseConnectionHandleonError := False;
  try
    ...
    Try
      ...
      try
        ...
        // prepare the SQL
        result := FLibrary.DSQLPrepare(DBHandle, TraHandle, StmtHandle, SQL,
          FSQLDIALECT, Sqlda);
    ...
    if aReleaseConnectionHandleonError then
      TransactionRollback(DBHandle, TraHandle, True);
    // const CloseConnection: Boolean = False
    raise;
  end;
...
{ *************************************************************************************** }
procedure TALFBXConnectionPoolClient.OnSelectDataDone
  (Query: TALFBXClientSelectDataQUERY; TimeTaken: integer);
begin
  // virtual
end;

{ *************************************************************************************** }
procedure TALFBXConnectionPoolClient.OnUpdateDataDone
  (Query: TALFBXClientUpdateDataQUERY; TimeTaken: integer);
begin
  // virtual
end;

{ ************************************************************************************* }
procedure TALFBXConnectionPoolClient.SelectData
  (Queries: TALFBXClientSelectDataQUERIES; XMLDATA: TalXmlNode;
  OnNewRowFunct: TALFBXClientSelectDataOnNewRowFunct; ExtData: Pointer;
  FormatSettings: TALFormatSettings; const DBHandle: IscDbHandle = nil;
  const TraHandle: IscTrHandle = nil;
  const StatementPool: TALFBXConnectionStatementPoolBinTree = nil;
...
{$IFDEF undef}{$REGION 'InternalSelectDataFetchRows'}{$ENDIF}
  ...
  Begin
    ...
    aRecIndex := 0;
    aRecAdded := 0;
    while FLibrary.DSQLFetch(aTmpDBHandle, aTmpTraHandle, aTmpStmtHandle,
      FSQLDIALECT, aTmpSqlda) do
    begin
      ...
      begin
        ...
        // loop throught all column
        For aColumnIndex := 0 to aTmpSqlda.FieldCount - 1 do
        begin
          aValueRec := aNewRec.AddChild
            (ALlowercase(aTmpSqlda.AliasName[aColumnIndex]));
          if (aTmpSqlda.SQLType[aColumnIndex] = SQL_BLOB) then
            aValueRec.ChildNodes.Add
              (aValueRec.OwnerDocument.CreateNode(GetFieldValue(aTmpSqlda,
              aTmpDBHandle, aTmpTraHandle, aColumnIndex, FormatSettings),
              ntCData))
          else
            aValueRec.text := GetFieldValue(aTmpSqlda, aTmpDBHandle,
              aTmpTraHandle, aColumnIndex, FormatSettings);
          if aUpdateRowTagByFieldValue and
            (aValueRec.NodeName = aNewRec.NodeName) then
            aNewRec.NodeName := ALlowercase(aValueRec.text);
        end;
...
{$IFDEF undef}{$REGION 'Check the params'}{$ENDIF}
  if ((not assigned(DBHandle)) and (assigned(TraHandle))) or
    ((assigned(DBHandle)) and (not assigned(TraHandle))) or
    ((assigned(StatementPool)) and (not assigned(DBHandle))) then
    raise Exception.Create('Wrong Params');
{$IFDEF undef}{$ENDREGION}{$ENDIF}
...
{$IFDEF undef}{$REGION 'init aTmpStatementPoolNode'}{$ENDIF}
        if assigned(StatementPool) then
          aTmpStatementPoolNode := StatementPool.FindNode
            (Queries[aQueriesIndex].SQL)
        else
          aTmpStatementPoolNode := nil;
{$IFDEF undef}{$ENDREGION}{$ENDIF}
{$IFDEF undef}{$REGION 'if aTmpStatementPoolNode is null'}{$ENDIF}
        if not assigned(aTmpStatementPoolNode) then
        begin

{$IFDEF undef}{$REGION 'if it''s a readonly Read Commited TPB AND DBHandle = nil'}{$ENDIF}
          if (aReadOnlyReadCommitedTPB) and (not assigned(DBHandle)) then
          begin
...
{$IFDEF undef}{$REGION 'if their is some params'}{$ENDIF}
            ...
            begin
              ...
              if aTransactionWasAcquiredFromPool then
              begin
                ReleaseReadTransaction(aTmpDBHandle, aTmpTraHandle, aTmpTPB);
                aTransactionWasAcquiredFromPool := False;
              end;

              // acquire a statement from the pool
              AcquireReadStatement(Queries[aQueriesIndex].SQL, aTmpDBHandle,
                aTmpTraHandle, aTmpStmtHandle, aTmpSqlda, aTmpTPB);
...
{$IFDEF undef}{$REGION 'if their is NO param'}{$ENDIF}
            ...
            begin
              ...
              if not aTransactionWasAcquiredFromPool then
              begin
                AcquireReadTransaction(aTmpDBHandle, aTmpTraHandle, aTmpTPB);
                aTransactionWasAcquiredFromPool := True;
              end;

              // Prepare the statement
              Prepare(Queries[aQueriesIndex].SQL, aTmpDBHandle, aTmpTraHandle,
                aTmpStmtHandle, aTmpSqlda, aTmpTPB);
              aStatementWasAcquiredFromPool := False;
...
{$IFDEF undef}{$REGION 'if it''s NOT a readonly Read Commited TPB or DBHandle <> nil'}{$ENDIF}
          ...
          begin
            ...
            if not assigned(aTmpDBHandle) then
            begin
              TransactionStart(aTmpDBHandle, aTmpTraHandle, aTmpTPB);
...
{$IFDEF undef}{$REGION 'else if aTmpStatementPoolNode is NOT null'}{$ENDIF}
        ...
        begin
          ...
          with TALFBXConnectionStatementPoolBinTreeNode
            (aTmpStatementPoolNode) do
          begin
...
{$IFDEF undef}{$REGION 'if their is params'}{$ENDIF}
          ...
          begin
            ...
            try
              ...
              begin
                ...
                begin
                  ...
                  begin
                    aSqlpa.AddFieldType('', uftBlob);
                    aBlobhandle := nil;
                    aSqlpa.AsQuad[aParamsIndex] :=
                      FLibrary.BlobCreate(aTmpDBHandle, aTmpTraHandle,
                      aBlobhandle);
                    Try
                      FLibrary.BlobWriteString(aBlobhandle, Value);
                    Finally
              ...
              // execute the sql with the params
              FLibrary.DSQLExecute(aTmpTraHandle, aTmpStmtHandle,
                FSQLDIALECT, aSqlpa);
...
{$IFDEF undef}{$REGION 'if the statement was acquired from the pool - CLOSE IT and RELEASE IT'}{$ENDIF}
            if aStatementWasAcquiredFromPool then
            begin
              ...
              try
                ...
                ReleaseReadStatement(Queries[aQueriesIndex].SQL, aTmpDBHandle,
                  aTmpTraHandle, aTmpStmtHandle, aTmpSqlda, aTmpTPB);
              ...
              Except
                ...
                ReleaseReadStatement(Queries[aQueriesIndex].SQL, aTmpDBHandle,
                  aTmpTraHandle, aTmpStmtHandle, aTmpSqlda, aTmpTPB, True);
                // CloseConnection
...
{$IFDEF undef}{$REGION 'Else if a statementPool was given in the params'}{$ENDIF}
          ...
          begin
            ...
            if (length(Queries[aQueriesIndex].Params) > 0) or
              (assigned(aTmpStatementPoolNode)) then
            begin
              ...
              if assigned(aTmpStatementPoolNode) then
                TALFBXConnectionStatementPoolBinTreeNode(aTmpStatementPoolNode)
                  .LastAccessDate := ALGettickCount64

                // else add it to the statement pool
              else
              begin
                aTmpStatementPoolNode :=
                  TALFBXConnectionStatementPoolBinTreeNode.Create;
                aTmpStatementPoolNode.id := Queries[aQueriesIndex].SQL;
                TALFBXConnectionStatementPoolBinTreeNode(aTmpStatementPoolNode)
                  .lib := lib;
                TALFBXConnectionStatementPoolBinTreeNode(aTmpStatementPoolNode)
                  .StmtHandle := aTmpStmtHandle;
                TALFBXConnectionStatementPoolBinTreeNode(aTmpStatementPoolNode)
                  .Sqlda := aTmpSqlda;
                if not StatementPool.AddNode(aTmpStatementPoolNode) then
                  aTmpStatementPoolNode.free;
                // don't raise any exception to not hide previous error
              end;
...
{$IFDEF undef}{$REGION 'Commit or release the transaction'}{$ENDIF}
      if aTransactionWasAcquiredFromPool then
        ReleaseReadTransaction(aTmpDBHandle, aTmpTraHandle, aTmpTPB)
      else if aConnectionWasAcquiredFromPool then
        TransactionCommit(aTmpDBHandle, aTmpTraHandle);
{$IFDEF undef}{$ENDREGION}{$ENDIF}
    Except
...
{$IFDEF undef}{$REGION 'On Exception'}{$ENDIF}
      ...
      begin
        ...
        { get the gdscode }
        if E is EALFBXError then
          aGDSCode := (E as EALFBXError).GDSCode
        else
          aGDSCode := -1;
        ...
        if aTransactionWasAcquiredFromPool then
          ...
          TransactionRollback(aTmpDBHandle, aTmpTraHandle,
            GetCloseConnectionByErrCode(aGDSCode));
...
{ ********************************************************************************* }
procedure TALFBXConnectionPoolClient.SelectData
  (Query: TALFBXClientSelectDataQUERY;
  OnNewRowFunct: TALFBXClientSelectDataOnNewRowFunct; ExtData: Pointer;
  FormatSettings: TALFormatSettings; const DBHandle: IscDbHandle = nil;
  const TraHandle: IscTrHandle = nil;
  const StatementPool: TALFBXConnectionStatementPoolBinTree = nil;
...
begin
  ...
  aSelectDataQUERIES[0] := Query;
  SelectData(aSelectDataQUERIES, nil, OnNewRowFunct, ExtData, FormatSettings,
    DBHandle, TraHandle, StatementPool, TPB);
end;

{ ************************************************************** }
procedure TALFBXConnectionPoolClient.SelectData(SQL: AnsiString; Skip: integer;
  First: integer; OnNewRowFunct: TALFBXClientSelectDataOnNewRowFunct;
  ExtData: Pointer; FormatSettings: TALFormatSettings;
  const DBHandle: IscDbHandle = nil; const TraHandle: IscTrHandle = nil;
  const StatementPool: TALFBXConnectionStatementPoolBinTree = nil;
...
begin
  ...
  SelectData(Queries, XMLDATA, nil, nil, FormatSettings, DBHandle, TraHandle,
    StatementPool, TPB);
...
{ ********************************************************************************* }
procedure TALFBXConnectionPoolClient.SelectData
  (Query: TALFBXClientSelectDataQUERY; XMLDATA: TalXmlNode;
  FormatSettings: TALFormatSettings; const DBHandle: IscDbHandle = nil;
  const TraHandle: IscTrHandle = nil;
  const StatementPool: TALFBXConnectionStatementPoolBinTree = nil;
  const TPB: AnsiString = '');
...
begin
  setlength(aSelectDataQUERIES, 1);
  aSelectDataQUERIES[0] := Query;
  SelectData(aSelectDataQUERIES, XMLDATA, nil, nil, FormatSettings, DBHandle,
    TraHandle, StatementPool, TPB);
end;
...
{ ************************************************************** }
procedure TALFBXConnectionPoolClient.SelectData(SQL: AnsiString;
  Params: array of AnsiString; RowTag: AnsiString; Skip: integer;
  First: integer; XMLDATA: TalXmlNode; FormatSettings: TALFormatSettings;
  const DBHandle: IscDbHandle = nil; const TraHandle: IscTrHandle = nil;
  const StatementPool: TALFBXConnectionStatementPoolBinTree = nil;
  const TPB: AnsiString = '');
...
{ ************************************************************** }
procedure TALFBXConnectionPoolClient.SelectData(SQL: AnsiString;
  Params: array of AnsiString; RowTag: AnsiString; XMLDATA: TalXmlNode;
  FormatSettings: TALFormatSettings; const DBHandle: IscDbHandle = nil;
  const TraHandle: IscTrHandle = nil;
  const StatementPool: TALFBXConnectionStatementPoolBinTree = nil;
  const TPB: AnsiString = '');
...
{ ************************************************************** }
procedure TALFBXConnectionPoolClient.SelectData(SQL: AnsiString;
  Params: array of AnsiString; XMLDATA: TalXmlNode;
  FormatSettings: TALFormatSettings; const DBHandle: IscDbHandle = nil;
  const TraHandle: IscTrHandle = nil;
  const StatementPool: TALFBXConnectionStatementPoolBinTree = nil;
  const TPB: AnsiString = '');
...
{ ************************************************************************************* }
procedure TALFBXConnectionPoolClient.UpdateData
  (Queries: TALFBXClientUpdateDataQUERIES; const DBHandle: IscDbHandle = nil;
  const TraHandle: IscTrHandle = nil;
  const StatementPool: TALFBXConnectionStatementPoolBinTree = nil;
  const TPB: AnsiString = '');
...
{$IFDEF undef}{$REGION 'init local variable'}{$ENDIF}
  ...
  aTmpTPB := TPB;
  if aTmpTPB = '' then
    aTmpTPB := fDefaultWriteTPB;
  aTmpStatementPool := nil;
  if not assigned(aTmpDBHandle) then
  begin
    TransactionStart(aTmpDBHandle, aTmpTraHandle, aTmpStatementPool, TPB);
    aConnectionWasAcquiredFromPool := True;
  end
  else
  begin
...
{$IFDEF undef}{$ENDREGION}{$ENDIF}
{$IFDEF undef}{$REGION 'init aTmpStatementPoolNode'}{$ENDIF}
      if assigned(aTmpStatementPool) then
        aTmpStatementPoolNode := aTmpStatementPool.FindNode
          (Queries[aQueriesIndex].SQL)
      else
        aTmpStatementPoolNode := nil;
{$IFDEF undef}{$ENDREGION}{$ENDIF}
...
{$IFDEF undef}{$REGION 'Prepare or acquire the statement'}{$ENDIF}
        ...
        begin
          Prepare(Queries[aQueriesIndex].SQL, aTmpDBHandle,
            // not nil => will not change
...
{$IFDEF undef}{$REGION 'drop or pool the statement'}{$ENDIF}
          ...
          begin
            ...
            begin
              ...
              TALFBXConnectionStatementPoolBinTreeNode(aTmpStatementPoolNode)
                .Sqlda := aTmpSqlda;
              if not aTmpStatementPool.AddNode(aTmpStatementPoolNode) then
                aTmpStatementPoolNode.free;
              // don't raise any exception to not hide previous error
            end
...
{$IFDEF undef}{$REGION 'if their is NO params and aTmpStatementPoolNode = nil'}{$ENDIF}
      ...
      begin
        ...
        FLibrary.DSQLExecuteImmediate(aTmpDBHandle, aTmpTraHandle,
          Queries[aQueriesIndex].SQL, FSQLDIALECT, nil);
...
{$IFDEF undef}{$REGION 'Commit the transaction'}{$ENDIF}
    if aConnectionWasAcquiredFromPool then
    begin
      TransactionCommit(aTmpDBHandle, aTmpTraHandle, aTmpStatementPool);
    end;
{$IFDEF undef}{$ENDREGION}{$ENDIF}
  Except
...
{$IFDEF undef}{$REGION 'On Exception'}{$ENDIF}
    ...
    begin
      ...
      if aConnectionWasAcquiredFromPool then
      begin
        TransactionRollback(aTmpDBHandle, aTmpTraHandle, aTmpStatementPool,
          GetCloseConnectionByErrCode(aGDSCode));
        // const CloseConnection: Boolean = False
      end;
...
{ ********************************************************************************* }
procedure TALFBXConnectionPoolClient.UpdateData
  (Query: TALFBXClientUpdateDataQUERY; const DBHandle: IscDbHandle = nil;
  const TraHandle: IscTrHandle = nil;
  const StatementPool: TALFBXConnectionStatementPoolBinTree = nil;
  const TPB: AnsiString = '');
Var
...
begin
  setlength(aUpdateDataQUERIES, 1);
  aUpdateDataQUERIES[0] := Query;
  UpdateData(aUpdateDataQUERIES, DBHandle, TraHandle, StatementPool, TPB);
end;
...
{ ************************************************************** }
procedure TALFBXConnectionPoolClient.UpdateData(SQL: AnsiString;
  Params: Array of AnsiString; const DBHandle: IscDbHandle = nil;
  const TraHandle: IscTrHandle = nil;
  const StatementPool: TALFBXConnectionStatementPoolBinTree = nil;
  const TPB: AnsiString = '');
Var
...
{ ****************************************************************************************** }
function TALFBXConnectionPoolClient.GetCloseConnectionByErrCode
  (aGDSCode: integer): Boolean;
begin
...
begin
  ...
  begin
    ...
    try
      if assigned(StmtHandle) then
        lib.DSQLFreeStatement(StmtHandle, DSQL_drop);
    Except
      // what else we can do here ?
      // this can happen if connection lost for exemple
    end;
...
procedure ALFBXEventCallback(UserData: Pointer; length: smallint;
  Updated: PAnsiChar); cdecl;
begin
  if (assigned(UserData) and assigned(Updated)) then
  begin
    with TALFBXEventThread(UserData) do
    begin
      if FEventCanceled then
      begin
        SetEvent(FSignal);
        Exit;
      end;
...
{ *************************************************** }
procedure TALFBXEventThread.initObject(aDataBaseName, aLogin, aPassword,
  aCharSet: AnsiString; aEventNames: AnsiString;
  aConnectionMaxIdleTime: integer; aNumbuffers: integer;
  aOpenConnectionExtraParams: AnsiString; aOnEvent: TALFBXEventThreadEvent;
  aOnException: TALFBXEventThreadException);
Var
  aLst: TALStrings;
  i: integer;
begin
  ...
  if FConnectionMaxIdleTime <= 0 then
    FConnectionMaxIdleTime := INFINITE
  else
  ...
  Try
    aLst.text := ALTrim(alStringReplace(aEventNames, ';', #13#10,
      [rfReplaceALL]));
    i := 0;
    while (i <= 14) and (i <= aLst.Count - 1) do
    begin
...
{ ************************************************* }
constructor TALFBXEventThread.Create(aDataBaseName, aLogin, aPassword,
  aCharSet: AnsiString; aEventNames: AnsiString;
  // ; separated value like EVENT1;EVENT2; etc...
  aApiVer: TALFBXVersion_API; aOnEvent: TALFBXEventThreadEvent;
  aOnException: TALFBXEventThreadException; const alib: AnsiString = GDS32DLL;
  const aConnectionMaxIdleTime: integer = -1; const aNumbuffers: integer = -1;
  const aOpenConnectionExtraParams: AnsiString = '');
begin
  FLibrary := TALFBXLibrary.Create(aApiVer);
  FLibrary.Load(alib);
  FownLibrary := True;
  initObject(aDataBaseName, aLogin, aPassword, aCharSet, aEventNames,
    aConnectionMaxIdleTime, aNumbuffers, aOpenConnectionExtraParams, aOnEvent,
    aOnException);
...
begin
  ...
  FownLibrary := True;
  initObject(aDataBaseName, aLogin, aPassword, aCharSet, aEventNames,
    aConnectionMaxIdleTime, aNumbuffers, aOpenConnectionExtraParams, nil, nil);
  inherited Create(False);
  // see http://www.gerixsoft.com/blog/delphi/fixing-symbol-resume-deprecated-warning-delphi-2010
end;
...
begin
  ...
  while (not FWaitingSignal) and (not FCompleted) do
    sleep(1);
...
{ ******************************************************************************* }
procedure TALFBXEventThread.DoEvent(const EventName: AnsiString;
  Count: integer);
begin
  if assigned(fOnEvent) then
    fOnEvent(Self, EventName, Count);
end;
...
var
  ...
  Begin
    ...
    if assigned(fResultBuffer) then
    begin
      Try
  ...
  begin
    Try
      ...
      begin
        ...
        FLibrary.AttachDatabase(FDataBaseName, fDBHandle,
          fOpenConnectionParams);

        // register the EventBlock
        aEventBufferLen := FLibrary.EventBlock(aEventBuffer, fResultBuffer,
          fEventNamesCount, PAnsiChar(fEventNamesArr[0]),
          PAnsiChar(fEventNamesArr[1]), PAnsiChar(fEventNamesArr[2]),
          PAnsiChar(fEventNamesArr[3]), PAnsiChar(fEventNamesArr[4]),
          PAnsiChar(fEventNamesArr[5]), PAnsiChar(fEventNamesArr[6]),
          PAnsiChar(fEventNamesArr[7]), PAnsiChar(fEventNamesArr[8]),
          PAnsiChar(fEventNamesArr[9]), PAnsiChar(fEventNamesArr[10]),
          PAnsiChar(fEventNamesArr[11]), PAnsiChar(fEventNamesArr[12]),
          PAnsiChar(fEventNamesArr[13]), PAnsiChar(fEventNamesArr[14]));

        // the First EventQueue
        ResetEvent(FSignal);
        FLibrary.EventQueue(fDBHandle, aEventID, aEventBufferLen, aEventBuffer,
          @ALFBXEventCallback, Self);
        if WaitForSingleObject(FSignal, 60000) <> WAIT_OBJECT_0 then
          raise Exception.Create('Timeout in the first call to isc_que_events');
        FLibrary.EventCounts(aStatusVector, aEventBufferLen, aEventBuffer,
          fResultBuffer);
      ...
      WaitForSingleObject(FSignal, FConnectionMaxIdleTime);
      // every 20 minutes reset the connection
      ...
      begin
        ...
        for aCurrentEventIdx := 0 to 14 do
          if aStatusVector[aCurrentEventIdx] <> 0 then
            DoEvent(fEventNamesArr[aCurrentEventIdx],
              aStatusVector[aCurrentEventIdx]);
