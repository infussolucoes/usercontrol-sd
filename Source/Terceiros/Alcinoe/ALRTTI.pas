...
Type
  ...
  public
    ...
    function GetMethods(const aVisibility: TMemberVisibility)
      : TArray<TRttiMethod>; overload;
    function GetMethods(const AName: string;
      const aVisibility: TMemberVisibility): TArray<TRttiMethod>; overload;
    function GetFields(const aVisibility: TMemberVisibility)
      : TArray<TRttiField>;
    function GetField(const AName: string; const aVisibility: TMemberVisibility)
      : TRttiField;
    function GetProperties(const aVisibility: TMemberVisibility)
      : TArray<TRttiProperty>;
    function GetProperty(const AName: string;
      const aVisibility: TMemberVisibility): TRttiProperty; overload;
    function GetProperty(const AIndex: integer;
      const aVisibility: TMemberVisibility): TRttiProperty; overload;
    function GetIndexedProperties(const aVisibility: TMemberVisibility)
      : TArray<TRttiIndexedProperty>;
...
{ ************************************************ }
var
  vALRttiTypeEXCache: TALStringKeyAVLBinaryTree;
...
begin
  case aRttiField.Visibility of
    ...
    raise Exception.Create('Unknown visibility');
  end;
end;

{ ************************************************************************************* }
procedure TALRttiTypeEx.AddIndexedProperty(aRttiIndexedProperty
  : TRttiIndexedProperty);
begin
  case aRttiIndexedProperty.Visibility of
    mvPrivate:
      begin
        setlength(fPrivateIndexedProperties,
          length(fPrivateIndexedProperties) + 1);
        fPrivateIndexedProperties[high(fPrivateIndexedProperties)] :=
          aRttiIndexedProperty;
      end;
    mvProtected:
      begin
        setlength(fPrivateIndexedProperties,
        ...
        setlength(fProtectedIndexedProperties,
          length(fProtectedIndexedProperties) + 1);
        fProtectedIndexedProperties[high(fProtectedIndexedProperties)] :=
          aRttiIndexedProperty;
      end;
    mvPublic:
      begin
        ...
        setlength(fPublicIndexedProperties,
          length(fPublicIndexedProperties) + 1);
        fPublicIndexedProperties[high(fPublicIndexedProperties)] :=
          aRttiIndexedProperty;
      end;
    mvPublished:
      begin
        ...
        setlength(fPublishedIndexedProperties,
          length(fPublishedIndexedProperties) + 1);
        fPublishedIndexedProperties[high(fPublishedIndexedProperties)] :=
          aRttiIndexedProperty;
      end;
  else
    raise Exception.Create('Unknown visibility');
...
{ ***************************************************************************************** }
function TALRttiTypeEx.GetFields(const aVisibility: TMemberVisibility)
  : TArray<TRttiField>;
begin
  case aVisibility of
    mvPrivate:
...
{ ***************************************************************************************************** }
function TALRttiTypeEx.GetField(const AName: string;
  const aVisibility: TMemberVisibility): TRttiField;
begin
...
{ ************************************************************************************************************** }
function TALRttiTypeEx.GetIndexedProperties(const aVisibility
  : TMemberVisibility): TArray<TRttiIndexedProperty>;
begin
  case aVisibility of
    mvPrivate:
      result := fPrivateIndexedProperties;
    mvProtected:
      result := fProtectedIndexedProperties;
    mvPublic:
      result := fPublicIndexedProperties;
    mvPublished:
      result := fPublishedIndexedProperties;
  else
    raise Exception.Create('Unknown visibility');
  end;
end;
...
{ ******************************************************************************************* }
function TALRttiTypeEx.GetMethods(const aVisibility: TMemberVisibility)
  : TArray<TRttiMethod>;
begin
  case aVisibility of
    mvPrivate:
...
{ **************************************************************************************************************** }
function TALRttiTypeEx.GetMethods(const AName: string;
  const aVisibility: TMemberVisibility): TArray<TRttiMethod>;
var
...
{ ************************************************************************************************ }
function TALRttiTypeEx.GetProperties(const aVisibility: TMemberVisibility)
  : TArray<TRttiProperty>;
begin
  case aVisibility of
    mvPrivate:
...
{ *********************************************************************************************************** }
function TALRttiTypeEx.GetProperty(const AName: string;
  const aVisibility: TMemberVisibility): TRttiProperty;
begin
...
{ ************************************************************************************************************* }
function TALRttiTypeEx.GetProperty(const AIndex: integer;
  const aVisibility: TMemberVisibility): TRttiProperty;
begin
  for result in GetProperties(aVisibility) do
...
var
  aRttiTypeEXCacheNode: TObject;
begin
  aRttiTypeEXCacheNode := vALRttiTypeEXCache.FindNode(aClassName);
  if not assigned(aRttiTypeEXCacheNode) then
    raise EALException.Create('Cannot obtain RTTI informations about the class '
      + aClassName)
  else
    result := TALRttiTypeExCacheNode(aRttiTypeEXCacheNode).RttiTypeEX;
end;
...
begin
  ...
  for i := Low(aRttiTypes) to High(aRttiTypes) do
  begin
    ...
    try
      ...
      for aRttiProperty in aRttiTypes[i].GetProperties do
      begin
      ...
      for aRttiIndexedProperty in aRttiTypes[i].GetIndexedProperties do
      begin
        aRttiIndexedProperty.GetAttributes;
        // to create the customAttribute object
        aRttiTypeEXCacheNode.RttiTypeEX.AddIndexedProperty
          (aRttiIndexedProperty);
      end;

      for aRttiField in aRttiTypes[i].GetFields do
      begin
        aRttiField.GetAttributes; // to create the customAttribute object
        aRttiTypeEXCacheNode.RttiTypeEX.AddField(aRttiField);
      end;

      for aRttiMethod in aRttiTypes[i].GetMethods do
      begin
      ...
      if not vALRttiTypeEXCache.AddNode(aRttiTypeEXCacheNode) then
        raise Exception.Create('Duplicate name (' +
          string(aRttiTypeEXCacheNode.ID) + ')');
