...
type
  ...
  { --------------------------------------- }
  TAlMemCachedClient_responseType = (rpEND, rpOK, rpCRLF, rpSTORAGE,
    rpRETRIEVAL, rpRETRIEVALS, rpDELETE, rpINCRDECR, rpTOUCH, rpNONE);
  ...
  protected
    ...
    Function SocketRead(aSocketDescriptor: TSocket; var Buf; len: integer)
      : integer; Virtual;
    Function SendCmd(aSocketDescriptor: TSocket; aCmd: ansiString;
    ...
    Function GetResponse(aSocketDescriptor: TSocket;
      aResponseType: TAlMemCachedClient_responseType;
      var aGETStoredItems: TAlMemCachedClient_StoredItems): ansiString;
    function DoGet(aSocketDescriptor: TSocket; const key: ansiString;
      var flags: integer; var data: ansiString): Boolean; overload; virtual;
    function DoGet(aSocketDescriptor: TSocket; const key: ansiString)
      : ansiString; overload; virtual;
    function DoGet(aSocketDescriptor: TSocket; const keys: array of ansiString)
      : TAlMemCachedClient_StoredItems; overload; virtual;
    function DoGets(aSocketDescriptor: TSocket; const key: ansiString;
      var flags: integer; var cas_id: int64; var data: ansiString): Boolean;
      overload; virtual;
    function DoGets(aSocketDescriptor: TSocket; const keys: array of ansiString)
      : TAlMemCachedClient_StoredItems; overload; virtual;
    Procedure DoSet(aSocketDescriptor: TSocket; const key: ansiString;
      const flags: integer; const exptime: integer;
      const data: ansiString); virtual;
    procedure DoAdd(aSocketDescriptor: TSocket; const key: ansiString;
      const flags: integer; const exptime: integer;
      const data: ansiString); virtual;
    procedure DoReplace(aSocketDescriptor: TSocket; const key: ansiString;
      const flags: integer; const exptime: integer;
      const data: ansiString); virtual;
    procedure DoAppend(aSocketDescriptor: TSocket; const key: ansiString;
      const data: ansiString); virtual;
    procedure DoPrepend(aSocketDescriptor: TSocket; const key: ansiString;
      const data: ansiString); virtual;
    function DoCas(aSocketDescriptor: TSocket; const key: ansiString;
      const flags: integer; const exptime: integer; const cas_id: int64;
      const data: ansiString): Boolean; virtual;
    function DoDelete(aSocketDescriptor: TSocket; const key: ansiString)
      : Boolean; virtual;
    function DoIncr(aSocketDescriptor: TSocket; const key: ansiString;
      const Value: int64): int64; virtual;
    function DoDecr(aSocketDescriptor: TSocket; const key: ansiString;
      const Value: int64): int64; virtual;
    Procedure DoTouch(aSocketDescriptor: TSocket; const key: ansiString;
      const exptime: integer); virtual;
    Function DoStats(aSocketDescriptor: TSocket; const args: ansiString)
      : ansiString; virtual;
    procedure DoFlush_all(aSocketDescriptor: TSocket; delay: integer); virtual;
    Function DoVersion(aSocketDescriptor: TSocket): ansiString; virtual;
    procedure DoVerbosity(aSocketDescriptor: TSocket; level: integer); virtual;
  public
  ...
  public
    ...
    function Get(const key: ansiString; var flags: integer;
      var data: ansiString): Boolean; overload; virtual;
    function Get(const key: ansiString): ansiString; overload; virtual;
    function Get(const keys: array of ansiString)
      : TAlMemCachedClient_StoredItems; overload; virtual;
    function Gets(const key: ansiString; var flags: integer; var cas_id: int64;
      var data: ansiString): Boolean; overload; virtual;
    function Gets(const keys: array of ansiString)
      : TAlMemCachedClient_StoredItems; overload; virtual;
    Procedure _Set(const key: ansiString; const flags: integer;
      const exptime: integer; const data: ansiString); virtual;
    procedure Add(const key: ansiString; const flags: integer;
      const exptime: integer; const data: ansiString); virtual;
    procedure Replace(const key: ansiString; const flags: integer;
      const exptime: integer; const data: ansiString); virtual;
    procedure Append(const key: ansiString; const data: ansiString); virtual;
    procedure Prepend(const key: ansiString; const data: ansiString); virtual;
    function Cas(const key: ansiString; const flags: integer;
      const exptime: integer; const cas_id: int64; const data: ansiString)
      : Boolean; virtual;
    function Delete(const key: ansiString): Boolean; virtual;
    function Incr(const key: ansiString; const Value: int64): int64; virtual;
    function Decr(const key: ansiString; const Value: int64): int64; virtual;
    Procedure Touch(const key: ansiString; const exptime: integer); virtual;
...
{ ************************************************************************************************************** }
constructor EAlMemCachedClientException.Create(const aMsg: ansiString;
  const aCloseConnection: Boolean = False);
begin
...
begin
  if key = '' then
    raise EAlMemCachedClientException.Create('Key can not be empty');
  if Length(key) > 250 then
    raise EAlMemCachedClientException.Create
      ('Length limit of a key is 250 characters');
  for i := 1 to Length(key) do
    if (key[i] in [#0 .. #32]) then
      raise EAlMemCachedClientException.Create
...
Function TAlBaseMemCachedClient.SendCmd(aSocketDescriptor: TSocket;
  aCmd: ansiString; aResponseType: TAlMemCachedClient_responseType;
  var aGETStoredItems: TAlMemCachedClient_StoredItems): ansiString;
Var
  P: PAnsiChar;
  L: integer;
...
begin
  ...
  if aResponseType <> rpNONE then
    Result := GetResponse(aSocketDescriptor, aResponseType, aGETStoredItems)
  else
  begin
...
{ ***************************************************************** }
function TAlBaseMemCachedClient.SendCmd(aSocketDescriptor: TSocket;
  aCmd: ansiString; aResponseType: TAlMemCachedClient_responseType): ansiString;
Var
  aGETStoredItems: TAlMemCachedClient_StoredItems;
begin
  Result := SendCmd(aSocketDescriptor, aCmd, aResponseType, aGETStoredItems);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (aResponseType <> rpCRLF) then
        aResponseStatusStartPos := aResultPos - 2 // ...#13#10END#13#10
        // ^aResponseStatusStartPos
      else
        aResponseStatusStartPos := aResultPos - 1; // ...#13#10END#13#10
      // ^aResponseStatusStartPos
      while (aResponseStatusStartPos > 1) and
        (Result[aResponseStatusStartPos] <> #13) do
        dec(aResponseStatusStartPos); // ...#13#10END#13#10
      // ^^^^^^^^^aResponseStatusStartPos
      aResponseStatus := AlCopyStr(Result, aResponseStatusStartPos,
        aResultPos - aResponseStatusStartPos + 1); // #13#10END#13#10
      if (aResponseType <> rpCRLF) then
        aResponseStatus := ALTrim(aResponseStatus); // END
      ...
      // END\r\n
      //
      if (aResponseType = rpEND) and (aResponseStatus = 'END') then
      begin
      ...
      // OK\r\n
      //
      else if (aResponseType = rpOK) and (aResponseStatusStartPos = 1) and
        (aResponseStatus = 'OK') then
      begin
      ...
      // STORED\r\n
      //
      else if (aResponseType = rpSTORAGE) and (aResponseStatusStartPos = 1) and
        ((aResponseStatus = 'STORED') or (aResponseStatus = 'NOT_STORED') or
        (aResponseStatus = 'EXISTS') or (aResponseStatus = 'NOT_FOUND')) then
      begin
      ...
      // DELETED\r\n
      //
      else if (aResponseType = rpDELETE) and (aResponseStatusStartPos = 1) and
        ((aResponseStatus = 'DELETED') or (aResponseStatus = 'NOT_FOUND')) then
      begin
      ...
      // 130\r\n
      //
      else if (aResponseType = rpINCRDECR) and (aResponseStatusStartPos = 1) and
        ((ALIsInt64(aResponseStatus)) or (aResponseStatus = 'NOT_FOUND')) then
      begin
        Result := aResponseStatus; // DELETED
        Break;
      end
      ...
      //
        ...
        ((aResponseStatus = 'TOUCHED') or (aResponseStatus = 'NOT_FOUND')) then
      begin
      ...
      //
        ...
        (aResponseStatus = 'END') then
      begin

        // no item founded
        if aResponseStatusStartPos = 1 then
        begin
          Result := '';
          Break;
        end;
        ...
        // init 1rt item
        if aDataStartPos < 0 then
        begin
          aDataStartPos := 1;
          while (aDataStartPos < aResultPos) and
            (Result[aDataStartPos] <> #13) do
            inc(aDataStartPos);
          // VALUE hihi 0 4 12\r\nAZER\r\nVALUE hoho 0 10 13\r\nAZDFUJNERT\r\nVALUE haha 0 5 14\r\nAZIUD\r\nEND\r\n
          // ^^^^^^^^^^^^^^^^^^aDataStartPos
          ...
          try
            ...
            // 4
            // 12
            if aResponseType = rpRETRIEVALS then
            begin
              if (aLst.Count <> 5) or (not alTryStrToInt(aLst[2], aFlags)) or
                (not alTryStrToInt(aLst[3], aDataLength)) or
                (not alTryStrToInt64(aLst[4], aCasID)) then
                raise EALException.Create
                  ('Memcache Error: Response does not conform to protocol - ' +
                  AlCopyStr(Result, 1, aResultPos));
            end
            else
            begin
              aCasID := 0;
              if (aLst.Count <> 4) or (not alTryStrToInt(aLst[2], aFlags)) or
                (not alTryStrToInt(aLst[3], aDataLength)) then
                raise EALException.Create
                  ('Memcache Error: Response does not conform to protocol - ' +
                  AlCopyStr(Result, 1, aResultPos));
            end;
            aKey := aLst[1];
            aDataStartPos := aDataStartPos + 2;
            // VALUE hihi 0 4 12\r\nAZER\r\nVALUE hoho 0 10 13\r\nAZDFUJNERT\r\nVALUE haha 0 5 14\r\nAZIUD\r\nEND\r\n
            // ^ aDataStartPos
        ...
        while aDataStartPos + aDataLength < aResponseStatusStartPos do
        begin
          ...
          aGETStoredItems[high(aGETStoredItems)].data :=
            AlCopyStr(Result, aDataStartPos, aDataLength);

          aHeaderStartPos := aDataStartPos + aDataLength + 2;
          // VALUE hihi 0 4 12\r\nAZER\r\nVALUE hoho 0 10 13\r\nAZDFUJNERT\r\nVALUE haha 0 5 14\r\nAZIUD\r\nEND\r\n
          // ^aHeaderStartPos
          ...
          try
            aLst.LineBreak := ' ';
            aLst.Text := AlCopyStr(Result, aHeaderStartPos,
              aDataStartPos - aHeaderStartPos); // VALUE
            // hoho
            // 0
            // 10
            // 13
        ...
        if (aResponseStatusStartPos = aDataStartPos + aDataLength) then
        begin
          setlength(aGETStoredItems, Length(aGETStoredItems) + 1);
          aGETStoredItems[high(aGETStoredItems)].key := aKey;
          aGETStoredItems[high(aGETStoredItems)].flags := aFlags;
      ...
      //
      else if (aResponseType = rpCRLF) and (aResponseStatus = #13#10) then
      begin
        setlength(Result, aResponseStatusStartPos - 1);
        // VERSION 1.4.5_4_gaa7839e
        Break;
      end

      // ERROR Response
      else if (aResponseStatus = 'ERROR') and (aResponseStatusStartPos = 1) then
        raise EALException.Create('Memcached Error - Nonexistent command')
      else if (ALpos('SERVER_ERROR', aResponseStatus) = 1) and
        (aResponseStatusStartPos = 1) then
        raise EALException.Create('Memcached Error - Server error - ' +
          aResponseStatus)
      else if (ALpos('CLIENT_ERROR', aResponseStatus) = 1) and
        (aResponseStatusStartPos = 1) then
        raise EAlMemCachedClientException.Create
          ('Memcached Error - Client error in the input line - ' +
          aResponseStatus);
...
function TAlBaseMemCachedClient.DoGet(aSocketDescriptor: TSocket;
  const key: ansiString; var flags: integer; var data: ansiString): Boolean;
var
  aGETStoredItems: TAlMemCachedClient_StoredItems;
begin
  CheckKey(key);
  SendCmd(aSocketDescriptor, 'get ' + key, rpRETRIEVAL, aGETStoredItems);
  if Length(aGETStoredItems) <> 1 then
  begin
    Result := False;
    flags := 0;
    data := '';
...
begin
  ...
  SendCmd(aSocketDescriptor, 'get ' + key, rpRETRIEVAL, aGETStoredItems);
  if Length(aGETStoredItems) <> 1 then
    raise EAlMemCachedClientException.Create('Not found');
...
function TAlBaseMemCachedClient.DoGets(aSocketDescriptor: TSocket;
  const key: ansiString; var flags: integer; var cas_id: int64;
...
procedure TAlBaseMemCachedClient.DoSet(aSocketDescriptor: TSocket;
  const key: ansiString; const flags: integer; const exptime: integer;
  const data: ansiString);
var
  aResultCode: ansiString;
begin
  CheckKey(key);
  if flags < 0 then
    raise EAlMemCachedClientException.Create
      ('flags must be upper or equal to 0');
  aResultCode := SendCmd(aSocketDescriptor, 'set ' + key + ' ' +
    AlintToStr(flags) + ' ' + AlintToStr(exptime) + ' ' +
    AlintToStr(Length(data)) + #13#10 + data + #13#10, rpSTORAGE);
  if aResultCode <> 'STORED' then
    raise EAlMemCachedClientException.Create(aResultCode);
end;
...
begin
  ...
  if flags < 0 then
    raise EAlMemCachedClientException.Create
      ('flags must be upper or equal to 0');
  aResultCode := SendCmd(aSocketDescriptor, 'add ' + key + ' ' +
    AlintToStr(flags) + ' ' + AlintToStr(exptime) + ' ' +
    AlintToStr(Length(data)) + #13#10 + data + #13#10, rpSTORAGE);
  if aResultCode <> 'STORED' then
    raise EAlMemCachedClientException.Create(aResultCode);
...
begin
  ...
  if flags < 0 then
    raise EAlMemCachedClientException.Create
      ('flags must be upper or equal to 0');
  aResultCode := SendCmd(aSocketDescriptor, 'replace ' + key + ' ' +
    AlintToStr(flags) + ' ' + AlintToStr(exptime) + ' ' +
    AlintToStr(Length(data)) + #13#10 + data + #13#10, rpSTORAGE);
  if aResultCode <> 'STORED' then
    raise EAlMemCachedClientException.Create(aResultCode);
...
procedure TAlBaseMemCachedClient.DoAppend(aSocketDescriptor: TSocket;
  const key: ansiString; const data: ansiString);
var
  aResultCode: ansiString;
begin
  CheckKey(key);
  aResultCode := SendCmd(aSocketDescriptor, 'append ' + key + ' ' + '0 ' + '0 '
    + AlintToStr(Length(data)) + #13#10 + data + #13#10, rpSTORAGE);
  if aResultCode <> 'STORED' then
    raise EAlMemCachedClientException.Create(aResultCode);
end;
...
begin
  CheckKey(key);
  aResultCode := SendCmd(aSocketDescriptor, 'prepend ' + key + ' ' + '0 ' + '0 '
    + AlintToStr(Length(data)) + #13#10 + data + #13#10, rpSTORAGE);
  if aResultCode <> 'STORED' then
    raise EAlMemCachedClientException.Create(aResultCode);
end;
...
function TAlBaseMemCachedClient.DoCas(aSocketDescriptor: TSocket;
  const key: ansiString; const flags: integer; const exptime: integer;
  const cas_id: int64; const data: ansiString): Boolean;
var
  aResultCode: ansiString;
begin
  CheckKey(key);
  if flags < 0 then
    raise EAlMemCachedClientException.Create
      ('flags must be upper or equal to 0');
  aResultCode := SendCmd(aSocketDescriptor, 'cas ' + key + ' ' +
    AlintToStr(flags) + ' ' + AlintToStr(exptime) + ' ' +
    AlintToStr(Length(data)) + ' ' + AlintToStr(cas_id) + #13#10 + data +
    #13#10, rpSTORAGE);
  if aResultCode = 'EXISTS' then
    Result := False
  else if aResultCode = 'STORED' then
    Result := True
  else
    raise EAlMemCachedClientException.Create(aResultCode);
end;
...
begin
  ...
  if aResultCode = 'NOT_FOUND' then
    Result := False
  else if aResultCode = 'DELETED' then
    Result := True
  else
    raise EAlMemCachedClientException.Create(aResultCode);
end;
...
function TAlBaseMemCachedClient.DoIncr(aSocketDescriptor: TSocket;
  const key: ansiString; const Value: int64): int64;
var
  aResultCode: ansiString;
begin
  CheckKey(key);
  aResultCode := SendCmd(aSocketDescriptor, 'incr ' + key + ' ' +
    AlintToStr(Value), rpINCRDECR);
  if not alTryStrToInt64(aResultCode, Result) then
    raise EAlMemCachedClientException.Create(aResultCode);
end;
...
begin
  CheckKey(key);
  aResultCode := SendCmd(aSocketDescriptor, 'decr ' + key + ' ' +
    AlintToStr(Value), rpINCRDECR);
  if not alTryStrToInt64(aResultCode, Result) then
    raise EAlMemCachedClientException.Create(aResultCode);
end;
...
procedure TAlBaseMemCachedClient.DoTouch(aSocketDescriptor: TSocket;
  const key: ansiString; const exptime: integer);
var
  aResultCode: ansiString;
begin
  CheckKey(key);
  aResultCode := SendCmd(aSocketDescriptor, 'touch ' + key + ' ' +
    AlintToStr(exptime), rpTOUCH);
  if aResultCode <> 'TOUCHED' then
    raise EAlMemCachedClientException.Create(aResultCode);
end;
...
begin
  if delay > 0 then
    SendCmd(aSocketDescriptor, 'flush_all ' + AlintToStr(delay), rpOK)
  else
    SendCmd(aSocketDescriptor, 'flush_all', rpOK);
end;
...
begin
  if args <> '' then
    Result := SendCmd(aSocketDescriptor, 'stats ' + args, rpEND)
  else
    Result := SendCmd(aSocketDescriptor, 'stats', rpEND);
end;
...
// server.
function TAlBaseMemCachedClient.DoVersion(aSocketDescriptor: TSocket)
  : ansiString;
begin
...
{ ***************************************************************************************************************************************************** }
Function TAlBaseMemCachedClient.SocketWrite(aSocketDescriptor: TSocket;
  {$IF CompilerVersion >= 23}const {$ELSE}var
...
{ ***************************************************************************************************** }
function TAlBaseMemCachedClient.SocketRead(aSocketDescriptor: TSocket; var Buf;
  len: integer): integer;
begin
...
{ ************************************************************************************************** }
procedure TAlBaseMemCachedClient.DoSetSendTimeout(aSocketDescriptor: TSocket;
  const Value: integer);
begin
...
{ ***************************************************************************************************** }
procedure TAlBaseMemCachedClient.DoSetReceiveTimeout(aSocketDescriptor: TSocket;
  const Value: integer);
begin
...
procedure TAlBaseMemCachedClient.DoSetKeepAlive(aSocketDescriptor: TSocket;
  const Value: Boolean);
var
  aIntBool: integer;
begin
...
procedure TAlBaseMemCachedClient.DoSetTCPNoDelay(aSocketDescriptor: TSocket;
  const Value: Boolean);
var
  aIntBool: integer;
begin
...
{ ********************************************************************************** }
procedure TAlMemCachedClient.Connect(const aHost: ansiString;
  const APort: integer);
begin

  if Fconnected then
    raise EALException.Create('MemCached component already connected');
...
{ **************************************************** }
function TAlMemCachedClient.Get(const key: ansiString; var flags: integer;
  var data: ansiString): Boolean;
begin
  Result := DoGet(FSocketDescriptor, key, flags, data);
end;
...
{ *********************************************************************************************** }
function TAlMemCachedClient.Get(const keys: array of ansiString)
  : TAlMemCachedClient_StoredItems;
begin
  Result := DoGet(FSocketDescriptor, keys);
end;

{ ***************************************************** }
function TAlMemCachedClient.Gets(const key: ansiString; var flags: integer;
  var cas_id: int64; var data: ansiString): Boolean;
begin
  Result := DoGets(FSocketDescriptor, key, flags, cas_id, data);
end;

{ ************************************************************************************************ }
function TAlMemCachedClient.Gets(const keys: array of ansiString)
  : TAlMemCachedClient_StoredItems;
begin
...
{ ****************************************************** }
Procedure TAlMemCachedClient._Set(const key: ansiString; const flags: integer;
  const exptime: integer; const data: ansiString);
begin
  DoSet(FSocketDescriptor, key, flags, exptime, data);
end;

{ ***************************************************** }
procedure TAlMemCachedClient.Add(const key: ansiString; const flags: integer;
  const exptime: integer; const data: ansiString);
begin
  DoAdd(FSocketDescriptor, key, flags, exptime, data);
end;

{ ********************************************************* }
procedure TAlMemCachedClient.Replace(const key: ansiString;
  const flags: integer; const exptime: integer; const data: ansiString);
begin
  DoReplace(FSocketDescriptor, key, flags, exptime, data);
end;
...
procedure TAlMemCachedClient.Append(const key: ansiString;
  const data: ansiString);
begin
  DoAppend(FSocketDescriptor, key, data);
end;
...
procedure TAlMemCachedClient.Prepend(const key: ansiString;
  const data: ansiString);
begin
  DoPrepend(FSocketDescriptor, key, data);
end;

{ **************************************************** }
function TAlMemCachedClient.Cas(const key: ansiString; const flags: integer;
  const exptime: integer; const cas_id: int64; const data: ansiString): Boolean;
begin
  Result := DoCas(FSocketDescriptor, key, flags, exptime, cas_id, data);
end;
...
begin
  DoTouch(FSocketDescriptor, key, exptime);
end;
...
{ ********************************************************************************* }
procedure TAlMemCachedConnectionPoolClient.SetReceiveTimeout
  (const Value: integer);
begin
  FConnectionPoolCS.Acquire;
  Try
...
Var
  aConnectionPoolContainer: TAlMemCachedConnectionPoolContainer;
  aTickCount: int64;
Begin
...
{ ***************************************************************************************** }
procedure TAlMemCachedConnectionPoolClient.ReleaseConnection
  (var SocketDescriptor: TSocket; const CloseConnection: Boolean = False);
Var
  aConnectionPoolContainer: TAlMemCachedConnectionPoolContainer;
begin
...
{ ************************************************************************************************* }
constructor TAlMemCachedConnectionPoolClient.Create(const aHost: ansiString;
  const APort: integer);
begin
  inherited Create;
  fHost := aHost;
...
{ ************************************************************************************************************* }
procedure TAlMemCachedConnectionPoolClient.ReleaseAllConnections
...
function TAlMemCachedConnectionPoolClient.Get(const key: ansiString;
  var flags: integer; var data: ansiString): Boolean;
var
  aSocketDescriptor: TSocket;
begin
  aSocketDescriptor := AcquireConnection;
  try
    Result := DoGet(aSocketDescriptor, key, flags, data);
...
{ ************************************************************************************************************* }
function TAlMemCachedConnectionPoolClient.Get(const keys: array of ansiString)
  : TAlMemCachedClient_StoredItems;
var
  aSocketDescriptor: TSocket;
begin
...
begin
  aSocketDescriptor := AcquireConnection;
  try
    Result := DoGets(aSocketDescriptor, key, flags, cas_id, data);
    ReleaseConnection(aSocketDescriptor);
  except
    On E: exception do
    begin
...
{ ************************************************************************************************************** }
function TAlMemCachedConnectionPoolClient.Gets(const keys: array of ansiString)
  : TAlMemCachedClient_StoredItems;
var
  aSocketDescriptor: TSocket;
begin
...
begin
  aSocketDescriptor := AcquireConnection;
  try
    DoSet(aSocketDescriptor, key, flags, exptime, data);
    ReleaseConnection(aSocketDescriptor);
  except
    On E: exception do
    begin
...
begin
  aSocketDescriptor := AcquireConnection;
  try
    DoAdd(aSocketDescriptor, key, flags, exptime, data);
    ReleaseConnection(aSocketDescriptor);
  except
    On E: exception do
    begin
...
begin
  aSocketDescriptor := AcquireConnection;
  try
    DoReplace(aSocketDescriptor, key, flags, exptime, data);
    ReleaseConnection(aSocketDescriptor);
  except
    On E: exception do
    begin
...
begin
  aSocketDescriptor := AcquireConnection;
  try
    DoAppend(aSocketDescriptor, key, data);
    ReleaseConnection(aSocketDescriptor);
  except
    On E: exception do
    begin
...
begin
  aSocketDescriptor := AcquireConnection;
  try
    DoPrepend(aSocketDescriptor, key, data);
    ReleaseConnection(aSocketDescriptor);
  except
    On E: exception do
    begin
...
begin
  aSocketDescriptor := AcquireConnection;
  try
    Result := DoCas(aSocketDescriptor, key, flags, exptime, cas_id, data);
    ReleaseConnection(aSocketDescriptor);
  except
    On E: exception do
    begin
...
{ ******************************************************************************* }
function TAlMemCachedConnectionPoolClient.Delete(const key: ansiString)
  : Boolean;
var
  aSocketDescriptor: TSocket;
begin
...
begin
  aSocketDescriptor := AcquireConnection;
  try
    DoTouch(aSocketDescriptor, key, exptime);
    ReleaseConnection(aSocketDescriptor);
  except
    On E: exception do
    begin
...
{ ********************************************************************************** }
function TAlMemCachedConnectionPoolClient.Stats(const args: ansiString)
  : ansiString;
var
  aSocketDescriptor: TSocket;
begin
