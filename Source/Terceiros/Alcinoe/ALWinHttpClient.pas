...
type
  ...
  { --------------------------------------------------------- }
  TALWinHttpClientInternetOpenAccessType = (wHttpAt_NO_PROXY,
    { Resolves all host names locally. }
    wHttpAt_DEFAULT_PROXY,
  ...
  TAlWinHttpClientInternetOption = (wHttpIo_Async, { NOT SUPPORTED YET!
    ...
    wHttpIo_BYPASS_PROXY_CACHE,
    { This flag provides the same behavior as WINHTTP_FLAG_REFRESH. }
    wHttpIo_ESCAPE_DISABLE,
    { Unsafe characters in the URL passed in for pwszObjectName are not converted to
      escape sequences. }
    wHttpIo_ESCAPE_DISABLE_QUERY,
    { Unsafe characters in the query component of the URL passed in for pwszObjectName are not
      converted to escape sequences. }
    wHttpIo_ESCAPE_PERCENT,
    { The string passed in for pwszObjectName is converted from an LPCWSTR to an LPSTR.
      ...
      sequence. }
    wHttpIo_NULL_CODEPAGE,
    { The string passed in for pwszObjectName is assumed to consist of valid ANSI characters
    ...
    wHttpIo_SECURE,
    { Uses secure transaction semantics. This translates to using Secure Sockets Layer
  ...
  TALWinHttpClientStatusChangeEvent = procedure(sender: Tobject;
  ...
  private
    ...
    procedure SetAccessType(const Value
  ...
  public
    ...
    property AccessType: TALWinHttpClientInternetOpenAccessType read FAccessType
      write SetAccessType default wHttpAt_NO_PROXY;
    property InternetOptions: TALWinHttpClientInternetOptionSet
      read FInternetOptions write FInternetOptions
    ...
    // Winhttp seam to handle internally the disconnection/reconnection !
    property OnStatusChange: TALWinHttpClientStatusChangeEvent
...
begin
  if Context = 0 then
    exit;
  with TALWinHttpClient(Context) do
  begin

    { fire the On Status change event }
    if Assigned(FOnStatusChange) then
      FOnStatusChange(TALWinHttpClient(Context), InternetStatus,
        StatusInformation, StatusInformationLength);

    { fire the On redirect event }
    If (InternetStatus = WINHTTP_CALLBACK_STATUS_REDIRECT) and
      Assigned(OnRedirect) then
    begin
      OnRedirect(TALWinHttpClient(Context),
        AnsiString(PWideChar(InternetStatus)));
    end;
...
begin
  ...
  begin
    ...
    FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM or FORMAT_MESSAGE_FROM_HMODULE,
      Pointer(GetModuleHandle('winhttp.dll')), ErrCode, 0, PAnsiChar(S),
      Length(S), nil);
    SetLength(S,
...
{ --------------------------------------------- }
  ...
  Begin
    If (FAccessType <> wHttpAt_NAMED_PROXY) or (ProxyParams.ProxyServer = '')
    then
      result := WINHTTP_NO_PROXY_NAME
    else
      result := PWideChar(WideString(ProxyParams.ProxyServer) + ':' +
        WideString(ALIntToStr(ProxyParams.ProxyPort)));
  end;
...
{ ----------------------------------------- }
  ...
  Begin
    ...
    if (FAccessType <> wHttpAt_NAMED_PROXY) or (ProxyParams.ProxyBypass = '')
    then
      result := WINHTTP_NO_PROXY_BYPASS
    else
      result := PWideChar(WideString(ProxyParams.ProxyBypass));
  end;

const
  AccessTypeArr: Array [TALWinHttpClientInternetOpenAccessType]
    of DWORD = (WINHTTP_ACCESS_TYPE_NO_PROXY, WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
    WINHTTP_ACCESS_TYPE_NAMED_PROXY);

var
  WinHttpSetStatusCallbackResult: PFNWinHttpStatusCallback;
...
begin
  ...
  try
    ...
    begin
      WinHttpSetStatusCallbackResult := WinHttpSetStatusCallback(FInetRoot,
        @ALWinHTTPCLientStatusCallback,
        WINHTTP_CALLBACK_FLAG_ALL_NOTIFICATIONS, 0);
      CheckError(WinHttpSetStatusCallbackResult = Pointer
        (WINHTTP_INVALID_STATUS_CALLBACK));
    end;

    { init FInetConnect }
    FInetConnect := WinHttpConnect(FInetRoot, PWideChar(WideString(FURLHost)),
      FURLPort, 0);
...
begin
  if Assigned(FInetConnect) then
    WinHttpCloseHandle(FInetConnect);
  FInetConnect := nil;
...
{ ---------------------------------------------- }
  ...
  Begin
    result := 0;
    If wHttpIo_BYPASS_PROXY_CACHE in InternetOptions then
      result := result or WINHTTP_FLAG_BYPASS_PROXY_CACHE;
    If wHttpIo_ESCAPE_DISABLE in InternetOptions then
      result := result or WINHTTP_FLAG_ESCAPE_DISABLE;
    If wHttpIo_ESCAPE_DISABLE_QUERY in InternetOptions then
      result := result or WINHTTP_FLAG_ESCAPE_DISABLE_QUERY;
    If wHttpIo_ESCAPE_PERCENT in InternetOptions then
      result := result or WINHTTP_FLAG_ESCAPE_PERCENT;
    If wHttpIo_NULL_CODEPAGE in InternetOptions then
      result := result or WINHTTP_FLAG_NULL_CODEPAGE;
    If wHttpIo_REFRESH in InternetOptions then
      result := result or WINHTTP_FLAG_REFRESH;
    if (wHttpIo_SECURE in InternetOptions) or
      (FURLScheme = INTERNET_SCHEME_HTTPS) then
      result := result or WINHTTP_FLAG_SECURE;
  end;
...
begin
  ...
  If ProtocolVersion = HTTPpv_1_1 then
    StrProtocolVersion := 'HTTP/1.1'
  else
    StrProtocolVersion := 'HTTP/1.0';

  if RequestMethod = HTTPmt_Post then
    StrVerb := 'POST'
  else if RequestMethod = HTTPmt_Get then
    StrVerb := 'GET'
  else if RequestMethod = HTTPmt_head then
    StrVerb := 'HEAD'
  else if RequestMethod = HTTPmt_trace then
    StrVerb := 'TRACE'
  else if RequestMethod = HTTPmt_Put then
    StrVerb := 'PUT'
  else if RequestMethod = HTTPmt_Delete then
    StrVerb := 'DELETE'
  else
  ...
  Request := nil;
  try
    Request := WinHttpOpenRequest(FInetConnect, PWideChar(StrVerb),
      PWideChar(WideString(FURLSite)), PWideChar(StrProtocolVersion),
      PWideChar(WideString(RequestHeader.Referer)), Pointer(AcceptTypes),
    ...
    if ConnectTimeout > 0 then
      CheckError(not WinHttpSetOption(Request, WINHTTP_OPTION_CONNECT_TIMEOUT,
        Pointer(@ConnectTimeout), SizeOf(ConnectTimeout)));
    if SendTimeout > 0 then
      CheckError(not WinHttpSetOption(Request, WINHTTP_OPTION_SEND_TIMEOUT,
        Pointer(@SendTimeout), SizeOf(SendTimeout)));
    if ReceiveTimeout > 0 then
      CheckError(not WinHttpSetOption(Request, WINHTTP_OPTION_RECEIVE_TIMEOUT,
        Pointer(@ReceiveTimeout), SizeOf(ReceiveTimeout)));

    { proxy user name and password }
    If ProxyParams.ProxyUserName <> '' then
      CheckError(not WinHttpSetOption(Request, WINHTTP_OPTION_PROXY_USERNAME,
        PWideChar(WideString(ProxyParams.ProxyUserName)),
        Length(ProxyParams.ProxyUserName)));
    If ProxyParams.ProxyPassword <> '' then
      CheckError(not WinHttpSetOption(Request, WINHTTP_OPTION_PROXY_PASSWORD,
        PWideChar(WideString(ProxyParams.ProxyPassword)),
        Length(ProxyParams.ProxyPassword)));

    { wHttpIo_Keep_connection, wHttpIo_No_cookies and wHttpIo_No_auto_redirect }
    If not(wHttpIo_Keep_connection in InternetOptions) then
    begin
      aDWORDOptions := WINHTTP_DISABLE_KEEP_ALIVE;
      CheckError(not WinHttpSetOption(Request, WINHTTP_OPTION_DISABLE_FEATURE,
        Pointer(@aDWORDOptions), SizeOf(aDWORDOptions)));
    end;
    ...
    WinHttpAddRequestHeaders(Request, PWideChar(aHeader), Length(aHeader),
    ...
    begin
      ...
      try
        ...
        CheckError(not WinHttpSendRequest(Request,
          WINHTTP_NO_ADDITIONAL_HEADERS, 0, nil, 0, BuffSize, DWORD_PTR(self)));
        ...
        try
          ...
          begin
            ...
            CheckError(not WinHttpWriteData(Request, @Buffer.Memory^,
              Len, RetVal));
    ...
    begin
      ...
      try
        ...
        CheckError(not WinHttpSendRequest(Request,
          WINHTTP_NO_ADDITIONAL_HEADERS, 0, @StrStr.DataString[1],
          Length(StrStr.DataString), Length(StrStr.DataString),
          DWORD_PTR(self)));
    ...
    begin
      CheckError(not WinHttpSendRequest(Request, WINHTTP_NO_ADDITIONAL_HEADERS,
        0, nil, 0, 0, DWORD_PTR(self)));
      CheckError(not WinHttpReceiveResponse(Request, nil));
    end;
...
var
  Size, Downloaded, Status, Len, ContentlengthDownloaded, ContentLength: DWORD;
  aWideStr: WideString;
  aStr: AnsiString;
begin
  ...
  begin
    ...
    begin
      ...
      If Boolean(WinHttpQueryHeaders(Pointer(aContext),
        ...
        @aWideStr[1], Size, WINHTTP_NO_HEADER_INDEX)) then
      begin
        SetLength(aWideStr, Size div SizeOf(wideChar));
  ...
  Len := SizeOf(Status);
  if Boolean(WinHttpQueryHeaders(Pointer(aContext), WINHTTP_QUERY_STATUS_CODE or
    WINHTTP_QUERY_FLAG_NUMBER, WINHTTP_HEADER_NAME_BY_INDEX, @Status, Len,
    WINHTTP_NO_HEADER_INDEX)) and (Status >= 300) then
  begin
    ...
    begin
      SetLength(aWideStr, Size div SizeOf(wideChar));
      if Boolean(WinHttpQueryHeaders(Pointer(aContext),
        WINHTTP_QUERY_STATUS_TEXT, WINHTTP_HEADER_NAME_BY_INDEX, @aWideStr[1],
        Size, WINHTTP_NO_HEADER_INDEX)) then
      begin
        SetLength(aWideStr, Size div SizeOf(wideChar));
        raise EALHTTPClientException.CreateFmt('%s (%d) - ''%s''',
          [aWideStr, Status, URL], Status);
      end
      else
        CheckError(GetLastError <> ERROR_INSUFFICIENT_BUFFER);
    end;
  ...
  if not WinHttpQueryHeaders(Pointer(aContext), WINHTTP_QUERY_CONTENT_LENGTH or
    WINHTTP_QUERY_FLAG_NUMBER, WINHTTP_HEADER_NAME_BY_INDEX, @ContentLength,
    Len, WINHTTP_NO_HEADER_INDEX) then
    ContentLength := 0;
  ...
  repeat
    ...
    begin
      SetLength(aStr, Size);
      CheckError(not WinHttpReadData(Pointer(aContext), @aStr[1], Size,
        Downloaded));
...
{ ******************************************************************************************** }
procedure TALWinHttpClient.SetAccessType(const Value
  : TALWinHttpClientInternetOpenAccessType);
begin
...
{ ******************************************************************************************* }
procedure TALWinHttpClient.SetOnStatusChange(const Value
  : TALWinHttpClientStatusChangeEvent);
begin
  Disconnect;
...
{ ******************************************************************************** }
procedure TALWinHttpClient.SetOnRedirect(const Value
...
{ ******************************************************************************************** }
procedure TALWinHttpClient.OnProxyParamsChange(sender: Tobject;
...
{ ********************************************************************************************** }
procedure TALWinHttpClient.OnRequestHeaderChange(sender: Tobject;
  Const PropertyIndex: Integer);
begin
