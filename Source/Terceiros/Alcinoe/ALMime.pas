...
procedure ALMimeBase64Encode(const InputBuffer: TByteDynArray;
  InputOffset: NativeInt; const InputByteCount: NativeInt;
  out OutputBuffer: TByteDynArray; OutputOffset: NativeInt = 0); overload;
procedure ALMimeBase64EncodeNoCRLF(const InputBuffer: TByteDynArray;
  InputOffset: NativeInt; const InputByteCount: NativeInt;
  out OutputBuffer: TByteDynArray; OutputOffset: NativeInt = 0); overload;
procedure ALMimeBase64EncodeFullLines(const InputBuffer: TByteDynArray;
  InputOffset: NativeInt; const InputByteCount: NativeInt;
  out OutputBuffer: TByteDynArray; OutputOffset: NativeInt = 0); overload;
function ALMimeBase64Decode(const InputBuffer: TByteDynArray;
  InputOffset: NativeInt; const InputByteCount: NativeInt;
  out OutputBuffer: TByteDynArray; OutputOffset: NativeInt = 0)
  : NativeInt; overload;
function ALMimeBase64DecodePartial(const InputBuffer: TByteDynArray;
  InputOffset: NativeInt; const InputByteCount: NativeInt;
...
function ALMimeBase64DecodePartialEnd(out OutputBuffer: TByteDynArray;
  OutputOffset: NativeInt; const ByteBuffer: Cardinal;
  const ByteBufferSpace: Cardinal): NativeInt; overload;

procedure ALMimeBase64Encode(const InputBuffer: TByteDynArray;
  const InputByteCount: NativeInt; out OutputBuffer: TByteDynArray); overload;
procedure ALMimeBase64EncodeNoCRLF(const InputBuffer: TByteDynArray;
  const InputByteCount: NativeInt; out OutputBuffer: TByteDynArray); overload;
procedure ALMimeBase64EncodeFullLines(const InputBuffer: TByteDynArray;
  const InputByteCount: NativeInt; out OutputBuffer: TByteDynArray); overload;
function ALMimeBase64Decode(const InputBuffer: TByteDynArray;
  const InputByteCount: NativeInt; out OutputBuffer: TByteDynArray)
  : NativeInt; overload;
function ALMimeBase64DecodePartial(const InputBuffer: TByteDynArray;
  const InputByteCount: NativeInt; out OutputBuffer: TByteDynArray;
  var ByteBuffer: Cardinal; var ByteBufferSpace: Cardinal): NativeInt; overload;
...
procedure ALMimeBase64Encode(const InputBuffer; const InputByteCount: NativeInt;
  out OutputBuffer); overload;
procedure ALMimeBase64EncodeNoCRLF(const InputBuffer;
  const InputByteCount: NativeInt; out OutputBuffer); overload;
procedure ALMimeBase64EncodeFullLines(const InputBuffer;
  const InputByteCount: NativeInt; out OutputBuffer); overload;
function ALMimeBase64Decode(const InputBuffer; const InputByteCount: NativeInt;
  out OutputBuffer): NativeInt; overload;
function ALMimeBase64DecodePartial(const InputBuffer;
  const InputByteCount: NativeInt; out OutputBuffer; var ByteBuffer: Cardinal;
  var ByteBufferSpace: Cardinal): NativeInt; overload;
function ALMimeBase64DecodePartialEnd(out OutputBuffer;
  const ByteBuffer: Cardinal; const ByteBufferSpace: Cardinal)
  : NativeInt; overload;

procedure ALMimeBase64EncodeFile(const InputFileName, OutputFileName
  : TFileName);
procedure ALMimeBase64EncodeFileNoCRLF(const InputFileName, OutputFileName
  : TFileName);
procedure ALMimeBase64DecodeFile(const InputFileName, OutputFileName
  : TFileName);
procedure ALMimeBase64EncodeStream(const InputStream: TStream;
  const OutputStream: TStream);
procedure ALMimeBase64EncodeStreamNoCRLF(const InputStream: TStream;
  const OutputStream: TStream);
procedure ALMimeBase64DecodeStream(const InputStream: TStream;
  const OutputStream: TStream);
...
Function ALGetDefaultFileExtFromMimeContentType(aContentType: AnsiString)
  : AnsiString;
Function ALGetDefaultMIMEContentTypeFromExt(aExt: AnsiString): AnsiString;

Var
  vAlMimeContentTypeByExtList: TALStrings; { .htm=text/html }
...
const
  ...
  cALMimeBase64_ENCODE_TABLE: array [0 .. 63] of Byte = (065, 066, 067, 068,
  ...
  cALMimeBase64_DECODE_TABLE: array [Byte] of Byte = (255, 255, 255, 255, 255,
    255, 255, 255, // 0 -   7
...
{ ******************************************************************************************** }
function ALMimeBase64Decode(const InputBuffer: TByteDynArray;
  const InputByteCount: NativeInt; out OutputBuffer: TByteDynArray): NativeInt;
begin
...
begin
  Result := ALMimeBase64DecodePartial(InputBuffer, 0, InputByteCount,
    OutputBuffer, 0, ByteBuffer, ByteBufferSpace);
end;
...
begin
  Result := ALMimeBase64DecodePartialEnd(OutputBuffer, 0, ByteBuffer,
    ByteBufferSpace);
end;
...
begin
  if InputSize > 0 then
    Result := (InputSize + 2) div 3 * 4 + (InputSize - 1)
      div cALMimeBase64_DECODED_LINE_BREAK * 2
  else
    Result := InputSize;
end;
...
begin
  ALMimeBase64EncodeFullLines(InputBuffer, InputOffset, InputByteCount,
    OutputBuffer, OutputOffset);
  IDelta := InputByteCount div cALMimeBase64_DECODED_LINE_BREAK;
  ...
  ALMimeBase64EncodeNoCRLF(InputBuffer, InputOffset + IDelta,
    InputByteCount - IDelta, OutputBuffer, OutputOffset + ODelta);
end;
...
{ ********************************************************************************************************** }
procedure ALMimeBase64EncodeFullLines(const InputBuffer;
  const InputByteCount: NativeInt; out OutputBuffer);
var
  B: Cardinal;
  ...
  case InputByteCount - OuterLimit of
    1:
      begin
        ...
        B := B shr 6;
        OutputBuffer[OutIndex + 0] := cALMimeBase64_ENCODE_TABLE[B];
        OutputBuffer[OutIndex + 2] := cALMimeBase64_PAD_CHAR;
...
function ALMimeBase64Decode(const InputBuffer: TByteDynArray;
  InputOffset: NativeInt; const InputByteCount: NativeInt;
  out OutputBuffer: TByteDynArray; OutputOffset: NativeInt): NativeInt;
var
  ByteBuffer, ByteBufferSpace: Cardinal;
begin
  ByteBuffer := 0;
  ByteBufferSpace := 4;
  Result := ALMimeBase64DecodePartial(InputBuffer, InputOffset, InputByteCount,
    OutputBuffer, OutputOffset, ByteBuffer, ByteBufferSpace);
  Inc(Result, ALMimeBase64DecodePartialEnd(OutputBuffer, OutputOffset + Result,
...
begin
  ByteBuffer := 0;
  ByteBufferSpace := 4;
  Result := ALMimeBase64DecodePartial(InputBuffer, InputByteCount, OutputBuffer,
    ByteBuffer, ByteBufferSpace);
  O := @OutputBuffer;
  Inc(O, Result);
...
function ALMimeBase64DecodePartial(const InputBuffer;
  const InputByteCount: NativeInt; out OutputBuffer; var ByteBuffer: Cardinal;
  var ByteBufferSpace: Cardinal): NativeInt;
var
  LByteBuffer, LByteBufferSpace, C: Cardinal;
...
var
  ...
  OutputBuffer: array [0 .. (cALMimeBase64_BUFFER_SIZE + 2) div 3 * 4 +
    cALMimeBase64_BUFFER_SIZE div cALMimeBase64_DECODED_LINE_BREAK * 2 -
    1] of Byte;
  BytesRead: NativeInt;
  IDelta, ODelta: NativeInt;
  I, O: PByte;
begin
  ...
  IDelta := BytesRead div cALMimeBase64_DECODED_LINE_BREAK;
...
var
  ByteBuffer, ByteBufferSpace: Cardinal;
  InputBuffer: array [0 .. cALMimeBase64_BUFFER_SIZE - 1] of Byte;
  OutputBuffer: array [0 .. (cALMimeBase64_BUFFER_SIZE + 3) div 4 * 3 -
    1] of Byte;
  BytesRead: NativeInt;
begin
  ByteBuffer := 0;
  ...
  begin
    OutputStream.Write(OutputBuffer, ALMimeBase64DecodePartial(InputBuffer,
      BytesRead, OutputBuffer, ByteBuffer, ByteBufferSpace));
    BytesRead := InputStream.Read(InputBuffer, Length(InputBuffer));
  end;
  OutputStream.Write(OutputBuffer, ALMimeBase64DecodePartialEnd(OutputBuffer,
    ByteBuffer, ByteBufferSpace));
end;
  ...
  try
    ...
    try
      ...
      begin
        ...
        begin
          aExt := KeyList[I];
          if (Length(aExt) > 1) and (aExt[1] = '.') then
          begin
            ...
            begin
              ...
              begin
                ...
                if aContentType <> '' then
                  AMIMEList.Values[alLowerCase(AnsiString(aExt))] :=
                    alLowerCase(AnsiString(aContentType));
              end;
              reg.CloseKey;
            end;
          end;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          If aContentType <> '' then
          begin
            if reg.OpenKeyReadOnly('\MIME\Database\Content Type\' + aContentType)
            then
            begin
              ...
              begin
                ...
                begin
                  If (aExt[1] <> '.') then
                    aExt := '.' + aExt;
                  AMIMEList.Values[alLowerCase(AnsiString(aContentType))] :=
                    alLowerCase(AnsiString(aExt))
                end;
              end;
              reg.CloseKey;
            end;
...
begin
  ...
  If (LExt = '') or (LExt[1] <> '.') then
    LExt := '.' + LExt;
  Index := vAlMimeContentTypeByExtList.IndexOfName(LExt);
  if Index <> -1 then
    Result := vAlMimeContentTypeByExtList.ValueFromIndex[Index]
  else
