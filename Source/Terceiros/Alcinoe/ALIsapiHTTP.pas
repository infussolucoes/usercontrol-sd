...
type
  ...
  public
    ...
    procedure ExtractMultipartFormDataFields(Fields: TALStrings;
    ...
    function TranslateURI(const URI: AnsiString): AnsiString; virtual; abstract;
    // Translate a relative URI to a local absolute path
    function WriteClient(var Buffer; Count: integer): integer; virtual;
      abstract; // Write count bytes back to client
    ...
    function WriteHeaders(StatusCode: integer;
      const ReasonString, Headers: AnsiString): Boolean; virtual; abstract;
    // Write HTTP header string
    function GetFieldByName(const Name: AnsiString): AnsiString; virtual;
      abstract; // Read an arbitrary HTTP/Server Field not lists here
    property MethodType: TALHTTPMethod read GetMethodType;
    ...
    // [added from TwebRequest]
    Property MaxContentSize: integer Read FMaxContentSize Write FMaxContentSize;
    // [added from TwebRequest]
  ...
  public
    ...
    // if you use readClient then you need to avoid to use ContentStream
    function ReadString(Count: integer): AnsiString; override;
    // if you use ReadString then you need to avoid to use ContentStream
    ...
    function WriteHeaders(StatusCode: integer;
      const StatusString, Headers: AnsiString): Boolean; override;
  ...
  public
    ...
    procedure SetCookieField(Values: TALStrings;
      const ADomain, APath: AnsiString; AExpires: TDateTime; ASecure: Boolean);
    ...
    property ProtocolVersion: AnsiString index 0 read GetStringVariable
      write SetStringVariable; // [Renamed from TwebRequest] Version
    property ReasonString: AnsiString index 1 read GetStringVariable
      write SetStringVariable;
    property Server: AnsiString index 2 read GetStringVariable
      write SetStringVariable;
    property WWWAuthenticate: AnsiString index 3 read GetStringVariable
      write SetStringVariable;
    property Realm: AnsiString index 4 read GetStringVariable
      write SetStringVariable;
    property Allow: AnsiString index 5 read GetStringVariable
      write SetStringVariable;
    property Location: AnsiString index 6 read GetStringVariable
      write SetStringVariable;
    property ContentEncoding: AnsiString index 7 read GetStringVariable
      write SetStringVariable;
    property ContentType: AnsiString index 8 read GetStringVariable
      write SetStringVariable;
    property ContentVersion: AnsiString index 9 read GetStringVariable
      write SetStringVariable;
    property DerivedFrom: AnsiString index 10 read GetStringVariable
      write SetStringVariable;
    property Title: AnsiString index 11 read GetStringVariable
      write SetStringVariable;
    property StatusCode: integer read GetStatusCode write SetStatusCode;
    property ContentLength: integer index 0 read GetIntegerVariable
      write SetIntegerVariable;
    property Date: TDateTime index 0 read GetDateVariable write SetDateVariable;
    property Expires: TDateTime index 1 read GetDateVariable
      write SetDateVariable;
    property LastModified: TDateTime index 2 read GetDateVariable
      write SetDateVariable;
    ...
    property FreeContentStream: Boolean read FFreeContentStream
  ...
  protected
    ...
    procedure SetStringVariable(Index: integer;
...
const
  CALWebRequestServerVariables: array [0 .. 28] of AnsiString = ('',
    'SERVER_PROTOCOL', 'URL', '', '', '', 'HTTP_CACHE_CONTROL', 'HTTP_DATE',
    'HTTP_ACCEPT', 'HTTP_FROM', 'HTTP_HOST', 'HTTP_IF_MODIFIED_SINCE',
    'HTTP_REFERER', 'HTTP_USER_AGENT', 'HTTP_CONTENT_ENCODING', 'CONTENT_TYPE',
    'CONTENT_LENGTH', 'HTTP_CONTENT_VERSION', 'HTTP_DERIVED_FROM',
    'HTTP_EXPIRES', 'HTTP_TITLE', 'REMOTE_ADDR', 'REMOTE_HOST', 'SCRIPT_NAME',
    'SERVER_PORT', '', 'HTTP_CONNECTION', 'HTTP_COOKIE', 'HTTP_AUTHORIZATION');
...
{ ************************************************************************************************************** }
procedure TALWebRequest.ExtractMultipartFormDataFields(Fields: TALStrings;
  Files: TALMultiPartFormDataContents);
var
...
begin
  ...
  If aBoundary = '' then
    raise Exception.Create('Bad multipart/form-data Content-Type');
  aMultipartFormDataDecoder := TALMultipartFormDataDecoder.Create;
  Try
    aMultipartFormDataDecoder.Decode(Content, aBoundary, Fields, Files);
  Finally
...
var
  aMethodStr: AnsiString;
begin
  aMethodStr := Method;
  if ALSameText(aMethodStr, 'GET') then
    Result := HTTPmt_Get
  else if ALSameText(aMethodStr, 'POST') then
    Result := HTTPmt_Post
  else if ALSameText(aMethodStr, 'PUT') then
    Result := HTTPmt_Put
  else if ALSameText(aMethodStr, 'HEAD') then
    Result := HTTPmt_Head
  else if ALSameText(aMethodStr, 'TRACE') then
    Result := HTTPmt_Trace
  else if ALSameText(aMethodStr, 'DELETE') then
    Result := HTTPmt_Delete
  else
...
begin
  ...
  if ECB.GetServerVariable(ECB.ConnID, PAnsiChar(Name), @Buffer, Size) or
    ECB.GetServerVariable(ECB.ConnID, PAnsiChar(AnsiString('HTTP_') + Name),
    @Buffer, Size) then
  begin
    if Size > 0 then
      Dec(Size);
...
begin
  case Index of
    ...
    10:
      Result := GetHost;
    1 .. 2, 6 .. 9, 11 .. 24, 26 .. 28:
      Result := GetFieldByName(CALWebRequestServerVariables[Index]);
    25:
...
begin
  ...
  begin
    ...
    If (FMaxContentSize > -1) and (ECB.cbTotalBytes > DWORD(FMaxContentSize))
    then
      Raise EALIsapiRequestContentSizeTooBig.Create
        ('Content size (' + inttostr(ECB.cbTotalBytes) +
        ' bytes) is bigger than the maximum allowed size (' +
        inttostr(FMaxContentSize) + ' bytes)');

    FcontentStream := TmemoryStream.Create;
    FcontentStream.Size := ECB.cbTotalBytes;
    ...
    if ECB.cbAvailable > 0 then
      FcontentStream.Write(ECB.lpbData^, ECB.cbAvailable);
    // The available number of bytes (out of a total of cbTotalBytes) in the buffer pointed to by lpbData.
    ...
    while FcontentStream.Position < FcontentStream.Size do
    begin
      aByteRead := ReadClient
        (Pointer(Longint(TmemoryStream(FcontentStream).Memory) +
        FcontentStream.Position)^, FcontentStream.Size -
        FcontentStream.Position);
      if aByteRead <= 0 then
        break; // The doc of Delphi say "If no more content is available, ReadClient returns -1."
      // but it's false !!
    ...
    if ContentLength > FcontentStream.Size then
      raise EALIsapiRequestConnectionDropped.Create
...
begin
  ...
  if Value <> '' then
    Result := ALRfc822StrToGMTDateTime(Value)
  else
    Result := -1;
end;
...
begin
  ...
  with TALISAPIRequest(Self) do
    ECB.ServerSupportFunction(ECB.ConnID, HSE_REQ_SEND_RESPONSE_HEADER,
      PAnsiChar(StatusString), nil, LPDWORD(Headers));
  Result := True;
end;
...
begin
  if Realm <> '' then
    Result := ALFormat('%s Realm=%s', [WWWAuthenticate, Realm])
  else
...
begin
  case StatusCode of
    100:
      Result := 'Continue'; { do not localize }
    101:
      Result := 'Switching Protocols'; { do not localize }
    200:
      Result := 'OK'; { do not localize }
    201:
      Result := 'Created'; { do not localize }
    202:
      Result := 'Accepted'; { do not localize }
    203:
      Result := 'Non-Authoritative Information'; { do not localize }
    204:
      Result := 'No Content'; { do not localize }
    205:
      Result := 'Reset Content'; { do not localize }
    206:
      Result := 'Partial Content'; { do not localize }
    300:
      Result := 'Multiple Choices'; { do not localize }
    301:
      Result := 'Moved Permanently'; { do not localize }
    302:
      Result := 'Moved Temporarily'; { do not localize }
    303:
      Result := 'See Other'; { do not localize }
    304:
      Result := 'Not Modified'; { do not localize }
    305:
      Result := 'Use Proxy'; { do not localize }
    400:
      Result := 'Bad Request'; { do not localize }
    401:
      Result := 'Unauthorized'; { do not localize }
    402:
      Result := 'Payment Required'; { do not localize }
    403:
      Result := 'Forbidden'; { do not localize }
    404:
      Result := 'Not Found'; { do not localize }
    405:
      Result := 'Method Not Allowed'; { do not localize }
    406:
      Result := 'None Acceptable'; { do not localize }
    407:
      Result := 'Proxy Authentication Required'; { do not localize }
    408:
      Result := 'Request Timeout'; { do not localize }
    409:
      Result := 'Conflict'; { do not localize }
    410:
      Result := 'Gone'; { do not localize }
    411:
      Result := 'Length Required'; { do not localize }
    412:
      Result := 'Unless True'; { do not localize }
    500:
      Result := 'Internal Server Error'; { do not localize }
    501:
      Result := 'Not Implemented'; { do not localize }
    502:
      Result := 'Bad Gateway'; { do not localize }
    503:
      Result := 'Service Unavailable'; { do not localize }
    504:
      Result := 'Gateway Timeout'; { do not localize }
  else
    Result := '';
  end
end;
...
begin
  if (Index >= Low(FIntegerVariables)) and (Index <= High(FIntegerVariables))
  then
...
{ ********************************************************************************* }
procedure TALISAPIResponse.SetDateVariable(Index: integer;
...
{ ************************************************************************************ }
procedure TALISAPIResponse.SetStringVariable(Index: integer;
...
var
  ...
  begin
    if Item <> '' then
      Headers := Headers + ALFormat(FormatStr, [Item]);
  end;
...
begin
  ...
  begin
    if (ReasonString <> '') and (StatusCode > 0) then
      StatusString := ALFormat('%d %s', [StatusCode, ReasonString])
    else
      StatusString := '200 OK';
    ...
    if Expires > 0 then
      AddHeaderItem
        (ALFormat(ALFormatDateTime('"%s", dd "%s" yyyy hh:nn:ss "GMT"', Expires,
        ALDefaultFormatSettings), [CAlRfc822DayOfWeekNames[DayOfWeek(Expires)],
        CALRfc822MonthOfTheYearNames[MonthOf(Expires)]]), 'Expires: %s'#13#10);
    if LastModified > 0 then
      AddHeaderItem
        (ALFormat(ALFormatDateTime('"%s", dd "%s" yyyy hh:nn:ss "GMT"',
        LastModified, ALDefaultFormatSettings),
    ...
    if (Content <> '') or (ContentStream <> nil) then
      AddHeaderItem(ALIntToStr(ContentLength), 'Content-Length: %s'#13#10);
