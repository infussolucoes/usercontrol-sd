...
type
  ...
  protected
    ...
    Function SendCmd(aCmd: AnsiString; const MultilineResponse: Boolean = False)
      : AnsiString; virtual;
  ...
  public
    ...
    Procedure Uidl(aMsgNumber: Integer; Var aUniqueIDListing: AnsiString);
      overload; virtual;
    Function Quit: AnsiString; virtual;
    Function Rset: AnsiString; virtual;
    Function Stat: AnsiString; overload; virtual;
    Procedure Stat(Var ANumberofMsgInthemaildrop: Integer;
      Var aSizeofthemaildrop: Integer); overload; virtual;
    Function Retr(aMsgNumber: Integer): AnsiString; overload; virtual;
    procedure Retr(aMsgNumber: Integer; var aMsgBodyContent: AnsiString;
      aMsgHeaderContent: TALEMailHeader); overload; virtual;
    Function Top(aMsgNumber: Integer; aNumberOfLines: Integer)
      : AnsiString; virtual;
...
{ ******************************************************************************* }
Procedure ALPOP3ClientSplitResponseLine(aResponse: AnsiString;
  ALst: TALStrings);
Begin
...
Function ALPOP3ClientExtractTextFromMultilineResponse(aMultilineResponse
...
{ **************************************************************************************** }
Function TAlPOP3Client.Connect(const aHost: AnsiString; const APort: Integer)
  : AnsiString;
...
begin
  ALst.Text :=
    ALTrim(AlStringReplace(ALPOP3ClientExtractTextFromMultilineResponse(List),
    ' ', ALst.NameValueSeparator, [RfReplaceAll]));
end;
...
begin
  ALst.Text :=
    ALTrim(AlStringReplace(ALPOP3ClientExtractTextFromMultilineResponse(Uidl),
    ' ', ALst.NameValueSeparator, [RfReplaceAll]));
end;
...
{ ********************************************************************************** }
Procedure TAlPOP3Client.Uidl(aMsgNumber: Integer;
  Var aUniqueIDListing: AnsiString);
Var
  ALst: TALStringList;
Begin
  ...
  Try
    ...
    If ALst.Count < 3 then
      raise Exception.Create('UIDL cmd Error');
...
{ **************************************************************************************************** }
procedure TAlPOP3Client.Stat(Var ANumberofMsgInthemaildrop: Integer;
  Var aSizeofthemaildrop: Integer);
Var
  ALst: TALStringList;
Begin
  ALst := TALStringList.Create;
  Try
    ...
    If not ALTryStrToInt(ALst[1], ANumberofMsgInthemaildrop) then
      raise Exception.Create('Stat cmd Error');
    If not ALTryStrToInt(ALst[2], aSizeofthemaildrop) then
      raise Exception.Create('Stat cmd Error');
  finally
    ALst.Free;
  end;
...
{ ******************************************************************************************************************** }
procedure TAlPOP3Client.Retr(aMsgNumber: Integer;
  var aMsgBodyContent: AnsiString; aMsgHeaderContent: TALEMailHeader);
Var
  P: Integer;
begin
  aMsgBodyContent := ALPOP3ClientExtractTextFromMultilineResponse
    (Retr(aMsgNumber));
...
Function TAlPOP3Client.Top(aMsgNumber: Integer; aNumberOfLines: Integer)
  : AnsiString;
begin
  Result := SendCmd('TOP ' + ALInttostr(aMsgNumber) + ' ' +
    ALInttostr(aNumberOfLines), True);
end;
...
{ Responses in the POP3 consist of a status indicator and a keyword
  ...
  server is ended and the line containing ".CRLF" is not considered
  part of the multi-line response. }
function TAlPOP3Client.GetResponse(Const MultilineResponse: Boolean = False)
  : AnsiString;
Var
  aBuffStr: AnsiString;
...
begin
  ...
  begin
    ...
    begin
      if alPos('+OK ', Result) = 1 then
        aGoodResponse := 1
      else if alPos('-ERR ', Result) = 1 then
        aGoodResponse := 0;
    end;
...
{ **************************************************************************************************************** }
Function TAlPOP3Client.SocketWrite({$IF CompilerVersion >= 23}const {$ELSE}var
