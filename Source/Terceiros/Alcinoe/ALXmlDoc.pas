...
{$IF CompilerVersion >= 23} { Delphi XE2 }
  ...
  TAlXMLParseProcessingInstructionEvent = reference to procedure
    (Sender: TObject; const Path, Target, Data: AnsiString);
  TAlXMLParseTextEvent = reference to procedure(Sender: TObject;
    const Path, Str: AnsiString);
  TAlXMLParseStartElementEvent = reference to procedure(Sender: TObject;
    const Path, Name: AnsiString; const Attributes: TALStrings);
  TAlXMLParseEndElementEvent = reference to procedure(Sender: TObject;
    const Path, Name: AnsiString);
{$ELSE}
  TAlXMLParseDocument = procedure(Sender: TObject) of object;
  TAlXMLParseProcessingInstructionEvent = procedure(Sender: TObject;
    const Path, Target, Data: AnsiString) of object;
  TAlXMLParseTextEvent = procedure(Sender: TObject; const Path, Str: AnsiString)
    of object;
  TAlXMLParseStartElementEvent = procedure(Sender: TObject;
    const Path, Name: AnsiString; const Attributes: TALStrings) of object;
  TAlXMLParseEndElementEvent = procedure(Sender: TObject;
    const Path, Name: AnsiString) of object;
{$IFEND}
{$IF CompilerVersion >= 23} { Delphi XE2 }
  TALXMLNodeListSortCompare = reference to function(List: TALXMLNodeList;
    Index1, Index2: Integer): Integer;
{$ELSE}
  TALXMLNodeListSortCompare = function(List: TALXMLNodeList;
    Index1, Index2: Integer): Integer;
{$IFEND}
  TALXmlNodeType = (ntElement,
    // The node represents an element. Element nodes represent simple tags that have child nodes. The child nodes of an element node can have the following node types: ntElement, ntText, ntCData, ntEntityRef, ntProcessingInstr, and ntComment. Element nodes can also have attributes (ntAttribute). Element nodes can be the child of a node of type ntDocument, ntDocFragment, ntEntityRef, and ntElement.
    ntAttribute,
    // The node represents an attribute of an element. It is not the child of another node, but its value can be accessed using the Attribute property of the element node. An attribute node can have child nodes of type ntText and ntEntityRef.
    ...
    ntDocType,
    // [not implemented yet !] The node represents the document type declaration, indicated by the <!DOCTYPE > tag. The document type node can child nodes of type ntNotation and ntEntity. It always appears as the child of the document node.
    ntDocFragment,
    // [not implemented yet !] The node represents a document fragment. A document fragment node associates a node or subtree with a document without actually being contained in the document. Document fragment nodes can have child nodes of type ntElement, ntProcessingInstr, ntComment, ntText, ntCData, and ntEntityRef. It never appears as the child of another node.
    ntNotation);
  // [not implemented yet !] A node represents a notation in the document type declaration. It always appears as the child of an ntDocType node and never has any child nodes.

  TALXMLDocOption = (
    // doAttrNull,      //[deleted from TALXMLDocOption] When reading the value of an attribute that does not exist, the value is given as a Null Variant (as opposed to a value of an empty string).
  ...
  TALXMLParseOption = (
    // poResolveExternals,   //[not implemented yet !] External definitions (resolvable namespaces, DTD external subsets, and external entity references) are resolved at parse time
    ...
    poIgnoreXMLReferences);
  // [added from TParseOption] don't decode xml entities (like &amp;) and not encode them also (when save / load)
  ...
  Private
    ...
    FList: PALPointerXMLNodeList;
    // [Replace from TXMLNodeList] FList: IInterfaceList;
    FOwner: TALXMLNode;
    procedure QuickSort(L, R: Integer; XCompare: TALXMLNodeListSortCompare);
    // [added from TXMLNodeList]
  protected
...
{$IFEND}
    function GetNodeByIndex(Const Index: Integer): TALXMLNode;
    // [added from TXMLNodeList]
    function GetNodeByName(Const Name: AnsiString): TALXMLNode;
    // [added from TXMLNodeList]
  ...
  public
    ...
    constructor Create(Owner: TALXMLNode);
    // [Replace from TXMLNodeList] constructor Create(Owner: TXMLNode; const DefaultNamespaceURI: DOMString; NotificationProc: TNodeListNotification);
    destructor Destroy; override;
    procedure CustomSort(Compare: TALXMLNodeListSortCompare);
    // [added from TXMLNodeList]
    ...
    function Extract(const Index: Integer): TALXMLNode; overload;
    // [added from TXMLNodeList]
    function Extract(const Node: TALXMLNode): TALXMLNode; overload;
    // [added from TXMLNodeList]
    ...
    function FindNode(NodeName: AnsiString; NodeAttributes: Array of AnsiString)
      : TALXMLNode; overload; // [added from TXMLNodeList]
...
{$IF CompilerVersion < 18.5}
    property Nodes[const IndexOrName: OleVariant]: TALXMLNode
      read GetNode; default;
{$ELSE}
    property Nodes[const Name: AnsiString]: TALXMLNode
      read GetNodeByName; default;
    property Nodes[const Index: Integer]: TALXMLNode
...
{$IFEND}
  ...
  protected
    ...
    procedure DoBeforeChildNodesInsert(Index: Integer; const Node: TALXMLNode);
      virtual; // [added from TXMLNode]
    function CreateAttributeList: TALXMLNodeList;
    function CreateChildList: TALXMLNodeList;
    function InternalGetAttributeNodes: TALXMLNodeList; virtual;
    // [added from TXMLNode]
    function InternalGetChildNodes: TALXMLNodeList; virtual;
    // [added from TXMLNode]
    function GetInternalChildValue: AnsiString; virtual;
    // [added from TXMLNode]
    ...
    procedure SetInternalChildValue(const Value: AnsiString); virtual;
    // [added from TXMLNode]
    procedure SetInternalValue(const Value: AnsiString); virtual;
    // [added from TXMLNode]
    procedure SetNodeName(const Value: AnsiString); // [added from TXMLNode]
    procedure SetOwnerDocument(const Value: TALXMLDocument);
    // [added from TXMLNode]
    ...
    property InternalChildValue: AnsiString read GetInternalChildValue
      write SetInternalChildValue; // [added from TXMLNode]
    property InternalValue: AnsiString read GetInternalValue
      write SetInternalValue; // [added from TXMLNode]
  public
    ...
    constructor Create(const NameOrData: AnsiString;
      const AddlData: AnsiString = ''); virtual;
    // [Replace from TXMLNode] constructor Create(const ADOMNode: IDOMNode; const AParentNode: TALXMLNode; const OwnerDoc: TXMLDocument);
    function CloneNode(Deep: Boolean): TALXMLNode;
    function AddChild(const TagName: AnsiString; Index: Integer = -1)
      : TALXMLNode; overload;
    ...
    property Attributes[const AttrName: AnsiString]: AnsiString
    ...
    procedure SaveToStream(const Stream: TStream;
      Const SaveOnlyChildNodes: Boolean = False); // [added from TXMLNode]
    ...
    procedure LoadFromFile(const AFileName: AnsiString;
      Const FileContainOnlyChildNodes: Boolean = False;
      Const ClearChildNodes: Boolean = True); // [added from TXMLNode]
    procedure LoadFromStream(const Stream: TStream;
      Const StreamContainOnlyChildNodes: Boolean = False;
      Const ClearChildNodes: Boolean = True); // [added from TXMLNode]
    procedure LoadFromXML(const XML: AnsiString;
      Const XmlContainOnlyChildNodes: Boolean = False;
      Const ClearChildNodes: Boolean = True); // [added from TXMLNode]
  end;
  ...
  private
    FChildNodes: TALXMLNodeList;
  protected
    procedure DoBeforeChildNodesInsert(Index: Integer;
      const Node: TALXMLNode); override;
    function GetNodeType: TALXmlNodeType; override;
    function GetChildNodes: TALXMLNodeList; override;
    procedure SetChildNodes(const Value: TALXMLNodeList); override;
    function InternalGetChildNodes: TALXMLNodeList; override;
  ...
  public
    constructor Create(const NameOrData: AnsiString;
      const AddlData: AnsiString = ''); override;
    Destructor Destroy; override;
  end;
  ...
  private
    ...
    FOnParseProcessingInstruction: TAlXMLParseProcessingInstructionEvent;
    // [added from TXMLDocument]
    ...
    FOnParseStartElement: TAlXMLParseStartElementEvent;
    // [added from TXMLDocument]
  ...
  protected
    ...
    function GetPrologValue(PrologItem: TALXMLPrologItem;
      const Default: AnsiString = ''): AnsiString;
    function InternalSetPrologValue(const PrologNode: TALXMLNode;
      const Value: AnsiString; PrologItem: TALXMLPrologItem): AnsiString;
    // [Replace from TXMLNodeList] function InternalSetPrologValue(const PrologNode: IXMLNode; const Value: Variant; PrologItem: TXMLPrologItem): string;
    procedure CheckActive;
    procedure DoParseProcessingInstruction(const Path, Target,
      Data: AnsiString); // [added from TXMLDocument]
    procedure DoParseStartDocument; // [added from TXMLDocument]
    procedure DoParseEndDocument; // [added from TXMLDocument]
    procedure DoParseStartElement(const Path, Name: AnsiString;
      const Attributes: TALStrings); // [added from TXMLDocument]
    procedure DoParseEndElement(const Path, Name: AnsiString);
    // [added from TXMLDocument]
    procedure DoParseText(const Path, Str: AnsiString);
    // [added from TXMLDocument]
    procedure DoParseComment(const Path, Str: AnsiString);
    // [added from TXMLDocument]
    procedure DoParseCData(const Path, Str: AnsiString);
    // [added from TXMLDocument]
    Procedure ParseXmlStream(RawXmlStream: TStream; ContainerNode: TALXMLNode;
      StreamContainOnlyChildNodes: Boolean = False);
    // [added from TXMLDocument]
    procedure ReleaseDoc;
    procedure SetPrologValue(const Value: AnsiString;
      PrologItem: TALXMLPrologItem);
    // [Replace from TXMLNodeList] procedure SetPrologValue(const Value: Variant; PrologItem: TXMLPrologItem);
    ...
    function GetXML: AnsiString;
    // [Replace from TXMLDocument] function GetXML: TALStrings;
    ...
    // [Replace from TXMLDocument] procedure SetXML(const Value: TALStrings);
  ...
  public
    ...
    // [Replace from TXMLDocument]  constructor Create
    constructor Create(const Rootname: AnsiString;
      const EncodingStr: AnsiString = cAlXMLUTF8EncodingStr); overload; virtual;
    // [added from TXMLDocument]
    destructor Destroy; override;
    procedure Clear(const Rootname: AnsiString;
      const EncodingStr: AnsiString = cAlXMLUTF8EncodingStr);
    // [added from TXMLDocument]
    function AddChild(const TagName: AnsiString): TALXMLNode;
    function CreateElement(const TagOrData: AnsiString): TALXMLNode;
    // [Replace from TXMLDocument] function CreateElement(const TagOrData, NamespaceURI: AnsiString): TALXMLNode;
    function CreateNode(const NameOrData: AnsiString;
      NodeType: TALXmlNodeType = ntElement; const AddlData: AnsiString = '')
      : TALXMLNode;
    function IsEmptyDoc: Boolean;
    procedure LoadFromFile(const AFileName: AnsiString;
      const saxMode: Boolean = False);
    // [Replace from TXMLDocument]  procedure LoadFromFile(const AFileName: AnsiString = '');
    procedure LoadFromStream(const Stream: TStream;
      const saxMode: Boolean = False);
    // [Replace from TXMLDocument] procedure LoadFromStream(const Stream: TStream; EncodingType: TALXMLEncodingType = xetUnknown);
    procedure LoadFromXML(const XML: AnsiString;
      const saxMode: Boolean = False);
    // [Replace from TXMLDocument]  procedure LoadFromXML(const XML: AnsiString); overload;
    procedure SaveToFile(const AFileName: AnsiString);
    procedure SaveToStream(const Stream: TStream);
    procedure SaveToXML(var XML: AnsiString);
    property ChildNodes: TALXMLNodeList read GetChildNodes;
    property DocumentElement: TALXMLNode read GetDocumentElement
    ...
    property ParseOptions: TALXMLParseOptions read GetParseOptions
      write SetParseOptions;
    property PathSeparator: AnsiString read GetPathSeparator
      write SetPathSeparator;
    property XML: AnsiString read GetXML write SetXML;
    property OnParseProcessingInstruction: TAlXMLParseProcessingInstructionEvent
      read FOnParseProcessingInstruction write FOnParseProcessingInstruction;
    // [added from TXMLDocument]
    property OnParseStartDocument: TAlXMLParseDocument
      read FOnParseStartDocument write FOnParseStartDocument;
    // [added from TXMLDocument]
    property OnParseEndDocument: TAlXMLParseDocument read FOnParseEndDocument
      write FOnParseEndDocument; // [added from TXMLDocument]
    property OnParseStartElement: TAlXMLParseStartElementEvent
      read FOnParseStartElement write FOnParseStartElement;
    // [added from TXMLDocument]
    property OnParseEndElement: TAlXMLParseEndElementEvent
      read FOnParseEndElement Write FOnParseEndElement;
    // [added from TXMLDocument]
    property OnParseText: TAlXMLParseTextEvent read FonParseText
      Write FonParseText; // [added from TXMLDocument]
    property OnParseComment: TAlXMLParseTextEvent read FonParseComment
      Write FonParseComment; // [added from TXMLDocument]
    property OnParseCData: TAlXMLParseTextEvent read FonParseCData
      Write FonParseCData; // [added from TXMLDocument]
...
Function ALFindXmlNodeByNameAndChildNodeValue(xmlrec: TALXMLNode;
  Const NodeName: AnsiString; Const ChildNodeName, ChildNodeValue: AnsiString;
  Const Recurse: Boolean = False): TALXMLNode;
...
Function ALFindXmlNodeByNameAndAttribute(xmlrec: TALXMLNode;
  Const NodeName: AnsiString; Const AttributeName, AttributeValue: AnsiString;
  Const Recurse: Boolean = False): TALXMLNode;
function ALExtractAttrValue(const AttrName, AttrLine: AnsiString;
  const Default: AnsiString = ''): AnsiString;
...
procedure ALXMLDocError(const Msg: String; const Args: array of const);
  overload;
begin
...
function ALNodeMatches(const Node: TALXMLNode;
  const TagName: AnsiString): Boolean;
begin
...
begin
  case NodeType of
    ntElement:
      Result := TALXmlElementNode.Create(NameOrData);
    ntAttribute:
      Result := TALXmlAttributeNode.Create(NameOrData);
    ntText:
      Result := TALXmlTextNode.Create(NameOrData);
    ntCData:
      Result := TALXmlCDataNode.Create(NameOrData);
    ntEntityRef:
      Result := nil; // todo
    ntProcessingInstr:
      Result := TALXmlProcessingInstrNode.Create(NameOrData, AddlData);
    ntComment:
      Result := TALXmlCommentNode.Create(NameOrData);
    ntDocFragment:
      Result := nil; // todo
  else
    begin
...
{ ************************************************************************************* }
procedure ALAppendItem(var AttrStr: AnsiString;
  const AttrName, AttrValue: AnsiString);
begin
  if AttrValue <> '' then
  begin
    if AttrStr <> '' then
...
{ ****************************************************************************************************************** }
constructor TALXMLDocument.Create(const Rootname: AnsiString;
  const EncodingStr: AnsiString = cAlXMLUTF8EncodingStr);
begin
  Create(True);
  Version := '1.0';
...
{ *************************************************************************************************************** }
procedure TALXMLDocument.Clear(const Rootname: AnsiString;
  const EncodingStr: AnsiString = cAlXMLUTF8EncodingStr);
begin
  ReleaseDoc;
  Active := True;
...
Procedure TALXMLDocument.ParseXmlStream(RawXmlStream: TStream;
  ContainerNode: TALXMLNode; StreamContainOnlyChildNodes: Boolean = False);

Const
  BufferSize: Integer = 8192;
Var
  RawXmlString: AnsiString;
  ...
  Begin
    If (RawXmlStringLength > 0) and (RawXmlStringPos > 1) then
    begin
      if (RawXmlStringPos > RawXmlStringLength) then
        RawXmlStream.Position := RawXmlStream.Position - RawXmlStringLength +
          RawXmlStringPos - 1;
      Byte2Read := min(RawXmlStringPos - 1, RawXmlStringLength);
      if RawXmlStringPos <= Length(RawXmlString) then
        ALMove(RawXmlString[RawXmlStringPos], RawXmlString[1],
    ...
    if RawXmlStream.Position < RawXmlStream.Size then
      ByteReaded := RawXmlStream.
        Read(RawXmlString[RawXmlStringLength - Byte2Read + 1], Byte2Read)
    else
      ByteReaded := 0;
...
{ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ }
  Function PosInXmlString(const Substr: AnsiString;
  ...
  Begin
    ...
    begin
      ...
      If not ExpandRawXmlString then
        break;
      Result := ALPosEx(Substr, RawXmlString, Offset);
    end;
  end;
...
{ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ }
  Function CharPosInString(const Substr: array of AnsiChar;
    const Str: AnsiString; Offset: Integer = 1): Integer;
  Var
    i: Integer;
...
{ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ }
  ...
  Begin
    ...
    begin
      ...
      P1 := ALPos('=', S1);
      IF (P1 <= 0) and (S2 = '=') then
      begin { aname = "obie2.html" }
        TagParams[i] := S1 + S2 + S3;
        TagParams.Delete(i + 2);
        TagParams.Delete(i + 1);
      end
      else if (L1 > 0) and (P1 = L1) then
      begin { aname= "obie2.html" }
        TagParams[i] := S1 + S2;
        TagParams.Delete(i + 1);
      end
      else if (L1 > 0) and (P1 <= 0) and (ALPos('=', S2) = 1) then
      begin { aname ="obie2.html" }
        TagParams[i] := S1 + S2;
        TagParams.Delete(i + 1);
      end;
...
{ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ }
  ...
  begin
    ...
    begin
      ...
      L := Length(LB);
      if (L <> 0) and ((i <> Count - 1) or (ExtraItems <> '')) then
      begin
...
{ ~~~~~~~~~~~~~~~~~~ }
  ...
  Begin
    ...
    begin
      ...
      P1 := CharPosInString([' ', #9, #13, #10], RawXmlString,
        RawXmlStringPos + 2);
      If (P1 <= 0) or (P1 > P2) then
        P1 := P2;
      AName := ALCopyStr(RawXmlString, RawXmlStringPos + 2,
        P1 - RawXmlStringPos - 2);
      ...
      begin
        ...
        If UseContainerNodeInsteadOfAddingChildNode then
          ALXMLDocError(cALXmlParseError);
        // because if not we need to delete and recreate the container node and this can cause trouble in the calling function
        WorkingNode.ChildNodes.Add(CreateNode(AName, ntProcessingInstr,
          aContent));
      end;
      ...
      if ((not NotSaxMode) or (ContainerNode.NodeType = ntDocument)) and
        (RawXmlStringPos in [1, 4 { UTF8 BOOM } ]) then
        CodePage := ALGetCodePageFromCharSetName(ALExtractAttrValue(CALEncoding,
          aContent, ''));
...
{ ~~~~~~~~~~~~~~~~~~~~~~~ }
  ...
  Begin
    ...
    If P1 > 0 then
    begin
      aContent := ALCopyStr(RawXmlString, RawXmlStringPos + 4,
        P1 - RawXmlStringPos - 4);
      if NotSaxMode then
      begin
        if not Assigned(WorkingNode) then
          ALXMLDocError(cALXmlParseError);
...
{ ~~~~~~~~~~~~~~~~~~~~~ }
  ...
  Begin
    ...
    If P1 > 0 then
    begin
      aContent := ALCopyStr(RawXmlString, RawXmlStringPos + 9,
        P1 - RawXmlStringPos - 9);
      if NotSaxMode then
      begin
        if not Assigned(WorkingNode) then
          ALXMLDocError(cALXmlParseError);
...
{ ~~~~~~~~~~~~~~~~~~~ }
  ...
  Begin
    ...
    begin
      ...
      // end tag
      If RawXmlString[RawXmlStringPos + 1] = '/' then
      begin
        AName := ALtrimRight(ALCopyStr(RawXmlString, RawXmlStringPos + 2,
          P1 - RawXmlStringPos - 2));
        if (Paths.Count = 0) or (Paths[Paths.Count - 1] <> AName) then
          ALXMLDocError(cALXmlParseError);
        if NotSaxMode then
        begin
      ...
      P2 := CharPosInString([' ', #9, #13, #10], RawXmlString,
        RawXmlStringPos + 1);
      If (P2 <= 0) or (P2 > P1) then
        P2 := P1;

      AName := ALCopyStr(RawXmlString, RawXmlStringPos + 1,
        P2 - RawXmlStringPos - 1);
      if (AName = '') or (AName[1] = #0) then
        ALXMLDocError(cALXmlParseError);
      // mean it's an UTF-16 encoding not yet supported
      ...
      begin
        ...
        begin
          ...
          if (aContentLn < 2) or (aContent[1] <> aContent[aContentLn]) or
            (not(aContent[1] in ['''', '"'])) then
            ALXMLDocError(cALXmlParseError)
          else
          begin
            if DecodeXmlReferences then
            begin
              if CodePage = CP_UTF8 then
                LstParams[i] := LstParams.Names[i] + '=' +
                  ALUTF8XMLTextElementDecode(ALCopyStr(aContent, 2,
                  aContentLn - 2))
              else
                LstParams[i] := LstParams.Names[i] + '=' +
                  ALUTF8decode
                  (ALUTF8XMLTextElementDecode(ALUTF8Encode(ALCopyStr(aContent,
                  2, aContentLn - 2), CodePage)), CodePage);
            end
            else
              LstParams[i] := LstParams.Names[i] + '=' + ALCopyStr(aContent, 2,
                aContentLn - 2)
          end;
        end;
...
{ ~~~~~~~~~~~~~~~~~~~~ }
  ...
  Begin
    ...
    If (P1 <= 0) then
      P1 := RawXmlStringLength + 1;

    Str1 := ALCopyStr(RawXmlString, RawXmlStringPos, P1 - RawXmlStringPos);
    If (PreserveWhiteSpace) or (ALTrim(Str1) <> '') then
    Begin

      if DecodeXmlReferences then
      begin
        if CodePage = CP_UTF8 then
          Str1 := ALUTF8XMLTextElementDecode(Str1)
        else
          Str1 := ALUTF8decode(ALUTF8XMLTextElementDecode(ALUTF8Encode(Str1,
            CodePage)), CodePage);
      end;
      ...
      begin
        if not Assigned(WorkingNode) then
          ALXMLDocError(cALXmlParseError);
        if (WorkingNode.NodeType <> ntDocument) then
        begin // ntdocument can not have any text node, so simply skip whiteSpace comment (else error)
          If UseContainerNodeInsteadOfAddingChildNode then
            ALXMLDocError(cALXmlParseError);
          // because if not we need to delete and recreate the container node and this can cause trouble in the calling function
          WorkingNode.ChildNodes.Add(CreateNode(Str1, ntText, ''));
        end
        else if (ALTrim(Str1) <> '') then
          ALXMLDocError(cALXmlParseError);
      end;
...
Begin
  ...
  Try
    ...
    PreserveWhiteSpace := (poPreserveWhiteSpace in ParseOptions) and
      (not(doNodeAutoIndent in Options));
    WorkingNode := ContainerNode;
    NotSaxMode := Assigned(ContainerNode);
    UseContainerNodeInsteadOfAddingChildNode := NotSaxMode and
      (ContainerNode.NodeType <> ntDocument) and
      (not StreamContainOnlyChildNodes);
    DecodeXmlReferences := not(poIgnoreXMLReferences in ParseOptions);
    if NotSaxMode then
      CodePage := ALGetCodePageFromCharSetName(Encoding)
    else
      CodePage := 0;
    RawXmlString := '';
    ...
    RawXmlStringPos := 1;
    ExpandRawXmlString;
    if AlUTF8DetectBOM(PAnsiChar(RawXmlString), Length(RawXmlString)) then
      RawXmlStringPos := 4;

    While RawXmlStringPos <= RawXmlStringLength do
    begin

      If RawXmlString[RawXmlStringPos] = '<' then
      begin
        If (RawXmlStringPos < RawXmlStringLength) and
          (RawXmlString[RawXmlStringPos + 1] = '?') then
          AnalyzePI
        else if (RawXmlStringPos + 2 < RawXmlStringLength) and
          (RawXmlString[RawXmlStringPos + 1] = '!') and
          (RawXmlString[RawXmlStringPos + 2] = '-') and
          (RawXmlString[RawXmlStringPos + 3] = '-') then
          AnalyzeComment
          ...
          (RawXmlString[RawXmlStringPos + 8] = '[') then
          AnalyzeCDATA
        else if (RawXmlStringPos + 7 < RawXmlStringLength) and
          (RawXmlString[RawXmlStringPos + 1] = '!') and
          ...
          (RawXmlString[RawXmlStringPos + 8] = 'E') then
          AnalyzeDOCTYPE
        else
          AnalyzeTag
      end
      ...
      if RawXmlStringPos + 7 >= RawXmlStringLength then
        ExpandRawXmlString;
...
{ Creates a new node that is associated with this document.
  ...
  *The meaning of AddlData depends on the node type, as indicated in the following table:
  ...
  CreateNode returns the interface for the new node.
  Note:	To add a node as the child of another node in this document, it is simpler to call that node’s AddChild method. }
function TALXMLDocument.CreateNode(const NameOrData: AnsiString;
  NodeType: TALXmlNodeType = ntElement; const AddlData: AnsiString = '')
  : TALXMLNode;
begin
...
var
  OldDocElement: TALXMLNode;
begin
  CheckActive;
  OldDocElement := GetDocumentElement;
  if Assigned(OldDocElement) then
    Node.ChildNodes.ReplaceNode(OldDocElement, Value).Free
  else
...
begin
  ...
  if (Node.ChildNodes.Count > 0) and
    ...
    (Node.ChildNodes[0].NodeName = CALXML) then
...
{ *************************************************************************************************************** }
function TALXMLDocument.GetPrologValue(PrologItem: TALXMLPrologItem;
  const Default: AnsiString = ''): AnsiString;
var
  PrologNode: TALXMLNode;
...
begin
  ...
  begin
    ...
    case PrologItem of
      xpVersion:
        Result := ALExtractAttrValue(CALVersion, PrologAttrs, Default);
      xpEncoding:
        Result := ALExtractAttrValue(CALEncoding, PrologAttrs, Default);
      xpStandalone:
        Result := ALExtractAttrValue(CALStandalone, PrologAttrs, Default);
    end
  end
  else
    Result := Default;
...
{ ********************************************************************************************************************************************** }
function TALXMLDocument.InternalSetPrologValue(const PrologNode: TALXMLNode;
  const Value: AnsiString; PrologItem: TALXMLPrologItem): AnsiString;
var
  Version, Encoding, StandAlone: AnsiString;
begin
  if Assigned(PrologNode) then
  begin
...
{ ********************************************************************************************* }
procedure TALXMLDocument.SetPrologValue(const Value: AnsiString;
  PrologItem: TALXMLPrologItem);
var
  PrologAttrs: AnsiString;
...
begin
  ...
  if Assigned(PrologNode) then
    Node.ChildNodes.ReplaceNode(PrologNode, NewPrologNode).Free
  else
...
begin
  if Assigned(FonParseComment) then
    FonParseComment(self, Path, Str);
end;

{ ***************************************************************** }
procedure TALXMLDocument.DoParseCData(const Path, Str: AnsiString);
begin
  if Assigned(FonParseCData) then
    FonParseCData(self, Path, Str);
end;
...
begin
  if Assigned(FOnParseEndElement) then
    FOnParseEndElement(self, Path, Name);
end;

{ ****************************************************************************************** }
procedure TALXMLDocument.DoParseProcessingInstruction(const Path, Target,
  Data: AnsiString);
begin
  if Assigned(FOnParseProcessingInstruction) then
    FOnParseProcessingInstruction(self, Path, Target, Data);
end;
...
{ ******************************************************************************************************* }
procedure TALXMLDocument.DoParseStartElement(const Path, Name: AnsiString;
  const Attributes: TALStrings);
begin
  if Assigned(FOnParseStartElement) then
    FOnParseStartElement(self, Path, Name, Attributes);
end;
...
{ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ }
  ...
  begin
    case SourceNode.NodeType of
      ntElement:
        begin
          Result := ALCreateXmlNode(SourceNode.NodeName,
            SourceNode.NodeType, '');
          if TargetParentNode <> nil then
            TargetParentNode.ChildNodes.Add(Result);
      ...
      ntAttribute:
        begin
          ...
          if TargetParentNode <> nil then
            TargetParentNode.AttributeNodes.Add(Result);
        end;
...
{ ************************************************************************************ }
procedure TALXMLNode.DoBeforeChildNodesInsert(Index: Integer;
  const Node: TALXMLNode);
begin
...
begin
  ...
  if Assigned(aNodeList) then
    Result := aNodeList.IndexOf(Name) <> -1
  else
    Result := False;
end;
...
procedure TALXMLNode.SetAttribute(const AttrName: AnsiString;
...
function TALXMLNode.AddChild(const TagName: AnsiString; Index: Integer = -1)
  : TALXMLNode;
begin
...
begin
  If not IsTextElement then
    ALXMLDocError(cALXmlNotSingleTextNode, [NodeName]);
end;
...
begin
  case NodeType of
    ntElement:
      Begin
        CheckTextNode;
        If (InternalGetChildNodes <> nil) then
...
begin
  case NodeType of
    ntElement:
      Begin
        ...
        if ChildNodes.Count = 1 then
          ChildNodes[0].Text := Value
        else
          ChildNodes.Add(ALCreateXmlNode(Value, ntText, ''))
      end;
    ntAttribute:
      Begin
        if ChildNodes.Count = 1 then
    ...
    // ntEntity:	         ;todo
    ntProcessingInstr:
      InternalChildValue := Value;
    ntComment:
      InternalValue := Value;
    ntDocument:
      ALXMLDocError(cALXmlOperationError, [CALXmlDocument]);
    // ntDocType:         ;todo
    // ntDocFragment:	   ;todo
    // ntNotation:	       ;todo
  else
...
procedure TALXMLNode.SaveToStream(const Stream: TStream;
  Const SaveOnlyChildNodes: Boolean = False);
...
{ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ }
  ...
  Begin
    with aAttributeNode do
      if EncodeXmlReferences then
        WriteStr2Buffer(' ' + NodeName + '="' + ALXMLTextElementEncode
          (Text) + '"')
      else
      begin
        if ALPos('"', Text) > 0 then
          WriteStr2Buffer(' ' + NodeName + '=''' + Text + '''')
        else
          WriteStr2Buffer(' ' + NodeName + '="' + Text + '"');
      end;
  end;
...
begin
  If not(NodeType in [ntElement, ntDocument]) then
    exit;
  ...
  Try
    ...
    { SaveOnlyChildNodes }
    If SaveOnlyChildNodes or (NodeType = ntDocument) then
      WriteDocumentNode2Stream(self)
    else
      NodeStack.Push(self);
    ...
    begin
          ...
          ntElement:
            Begin
              if CurrentNode = CurrentParentNode then
                WriteEndElementNode2Stream(CurrentNode)
              else
                WriteStartElementNode2Stream(CurrentNode);
            end;
          ntText:
            WriteTextNode2Stream(CurrentNode);
          ntCData:
            WriteCDATANode2Stream(CurrentNode);
          // ntEntityRef:       ;Todo
          // ntEntity:	         ;todo
          ntProcessingInstr:
            WriteProcessingInstrNode2Stream(CurrentNode);
          ntComment:
            WriteCommentNode2Stream(CurrentNode);
          // ntDocType:         ;todo
          // ntDocFragment:	   ;todo
          // ntNotation:	       ;todo
        else
      ...
      If Assigned(CurrentParentNode) and
        ...
        (CurrentNode.NodeType <> ntElement) then
...
{ StreamContainOnlyChildNodes mean that the stream contain ONLY the child node, so it's not a valid xml stream
  like <root>...</root> but more like <rec>...</rec><rec>...</rec><rec>...</rec> }
procedure TALXMLNode.LoadFromStream(const Stream: TStream;
  Const StreamContainOnlyChildNodes: Boolean = False;
  Const ClearChildNodes: Boolean = True);
Begin
  If not(NodeType in [ntElement, ntDocument]) then
    ALXMLDocError(cALXmlOperationError, [GetNodeTypeStr]);
  if (not StreamContainOnlyChildNodes) or ClearChildNodes then
  begin
    ChildNodes.Clear;
...
{ **************************************************************************************************** }
procedure TALXMLNode.SaveToFile(const AFileName: AnsiString;
  Const SaveOnlyChildNodes: Boolean = False);
Var
  afileStream: TFileStream;
begin
...
{ load FileContainOnlyChildNodes mean the the stream contain ONLY the child node, so it's not a valid xml stream
  like <root>...</root> but more like <rec>...</rec><rec>...</rec><rec>...</rec> }
procedure TALXMLNode.LoadFromFile(const AFileName: AnsiString;
  Const FileContainOnlyChildNodes: Boolean = False;
  Const ClearChildNodes: Boolean = True);
Var
  afileStream: TFileStream;
...
{ ******************************************************************************************* }
procedure TALXMLNode.SaveToXML(var XML: AnsiString;
  Const SaveOnlyChildNodes: Boolean = False);
Var
...
{ load XmlContainOnlyChildNodes mean the the stream contain ONLY the child node, so it's not a valid xml stream
  like <root>...</root> but more like <rec>...</rec><rec>...</rec><rec>...</rec> }
procedure TALXMLNode.LoadFromXML(const XML: AnsiString;
  Const XmlContainOnlyChildNodes: Boolean = False;
  Const ClearChildNodes: Boolean = True);
Var
  aStringStream: TALStringStream;
...
Begin
  LoadFromXML(Value, False { XmlContainOnlyChildNodes } ,
    True { ClearChildNodes } );
end;
...
begin
  If NodeType in [ntElement, ntAttribute] then
    Result := ALExtractLocalName(NodeName)
  else
    Result := '';
end;
...
begin
  ...
  for i := 1 to Length(Value) do
    if Value[i] in [' ', #9, #13, #10] then
      ALXMLDocError(CALXMLParameterIsIncorrect);
  If Value = '' then
    ALXMLDocError(CALXMLParameterIsIncorrect);
  ...
  case NodeType of
    ...
    ntAttribute:
      InternalValue := Value;
    ntText:
      ALXMLDocError(cALXmlOperationError, ['TEXT']);
    ntCData:
      ALXMLDocError(cALXmlOperationError, ['CDATA']);
    ntEntityRef:
      ; // todo
    ntEntity:
      ; // todo
    ntProcessingInstr:
      InternalValue := Value;
    ntComment:
      ALXMLDocError(cALXmlOperationError, ['COMMENT']);
    ntDocument:
      ALXMLDocError(cALXmlOperationError, ['DOCUMENT']);
    ntDocType:
      ; // todo
    ntDocFragment:
      ALXMLDocError(cALXmlOperationError, ['DOCFRAGMENT']);
    ntNotation:
      ; // todo
  else
    ALXMLDocError(cALXmlInvalidNodeType);
...
begin
  If NodeType in [ntElement, ntAttribute] then
    Result := ALExtractPrefix(NodeName)
  else
    Result := '';
end;
...
begin
  ...
  Result := (NodeType = ntElement) and
    ((not Assigned(aNodeList)) or ((aNodeList.Count = 1) and
...
begin
  ...
  for i := 1 to Length(NameOrData) do
    if NameOrData[i] in [' ', #9, #13, #10] then
      ALXMLDocError(CALXMLParameterIsIncorrect);
  If NameOrData = '' then
    ALXMLDocError(CALXMLParameterIsIncorrect);
...
{ ******************************************************************************************* }
procedure TALXmlElementNode.DoBeforeChildNodesInsert(Index: Integer;
  const Node: TALXMLNode);
begin
  If Not(Node.NodeType in [ntElement, ntText, ntAttribute, ntCData, ntEntityRef,
    ntProcessingInstr, ntComment]) then
    ALXMLDocError(CALXMLParameterIsIncorrect);
end;
...
{ ********************************************************************************************* }
procedure TALXmlAttributeNode.DoBeforeChildNodesInsert(Index: Integer;
  const Node: TALXMLNode);
begin
  If Not(Node.NodeType in [ntText, ntEntityRef]) then
    ALXMLDocError(CALXMLParameterIsIncorrect);
  if FChildNodes.Count > 0 then
    ALXMLDocError(CALXMLOnlyOneChildError);
end;
...
{ ********************************************************************************* }
procedure TALXmlProcessingInstrNode.SetInternalChildValue
  (const Value: AnsiString);
begin
...
{ ******************************************************************************************** }
procedure TALXmlDocumentNode.DoBeforeChildNodesInsert(Index: Integer;
  const Node: TALXMLNode);
var
  aNodeList: TALXMLNodeList;
  i: Integer;
begin
  ...
  If Assigned(aNodeList) then
    For i := 0 to aNodeList.Count - 1 do
      If aNodeList[i].NodeType = ntElement then
        ALXMLDocError(CALXMLOnlyOneTopLevelError);

  If Not(Node.NodeType in [ntElement, ntProcessingInstr, ntComment, ntDocType])
  then
    ALXMLDocError(CALXMLParameterIsIncorrect);
...
{ Returns a specified node from the list.
  ...
  *NodeAttributes is the attributes the node must have. ex: ['attribute1=value1', 'attribute2=value2']
  ...
  FindNode returns nil (Delphi) or NULL (C++). }
function TALXMLNodeList.FindNode(NodeName: AnsiString;
  NodeAttributes: Array of AnsiString): TALXMLNode; // [added from TXMLNodeList]
...
{ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ }
  ...
  begin
    Result := True;
    for i := low(NodeAttributes) to high(NodeAttributes) do
    begin
      P := ALPos('=', NodeAttributes[i]);
      if (P <= 0) or (Node.Attributes[ALCopyStr(NodeAttributes[i], 1, P - 1)] <>
        ALCopyStr(NodeAttributes[i], P + 1, Maxint)) then
      begin
        Result := False;
        exit;
      end;
...
begin
  ...
  begin
    Result := Get(Index);
    if ALNodeMatches(Result, NodeName) and _AttributesMatches(Result) then
      exit;
  end;
  Result := nil;
end;
...
function TALXMLNodeList.FindSibling(const Node: TALXMLNode; Delta: Integer)
  : TALXMLNode;
var
  Index: Integer;
begin
...
begin
  if (Index < 0) or (Index >= FCount) then
    ALXMLDocError(cALXmlListIndexError, [Index]);
  Result := FList^[Index];
end;
...
begin
  ...
  if (not Assigned(Result)) and (Assigned(FOwner.OwnerDocument)) and
    ...
    ((FOwner.GetNodeType <> ntDocument) or
    (FOwner.OwnerDocument.DocumentElement = nil))
  // Don't try to autocreate a second document element !
  then
    Result := FOwner.AddChild(Name);
  if not Assigned(Result) then
    ALXMLDocError(cAlXmlNodeNotFound, [Name]);
end;

{ ************************************************************************************ }
procedure TALXMLNodeList.QuickSort(L, R: Integer;
  XCompare: TALXMLNodeListSortCompare);
var
  i, J, P: Integer;
begin
  repeat
...
{ Provides the underlying implementation of the Insert method
  ...
  Index is not –1. }
Function TALXMLNodeList.InternalInsert(Index: Integer;
  const Node: TALXMLNode): Integer;
begin
...
begin
  ...
  if Assigned(Owner.ParentNode) and Assigned(Owner.OwnerDocument) and
  ...
  begin
    ...
    if Index = -1 then
      TrailIndent := Owner.ParentNode.NestingLevel
    else
...
begin
  if (Index1 < 0) or (Index1 >= FCount) then
    ALXMLDocError(cALXmlListIndexError, [Index1]);
  if (Index2 < 0) or (Index2 >= FCount) then
    ALXMLDocError(cALXmlListIndexError, [Index2]);
  Item := FList^[Index1];
...
function TALXMLNodeList.ReplaceNode(const OldNode, NewNode: TALXMLNode)
  : TALXMLNode;
var
  Index: Integer;
begin
...
begin
  if (NewCapacity < FCount) or (NewCapacity > cALXMLNodeMaxListSize) then
    ALXMLDocError(cALXmlListCapacityError, [NewCapacity]);
...
var
  i: Integer;
begin
  if (NewCount < 0) or (NewCount > cALXMLNodeMaxListSize) then
    ALXMLDocError(cALXmlListCountError, [NewCount]);
...
Begin
  Result := nil;
  if not(xmlrec is TALXmlElementNode) then
    exit;
  for i := 0 to xmlrec.ChildNodes.Count - 1 do
  begin
    for J := 0 to xmlrec.ChildNodes[i].ChildNodes.Count - 1 do
    begin
      If ALSametext(xmlrec.ChildNodes[i].ChildNodes[J].NodeName, ChildNodeName)
        and ALSametext(xmlrec.ChildNodes[i].ChildNodes[J].Text, ChildNodeValue)
      then
      begin
    ...
    begin
      Result := ALFindXmlNodeByChildNodeValue(xmlrec.ChildNodes[i],
        ChildNodeName, ChildNodeValue, Recurse);
...
Begin
  ...
  for i := 0 to xmlrec.ChildNodes.Count - 1 do
  begin
    if ALSametext(xmlrec.ChildNodes[i].NodeName, NodeName) then
    begin
      for J := 0 to xmlrec.ChildNodes[i].ChildNodes.Count - 1 do
      begin
        If ALSametext(xmlrec.ChildNodes[i].ChildNodes[J].NodeName,
          ChildNodeName) and ALSametext(xmlrec.ChildNodes[i].ChildNodes[J].Text,
          ChildNodeValue) then
        begin
          Result := xmlrec.ChildNodes[i];
          exit;
    ...
    begin
      Result := ALFindXmlNodeByNameAndChildNodeValue(xmlrec.ChildNodes[i],
        NodeName, ChildNodeName, ChildNodeValue, Recurse);
      if Assigned(Result) then
        break;
    end;
  end;
...
Begin
  ...
  for i := 0 to xmlrec.ChildNodes.Count - 1 do
  begin
    If ALSametext(xmlrec.ChildNodes[i].Attributes[AttributeName], AttributeValue)
    then
    begin
      Result := xmlrec.ChildNodes[i];
      break;
    ...
    if Recurse then
    begin
      Result := ALFindXmlNodeByAttribute(xmlrec.ChildNodes[i], AttributeName,
...
Begin
  ...
  begin
    If ALSametext(xmlrec.ChildNodes[i].NodeName, NodeName) and
      ALSametext(xmlrec.ChildNodes[i].Attributes[AttributeName], AttributeValue)
    then
    begin
      Result := xmlrec.ChildNodes[i];
      break;
    ...
    if Recurse then
    begin
      Result := ALFindXmlNodeByNameAndAttribute(xmlrec.ChildNodes[i], NodeName,
        AttributeName, AttributeValue, Recurse);
      if Assigned(Result) then
        break;
    end;
  end;
