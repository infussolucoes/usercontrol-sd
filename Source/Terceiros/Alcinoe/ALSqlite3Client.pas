...
{$IF CompilerVersion >= 23} { Delphi XE2 }
  TalSqlite3ClientSelectDataOnNewRowFunct = reference to Procedure
...
{$IFEND}
  ...
  TalSqlite3ClientSelectDataQUERY = record
    ...
    class function Create: TalSqlite3ClientSelectDataQUERY; static;
  ...
  TalSqlite3ClientUpdateDataQUERY = record
    SQL: AnsiString;
    class function Create: TalSqlite3ClientUpdateDataQUERY; static;
    {$IF CompilerVersion >= 17.0}inline; {$IFEND}
  end;
  ...
  Protected
    ...
    Function GetFieldValue(aSqlite3stmt: PSQLite3Stmt; aIndex: Integer;
    ...
    procedure OnUpdateDataDone(Query: TalSqlite3ClientUpdateDataQUERY;
      TimeTaken: Integer); virtual;
  Public
    Constructor Create(const lib: AnsiString = 'sqlite3.dll';
    ...
    // can not be put in the create because config can/must be call prior initialize
    procedure shutdown;
    // can not be put in the create because config can/must be call after shutdown
    ...
    Procedure SelectData(Queries: TalSqlite3ClientSelectDataQUERIES;
      XMLDATA: TalXmlNode;
      OnNewRowFunct: TalSqlite3ClientSelectDataOnNewRowFunct; ExtData: Pointer;
  ...
  Protected
    ...
    procedure initObject(aDataBaseName: AnsiString;
      const aOpenConnectionFlags: Integer = SQLITE_OPEN_READWRITE or
      SQLITE_OPEN_CREATE; const aOpenConnectionPragmaStatements
      : AnsiString = ''); virtual;
    Function AcquireConnection(const readonly: Boolean = False)
      : PSQLite3; virtual;
  ...
  Public
    Constructor Create(aDataBaseName: AnsiString;
      const aOpenConnectionFlags: Integer = SQLITE_OPEN_READWRITE or
      SQLITE_OPEN_CREATE; const aOpenConnectionPragmaStatements
      : AnsiString = ''; const alib: AnsiString = 'sqlite3.dll';
      const initializeLib: Boolean = True); overload; virtual;
    Constructor Create(aDataBaseName: AnsiString; alib: TALSqlite3Library;
      const aOpenConnectionFlags: Integer = SQLITE_OPEN_READWRITE or
      SQLITE_OPEN_CREATE; const aOpenConnectionPragmaStatements
    ...
    Procedure TransactionStart(Var ConnectionHandle: PSQLite3;
      const readonly: Boolean = False); virtual;
...
{ ************************************************************************************* }
class function TalSqlite3ClientSelectDataQUERY.Create
  : TalSqlite3ClientSelectDataQUERY;
begin
  with result do
  begin
    SQL := '';
...
{ ************************************************************************************* }
class function TalSqlite3ClientUpdateDataQUERY.Create
  : TalSqlite3ClientUpdateDataQUERY;
begin
  with result do
  begin
    SQL := '';
...
Begin
  if Error then
  begin
    if assigned(fSqlite3) then
      raise EALSqlite3Error.Create(AnsiString(fLibrary.sqlite3_errmsg(fSqlite3)
        ), fLibrary.sqlite3_errcode(fSqlite3))
      // !! take care that sqlite3_errmsg(Fsqlite3) return an UTF8 !!
    else
      raise EALSqlite3Error.Create('Sqlite3 error', -1);
  end;
end;

{ ***************************************************************** }
function TalSqlite3Client.GetFieldValue(aSqlite3stmt: PSQLite3Stmt;
  aIndex: Integer; aFormatSettings: TALFormatSettings): AnsiString;
begin
  Case fLibrary.sqlite3_column_type(aSqlite3stmt, aIndex) of
    SQLITE_FLOAT:
      result := ALFloattostr(fLibrary.sqlite3_column_double(aSqlite3stmt,
        aIndex), aFormatSettings);
    SQLITE_INTEGER, SQLITE3_TEXT:
      result := AnsiString(fLibrary.sqlite3_column_text(aSqlite3stmt, aIndex));
    ...
    raise Exception.Create('Unsupported column type');
  end;
end;
...
{ ************************************************************************************************************** }
constructor TalSqlite3Client.Create(const lib: AnsiString = 'sqlite3.dll';
...
begin
  if enable then
    CheckAPIError(fLibrary.sqlite3_enable_shared_cache(1) <> SQLITE_OK)
  else
    CheckAPIError(fLibrary.sqlite3_enable_shared_cache(0) <> SQLITE_OK);
end;
...
begin
  ...
  Try
    CheckAPIError(fLibrary.sqlite3_open_v2(PAnsiChar(DatabaseName), fSqlite3,
      flags, nil) <> SQLITE_OK);
  Except
...
{ ********************************************************************************* }
procedure TalSqlite3Client.OnSelectDataDone
  (Query: TalSqlite3ClientSelectDataQUERY; TimeTaken: Integer);
begin
  // virtual
end;

{ ********************************************************************************* }
procedure TalSqlite3Client.OnUpdateDataDone
  (Query: TalSqlite3ClientUpdateDataQUERY; TimeTaken: Integer);
begin
  // virtual
end;
...
procedure TalSqlite3Client.SelectData
  (Queries: TalSqlite3ClientSelectDataQUERIES; XMLDATA: TalXmlNode;
  OnNewRowFunct: TalSqlite3ClientSelectDataOnNewRowFunct; ExtData: Pointer;
  FormatSettings: TALFormatSettings);
Var
  astmt: PSQLite3Stmt;
...
begin
  ...
  try
    ...
    begin
      ...
      // prepare the query
      astmt := nil;
      CheckAPIError(fLibrary.sqlite3_prepare_v2(fSqlite3,
        PAnsiChar(Queries[aQueriesIndex].SQL),
        length(Queries[aQueriesIndex].SQL), astmt, nil) <> SQLITE_OK);
      Try
        ...
        For aColumnIndex := 0 to aColumnCount - 1 do
          aColumnNames[aColumnIndex] := fLibrary.sqlite3_column_name(astmt,
            aColumnIndex);
        ...
        begin
          ...
          if aStepResult = SQLITE_DONE then
            break

            // download the row
          else if aStepResult = SQLITE_ROW then
          begin
            ...
            begin
              ...
              begin
                aValueRec := aNewRec.AddChild
                  (ALlowercase(aColumnNames[aColumnIndex]));
                aValueRec.Text := GetFieldValue(astmt, aColumnIndex,
                  FormatSettings);
...
{ **************************************************** }
procedure TalSqlite3Client.SelectData(SQL: AnsiString; Skip: Integer;
  First: Integer; OnNewRowFunct: TalSqlite3ClientSelectDataOnNewRowFunct;
...
{ **************************************************** }
procedure TalSqlite3Client.SelectData(SQL: AnsiString; RowTag: AnsiString;
...
{ **************************************************** }
procedure TalSqlite3Client.SelectData(SQL: AnsiString; XMLDATA: TalXmlNode;
  FormatSettings: TALFormatSettings);
var
  aSelectDataQUERIES: TalSqlite3ClientSelectDataQUERIES;
begin
...
{ ******************************************************************************** }
procedure TalSqlite3Client.UpdateData
  (Queries: TalSqlite3ClientUpdateDataQUERIES);
Var
  astmt: PSQLite3Stmt;
  aQueriesIndex: Integer;
  aStopWatch: TStopWatch;
begin
  ...
  begin
    ...
    // prepare the query
    CheckAPIError(fLibrary.sqlite3_prepare_v2(fSqlite3,
      PAnsiChar(Queries[aQueriesIndex].SQL), length(Queries[aQueriesIndex].SQL),
      astmt, nil) <> SQLITE_OK);
    Try

      // retrieve the next row
      CheckAPIError(not(fLibrary.sqlite3_step(astmt) in [SQLITE_DONE,
        SQLITE_ROW]));
...
{ ************************************************************************************************* }
procedure TalSqlite3ConnectionPoolClient.CheckAPIError(ConnectionHandle
  : PSQLite3; Error: Boolean);
begin
  if Error then
  begin
    if assigned(ConnectionHandle) then
      raise EALSqlite3Error.Create
        (AnsiString(fLibrary.sqlite3_errmsg(ConnectionHandle)),
        fLibrary.sqlite3_errcode(ConnectionHandle))
      // !! take care that sqlite3_errmsg(Fsqlite3) return an UTF8 !!
    else
      raise EALSqlite3Error.Create('Sqlite3 error', -1);
  end
...
{ ******************************************************************************* }
function TalSqlite3ConnectionPoolClient.GetFieldValue
  (aSqlite3stmt: PSQLite3Stmt; aIndex: Integer;
  aFormatSettings: TALFormatSettings): AnsiString;
begin
  Case fLibrary.sqlite3_column_type(aSqlite3stmt, aIndex) of
    SQLITE_FLOAT:
...
procedure TalSqlite3ConnectionPoolClient.initObject(aDataBaseName: AnsiString;
  const aOpenConnectionFlags: Integer = SQLITE_OPEN_READWRITE or
  SQLITE_OPEN_CREATE; const aOpenConnectionPragmaStatements: AnsiString = '');
begin
  FDataBaseName := aDataBaseName;
  FOpenConnectionFlags := aOpenConnectionFlags;
  FOpenConnectionPragmaStatements := TALStringList.Create;
  FOpenConnectionPragmaStatements.Text :=
    ALTrim(AlStringReplace(aOpenConnectionPragmaStatements, ';', #13#10,
    [rfReplaceAll]));
...
constructor TalSqlite3ConnectionPoolClient.Create(aDataBaseName: AnsiString;
  const aOpenConnectionFlags: Integer = SQLITE_OPEN_READWRITE or
  SQLITE_OPEN_CREATE; const aOpenConnectionPragmaStatements: AnsiString = '';
  const alib: AnsiString = 'sqlite3.dll'; const initializeLib: Boolean = True);
begin
  fLibrary := TALSqlite3Library.Create;
  try
    fLibrary.Load(alib, False);
    config(SQLITE_CONFIG_MULTITHREAD);
    if initializeLib then
      initialize;
    FownLibrary := True;
    initObject(aDataBaseName, aOpenConnectionFlags,
...
{ ************************************************************************** }
constructor TalSqlite3ConnectionPoolClient.Create(aDataBaseName: AnsiString;
  alib: TALSqlite3Library; const aOpenConnectionFlags
  : Integer = SQLITE_OPEN_READWRITE or SQLITE_OPEN_CREATE;
...
begin
  if enable then
    CheckAPIError(nil, fLibrary.sqlite3_enable_shared_cache(1) <> SQLITE_OK)
  else
    CheckAPIError(nil, fLibrary.sqlite3_enable_shared_cache(0) <> SQLITE_OK);
end;
...
{ *************************************************************************************************** }
function TalSqlite3ConnectionPoolClient.AcquireConnection(const readonly
  : Boolean = False): PSQLite3;
Var
  aConnectionPoolContainer: TalSqlite3ConnectionPoolContainer;
  aTickCount: int64;
  aDoPragma: Boolean;
Begin
  ...
  Try
    ...
    begin
      result := nil;
      Try
        CheckAPIError(result, fLibrary.sqlite3_open_v2(PAnsiChar(FDataBaseName),
          result, FOpenConnectionFlags, nil) <> SQLITE_OK);
        aDoPragma := True;
      Except
        // A database connection handle is usually returned in *ppDb, even if an error occurs.
        // Whether or not an error occurs when it is opened, resources associated with the
  ...
  try
    if aDoPragma then
      UpdateData(FOpenConnectionPragmaStatements, result);
  Except
    ...
    // release the lock
    if FDatabaseWriteLocked then
    begin
      FDatabaseWriteLocked := False;
      FDatabaseRWCS.EndWrite;
    end
    else
      FDatabaseRWCS.EndRead;
...
{ Applications must finalize all prepared statements and close all BLOB handles associated
  ...
  If sqlite3_close() is invoked while a transaction is open, the transaction is
  automatically rolled back. }
procedure TalSqlite3ConnectionPoolClient.ReleaseConnection(var ConnectionHandle
  : PSQLite3; const CloseConnection: Boolean = False);
Var
  aConnectionPoolContainer: TalSqlite3ConnectionPoolContainer;
begin
...
{ *********************************************************************************************************** }
procedure TalSqlite3ConnectionPoolClient.ReleaseAllConnections
...
{ *************************************************************************************** }
procedure TalSqlite3ConnectionPoolClient.TransactionStart(Var ConnectionHandle
  : PSQLite3; const readonly: Boolean = False);
begin
...
{ **************************************************************************************** }
procedure TalSqlite3ConnectionPoolClient.TransactionCommit(var ConnectionHandle
  : PSQLite3; const CloseConnection: Boolean = False);
begin
...
{ ****************************************************************************************** }
procedure TalSqlite3ConnectionPoolClient.TransactionRollback
  (var ConnectionHandle: PSQLite3; const CloseConnection: Boolean = False);
var
...
{ *********************************************************************************************** }
procedure TalSqlite3ConnectionPoolClient.OnSelectDataDone
  (Query: TalSqlite3ClientSelectDataQUERY; TimeTaken: Integer);
begin
  // virtual
end;
...
{ *********************************************************************************************** }
procedure TalSqlite3ConnectionPoolClient.OnUpdateDataDone
  (Query: TalSqlite3ClientUpdateDataQUERY; TimeTaken: Integer);
begin
  // virtual
end;
...
procedure TalSqlite3ConnectionPoolClient.SelectData
  (Queries: TalSqlite3ClientSelectDataQUERIES; XMLDATA: TalXmlNode;
  OnNewRowFunct: TalSqlite3ClientSelectDataOnNewRowFunct; ExtData: Pointer;
  FormatSettings: TALFormatSettings; const ConnectionHandle: PSQLite3 = nil);
...
begin
  ...
  try
    ...
    if aOwnConnection then
      TransactionStart(aTmpConnectionHandle, True);
    Try
      ...
      begin
        ...
        CheckAPIError(aTmpConnectionHandle,
          fLibrary.sqlite3_prepare_v2(aTmpConnectionHandle,
          PAnsiChar(Queries[aQueriesIndex].SQL),
          length(Queries[aQueriesIndex].SQL), astmt, nil) <> SQLITE_OK);
        Try
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                begin
                  aValueRec :=
                    aNewRec.AddChild(ALlowercase(aColumnNames[aColumnIndex]));
                  aValueRec.Text := GetFieldValue(astmt, aColumnIndex,
                    FormatSettings);
                  if aUpdateRowTagByFieldValue and
                    (aValueRec.NodeName = aNewRec.NodeName) then
            ...
            // misc error, raise an exception
              ...
              CheckAPIError(aTmpConnectionHandle, True);

          end;

        Finally
          // free the memory used by the API
          CheckAPIError(aTmpConnectionHandle, fLibrary.sqlite3_finalize(astmt)
            <> SQLITE_OK);
        End;
...
{ ***************************************************************************************** }
procedure TalSqlite3ConnectionPoolClient.SelectData
  (Query: TalSqlite3ClientSelectDataQUERY;
  OnNewRowFunct: TalSqlite3ClientSelectDataOnNewRowFunct; ExtData: Pointer;
  FormatSettings: TALFormatSettings; const ConnectionHandle: PSQLite3 = nil);
var
  aSelectDataQUERIES: TalSqlite3ClientSelectDataQUERIES;
...
{ ***************************************************************************************** }
procedure TalSqlite3ConnectionPoolClient.SelectData
  (Query: TalSqlite3ClientSelectDataQUERY; XMLDATA: TalXmlNode;
  FormatSettings: TALFormatSettings; const ConnectionHandle: PSQLite3 = nil);
var
  aSelectDataQUERIES: TalSqlite3ClientSelectDataQUERIES;
begin
...
{ ********************************************************************************************* }
procedure TalSqlite3ConnectionPoolClient.UpdateData
  (Queries: TalSqlite3ClientUpdateDataQUERIES;
  const ConnectionHandle: PSQLite3 = nil);
Var
  astmt: PSQLite3Stmt;
...
begin
  ...
  aOwnConnection := (not assigned(ConnectionHandle));
  if aOwnConnection then
    TransactionStart(aTmpConnectionHandle, False);
  Try
    ...
    begin
      ...
      Try
        ...
        // retrieve the next row
        CheckAPIError(aTmpConnectionHandle,
          not(fLibrary.sqlite3_step(astmt) in [SQLITE_DONE, SQLITE_ROW]));
...
{ ***************************************************************************************** }
procedure TalSqlite3ConnectionPoolClient.UpdateData
  (Query: TalSqlite3ClientUpdateDataQUERY;
  const ConnectionHandle: PSQLite3 = nil);
Var
  aUpdateDataQUERIES: TalSqlite3ClientUpdateDataQUERIES;
begin
