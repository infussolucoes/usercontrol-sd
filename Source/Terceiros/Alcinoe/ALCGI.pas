...
Procedure AlCGIInitDefaultServerVariablesFromWebRequest
  (WebRequest: TALWebRequest; ServerVariables: TALStrings); overload;
Procedure AlCGIInitDefaultServerVariablesFromWebRequest
  (WebRequest: TALWebRequest; ServerVariables: TALStrings;
  ScriptName, ScriptFileName: AnsiString; Url: AnsiString); overload;
...
Procedure AlCGIInitServerVariablesFromWebRequest(WebRequest: TALWebRequest;
  ServerVariables: TALStrings; ScriptName, ScriptFileName: AnsiString;
  Url: AnsiString); overload;
Procedure AlCGIExec(InterpreterFilename: AnsiString;
  ServerVariables: TALStrings; RequestContentStream: Tstream;
  ResponseContentStream: Tstream;
  ResponseHeader: TALHTTPResponseHeader); overload;
Procedure AlCGIExec(ScriptName, ScriptFileName, Url, X_REWRITE_URL,
  InterpreterFilename: AnsiString; WebRequest: TALIsapiRequest;
  overloadedCookies: AnsiString; overloadedQueryString: AnsiString;
  overloadedReferer: AnsiString; overloadedRequestContentStream: Tstream;
...
Begin
  ...
  ServerVariables.Add('HTTP_UA_CPU=' + WebRequest.GetFieldByName('HTTP_UA_CPU')
    ); // HTTP_UA_CPU=x86
  ServerVariables.Add('HTTP_CONNECTION=Keep-Alive');
  // HTTP_CONNECTION=Keep-Alive | Whether to close the connection when done
  ServerVariables.Add('SERVER_PROTOCOL=HTTP/1.1');
  // SERVER_PROTOCOL=HTTP/1.1 | Server Protocol
  ServerVariables.Add('SERVER_SOFTWARE=Microsoft-IIS/7.0');
  // SERVER_SOFTWARE=Microsoft-IIS/5.1 | Server Software
  ServerVariables.Add('SERVER_NAME=' + WebRequest.GetFieldByName('SERVER_NAME')
    ); // SERVER_NAME=www.mywebsite.com  | Server Host Name
  ServerVariables.Add('SERVER_PORT=' + WebRequest.GetFieldByName('SERVER_PORT')
    ); // SERVER_PORT=80 | Server Port Number
  ServerVariables.Add('REMOTE_HOST=' + WebRequest.GetFieldByName('REMOTE_HOST')
    ); // REMOTE_HOST=88.167.177.200 | Client Host Name
  ServerVariables.Add('REMOTE_ADDR=' + WebRequest.GetFieldByName('REMOTE_ADDR')
    ); // REMOTE_ADDR=88.167.177.200 | Client IP Number
  ServerVariables.Add('HTTP_HOST=' + WebRequest.GetFieldByName('HTTP_HOST'));
  // HTTP_HOST=www.mywebsite.com | Requested Host
  { ---------- }
  ServerVariables.Add('HTTP_USER_AGENT=' + WebRequest.GetFieldByName
    ('HTTP_USER_AGENT'));
  // HTTP_USER_AGENT=Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; SV1) | Client Software you are using
  ServerVariables.Add('HTTP_ACCEPT=*/*');
  // HTTP_ACCEPT=*/* | List of acceptable content type
end;
...
Begin
  ...
  { ---------- }
  ServerVariables.Values['URL'] := Url;
  // URL=/vbseo.php?vbseourl=forum4/discussion98851/showthread.php&bleubleu=24
  ServerVariables.Values['PATH_INFO'] := ScriptName; // PATH_INFO=/vbseo.php
  ServerVariables.Values['PATH_TRANSLATED'] := ScriptFileName;
  // PATH_TRANSLATED=D:\wwwroot\www.mywebsite.com\vbseo.php
  ServerVariables.Values['PHP_SELF'] := ScriptName; // PHP_SELF=/vbseo.php
  ServerVariables.Values['SCRIPT_NAME'] := ScriptName;
  // SCRIPT_NAME=/vbseo.php | Script Name
  ServerVariables.Values['SCRIPT_FILENAME'] := ScriptFileName;
  // SCRIPT_FILENAME=D:\wwwroot\www.mywebsite.com\vbseo.php
  ServerVariables.Values['REQUEST_URI'] := Url;
  // REQUEST_URI=/vbseo.php?vbseourl=forum4/discussion98851/showthread.php&bleubleu=24
end;
...
Begin
  ...
  ServerVariables.Add('SERVER_SOFTWARE=Microsoft-IIS/7.0');
  // SERVER_SOFTWARE=Microsoft-IIS/5.1 | Server Software
  ServerVariables.Add('SERVER_NAME=127.0.0.1');
  // SERVER_NAME=www.mywebsite.com  | Server Host Name
  ServerVariables.Add('SERVER_PORT=80'); // SERVER_PORT=80 | Server Port Number
  ServerVariables.Add('REMOTE_HOST=127.0.0.1');
  // REMOTE_HOST=88.167.177.200 | Client Host Name
  ServerVariables.Add('REMOTE_ADDR=127.0.0.1');
  // REMOTE_ADDR=88.167.177.200 | Client IP Number
  ServerVariables.Add('HTTP_HOST=127.0.0.1');
  // HTTP_HOST=www.mywebsite.com | Requested Host
  { ---------- }
  ServerVariables.Add('HTTP_USER_AGENT=Mozilla/4.0 (compatible)');
  // HTTP_USER_AGENT=Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; SV1) | Client Software you are using
  ServerVariables.Add('HTTP_ACCEPT=*/*');
  // HTTP_ACCEPT=*/* | List of acceptable content type
end;
...
Begin
  AlCGIInitDefaultServerVariablesFromWebRequest(WebRequest, ServerVariables,
    ScriptName, ScriptFileName, Url);
  { ---------- }
  ServerVariables.Add('REQUEST_METHOD=' + WebRequest.GetFieldByName
    ('REQUEST_METHOD'));
  // REQUEST_METHOD=GET | Either GET (normal for HTML docs) or POST (normal for forms)
  ServerVariables.Add('CONTENT_TYPE=' + WebRequest.GetFieldByName
    ('CONTENT_TYPE')); // CONTENT_TYPE=application/x-www-form-urlencoded
  ServerVariables.Add('CONTENT_LENGTH=' + WebRequest.GetFieldByName
    ('CONTENT_LENGTH')); // CONTENT_LENGTH=207
  ServerVariables.Add('HTTP_CONTENT_ENCODING=' + WebRequest.GetFieldByName
    ('HTTP_CONTENT_ENCODING')); // HTTP_CONTENT_ENCODING=
  ServerVariables.Add('HTTP_CONTENT_VERSION=' + WebRequest.GetFieldByName
    ('HTTP_CONTENT_VERSION')); // HTTP_CONTENT_VERSION=
  ServerVariables.Add('QUERY_STRING=' + WebRequest.GetFieldByName
    ('QUERY_STRING'));
  // QUERY_STRING=goto=newpost&t=1 | Query String: The characters appended to the URL after a question mark.
  ServerVariables.Add('HTTP_COOKIE=' + WebRequest.GetFieldByName('HTTP_COOKIE')
    ); // HTTP_COOKIE=bblastvisit=1155301716; bblastactivity=0 | Cookies: information that has been stored on your machine (in the file cookies.txt or MagicCookie) by our server
  { ---------- }
  ServerVariables.Add('HTTP_REFERER=' + WebRequest.GetFieldByName
    ('HTTP_REFERER'));
  // HTTP_REFERER=http://127.0.0.1/usa/forum/ | Address of the document you linked from
  { ---------- }
...
begin
  ...
  If not assigned(RequestContentStream) then
  begin
    RequestContentStream := TmemoryStream.Create;
    FreeRequestContentStream := True;
  end
  else
    FreeRequestContentStream := False;
  Try
    ...
    { ---------- }
    Environment := AlGetEnvironmentString;
    // =C:=C:\Program Files\Borland\Delphi7\Projects
    ...
    For i := 0 to ServerVariables.Count - 1 do
      Environment := Environment + ALFormat(EnvironmentTemplate,
        [ServerVariables.Names[i], ServerVariables.ValueFromIndex[i]]);
    ...
    ALWinExec(ALQuotedStr(InterpreterFilename, '"') + ' ' + ScriptFileName,
      ALExtractFileDir(InterpreterFilename), Environment,
      RequestContentStream, aStream);
    ...
    aStream.Free;
    If FreeRequestContentStream then
...
Procedure AlCGIExec(ScriptName, ScriptFileName, Url, X_REWRITE_URL,
  ...
  overloadedCookies: AnsiString; overloadedQueryString: AnsiString;
  overloadedReferer: AnsiString; overloadedRequestContentStream: Tstream;
  Var ResponseContentString: AnsiString; ResponseHeader: TALHTTPResponseHeader);
...
begin
  ...
  Try
    ...
    If overloadedRequestContentStream <> nil then
      RequestContentStream := overloadedRequestContentStream
    else
      RequestContentStream := WebRequest.ContentStream;

    AlCGIInitServerVariablesFromWebRequest(WebRequest, ServerVariables,
      ScriptName, ScriptFileName, Url);
    ServerVariables.Values['HTTP_X_REWRITE_URL'] := X_REWRITE_URL;
    // HTTP_X_REWRITE_URL=/forum4/discussion98851/showthread.php?bleubleu=24
    If overloadedCookies <> #0 then
      ServerVariables.Values['HTTP_COOKIE'] := overloadedCookies;
    // HTTP_COOKIE=bblastvisit=1155301716; bblastactivity=0; bbthreadedmode=linear; sessionid=2025726278; __utmc=96992031; CurrencyCode=EUR; MeasurementSystem=2; language=USA; Source=127.0.0.1; __utma=96992031.751139461.1154095149.1155298085.1155307847.37; __utmz=96992031.1154095149.1.1.utmccn=(direct)|utmcsr=(direct)|utmcmd=(none); vbulletin_collapse=; __utmb=96992031
    If overloadedQueryString <> #0 then
      ServerVariables.Values['QUERY_STRING'] := overloadedQueryString;
    // QUERY_STRING=goto=newpost&t=1
    If overloadedReferer <> #0 then
      ServerVariables.Values['HTTP_REFERER'] := overloadedReferer;
    // HTTP_REFERER=http://www.yahoo.fr

    AlCGIExec(InterpreterFilename, ServerVariables, RequestContentStream,
...
Procedure AlCGIExec(ScriptName, ScriptFileName, Url, X_REWRITE_URL,
  ...
  Var ResponseContentString: AnsiString; ResponseHeader: TALHTTPResponseHeader);
begin
  AlCGIExec(ScriptName, ScriptFileName, Url, X_REWRITE_URL, InterpreterFilename,
    WebRequest, #0, #0, #0, nil, ResponseContentString, ResponseHeader);
end;
