...
type
  ...
  TALResamplingFilter = (sfBox, sfTriangle, sfHermite, sfBell, sfSpline,
    sfLanczos3, sfMitchell);

  // Resampling support routines
procedure ALStretch(NewWidth, NewHeight: Cardinal; Filter: TALResamplingFilter;
  Radius: Single; Source, Target: TBitmap); overload;
procedure ALStretch(NewWidth, NewHeight: Cardinal; Filter: TALResamplingFilter;
  Radius: Single; Source: TBitmap); overload;
procedure ALStrecth(Source, Target: TBitmap; Filter: TALResamplingFilter;
  Radius: Single); overload;

// tolerance is same value as tolerance in photoshop
procedure ALReplaceColor(SrcBitmap: TBitmap; OldColor, NewColor: Tcolor;
  Tolerance: Integer);
...
Function AlGetAverageColorMosaicKey(aSrcBmp: TBitmap; aDestCanvas: Tcanvas;
  // can be nil if we don't want to draw the result
...
Function AlGetAverageColorMosaicKey(aSrcBmp: TBitmap)
  : TALAverageColorMosaicKey; overload;
...
Const
  cAlPixelCountMax = 32768;
  // Using a large value for PixelCountMax serves two purposes.
...
const
  cALDefaultFilterRadius: array [TALResamplingFilter] of Single = (0.5, 1, 1,
    1.5, 2, 3, 2);
...
begin
  ...
  begin
    Value := (((12 - 9 * B - 6 * C) * (Value * Temp)) +
      ((-18 + 12 * B + 6 * C) * Temp) + (6 - 2 * B));
    Result := Value / 6;
  end
  else if Value < 2 then
  begin
    Value := (((-B - 6 * C) * (Value * Temp)) + ((6 * B + 30 * C) * Temp) +
      ((-12 * B - 48 * C) * Value) + (8 * B + 24 * C));
    Result := Value / 6;
  end
  else
    Result := 0;
...
const
  cALFilterList: array [TALResamplingFilter] of TALFilterFunction =
    (ALBoxFilter, ALTriangleFilter, ALHermiteFilter, ALBellFilter,
    ALSplineFilter, ALLanczos3Filter, ALMitchellFilter);

  { ******************************************* }
procedure ALFillLineChache(N, Delta: Integer; Line: Pointer;
  var CurrentLineR: array of Integer; var CurrentLineG: array of Integer;
...
{ ************************************** }
function ALApplyContributors(N: Integer; Contributors: TALContributors;
  var CurrentLineR: array of Integer; var CurrentLineG: array of Integer;
...
procedure ALDoStretch(Filter: TALFilterFunction; Radius: Single;
...
var
  ...
  SourceLine, DestLine: PALPixelArray;
  DestPixel: PALBGR;
...
begin
  ...
  if (SourceHeight = 0) or (SourceWidth = 0) or (TargetHeight = 0) or
    (TargetWidth = 0) then
    Exit;
  ...
  try
    ...
    begin
      ...
      ALFillLineChache(SourceWidth, 3, SourceLine, CurrentLineR, CurrentLineG,
        CurrentLineB);
      DestPixel := Work.ScanLine[K];
      for I := 0 to TargetWidth - 1 do
        with ContributorList[I] do
        begin
          DestPixel^ := ALApplyContributors(N, ContributorList[I].Contributors,
            CurrentLineR, CurrentLineG, CurrentLineB);
...
begin
  if Radius = 0 then
    Radius := cALDefaultFilterRadius[Filter];
  Target.Handle := 0;
  ...
  Try
    For x := 0 to SrcBitmap.Width do
      ...
      begin
        ...
        If (Tolerance <= 0) or
          (sqrt(Sqr(GetRValue(aColor) - R1) + Sqr(GetGValue(aColor) - G1) +
          Sqr(GetBValue(aColor) - B1)) <= Tolerance) then
          SrcBitmap.Canvas.Pixels[x, y] := NewColor;
      end;
  finally
...
Begin
  ...
  if aMosaicSquareWidthTmp <= 0 then
    aMosaicSquareWidthTmp := Round(aSrcBmp.Width / 5);
  aMosaicSquareHeightTmp := aMosaicSquareHeight;
  if aMosaicSquareHeightTmp <= 0 then
    aMosaicSquareHeightTmp := Round(aSrcBmp.Height / 5);

  // init Result
  X2 := Trunc(aSrcBmp.Width / aMosaicSquareWidthTmp);
  if aSrcBmp.Width mod aMosaicSquareWidthTmp <> 0 then
    Inc(X2);
  Y2 := Trunc(aSrcBmp.Height / aMosaicSquareHeightTmp);
  if aSrcBmp.Height mod aMosaicSquareHeightTmp <> 0 then
    Inc(Y2);
  ...
  X1 := 0;
  X2 := 0;
  while X1 < aSrcBmp.Width do
  begin

    Y1 := 0;
    Y2 := 0;
    while Y1 < aSrcBmp.Height do
    begin

      // calc the rect
      aRect := Rect(X1, Y1, Min(X1 + aMosaicSquareWidthTmp, aSrcBmp.Width),
      ...
      begin
        ...
        R := Round(0.212671 * GetRValue(aColor) + 0.715160 * GetGValue(aColor) +
          0.072169 * GetBValue(aColor));
        G := R;
        B := R;
        aColor := RGB(R, G, B);
      end;
...
begin
  ...
  try
    ...
    Result := AlGetAverageColorMosaicKey(aResizedBitmap, nil, true, 40, 40);
...
procedure AlTrimImage(aSrcBmp: TBitmap; const aTolerance: Integer = 0);
var
  aRow: pAlRGBTripleArray;
  aIsBlack: Integer;
  aCropRect: Trect;
  aBackRColor, aBackGColor, aBackBColor: Byte;
  aTmpBmp: TBitmap;
  x, y: Integer;
begin
  ...
  // Find Top
  aCropRect.top := 0;
  for y := 0 to aSrcBmp.Height - 1 do
  begin
    aRow := aSrcBmp.ScanLine[y];
    aIsBlack := 0;
    for x := 0 to aSrcBmp.Width - 1 do
    begin
      With aRow[x] do
      begin
        if (sqrt(Sqr(rgbtRed - aBackRColor) + Sqr(rgbtGreen - aBackGColor) +
          Sqr(rgbtBlue - aBackBColor)) > aTolerance) then
        begin
          aIsBlack := 1;
          break;
        end;
  ...
  for y := aSrcBmp.Height - 1 Downto 0 do
  begin
    aRow := aSrcBmp.ScanLine[y];
    aIsBlack := 0;
    for x := 0 to aSrcBmp.Width - 1 do
  ...
  begin
    ...
    for x := aSrcBmp.Width - 1 downto 0 do
    begin
      With aRow[x] do
      begin
        if (sqrt(Sqr(rgbtRed - aBackRColor) + Sqr(rgbtGreen - aBackGColor) +
  ...
  if (aCropRect.top > 0) or (aCropRect.Left > 0) or
    (aCropRect.bottom < aSrcBmp.Height) or (aCropRect.Right < aSrcBmp.Width)
  then
  begin

    // if the picture was blanc
    if (aCropRect.bottom < aCropRect.top) or (aCropRect.Right < aCropRect.Left)
    then
    begin
    ...
    begin
      ...
      Try
        ...
        If Not BitBlt(aTmpBmp.Canvas.Handle,
          ...
          SRCCOPY) then
          raiseLastOsError; // [in] Specifies a raster-operation code
...
begin
  ...
  // init the aCropRect
  if (aCropRect.top < 0) then
    aCropRect.top := 0;
  if (aCropRect.Left < 0) then
    aCropRect.Left := 0;
  if (aCropRect.bottom > aSrcBmp.Height) then
    aCropRect.bottom := aSrcBmp.Height;
  if (aCropRect.Right > aSrcBmp.Width) then
    aCropRect.Right := aSrcBmp.Width;
...
begin
  ...
  for y := 0 to aDestBmp.Height - 1 do
    aDestScanLines[y] := aDestBmp.ScanLine[y];
...
begin
  ...
  begin
    ...
    for x := 0 to aSrcBmp.Width - 1 do
    begin
      aDestScanLines[(aSrcBmp.Height - 1) - y][(aSrcBmp.Width - 1) - x] :=
        aSrcScanLine[x]
    end;
  end;
