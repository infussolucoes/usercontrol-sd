...
type
  ...
  { ------------------------------------------------------------------------------ }
  TALWinInetFTPClientInternetOptionSet = Set of
  ...
  TAlWinInetFTPClientStatusChangeEvent = procedure(sender: Tobject;
  ...
  private
    ...
    procedure SetInternetOptions(const Value
      : TALWinInetFTPClientInternetOptionSet);
  ...
  public
    ...
    Function FindFirst(const Path: AnsiString; const Attr: Integer;
    ...
    Procedure GetFile(const RemoteFile: AnsiString; const LocalFile: AnsiString;
      FailIfExists: Boolean); overload; override;
    // warning: The onprogress will not work in that case !
    Procedure GetFile(const RemoteFile: AnsiString; DataStream: Tstream);
      overload; override;
    ...
    Procedure PutFile(DataStream: Tstream; const RemoteFile: AnsiString);
      overload; override;
    ...
    property TransferType: TALWinInetFtpTransferType read FTranferType
      write FTranferType default wFtpTt_BINARY;
    property AccessType: TALWinInetFTPInternetOpenAccessType read FAccessType
      write SetAccessType default wFtpAt_Preconfig;
    property InternetOptions: TALWinInetFTPClientInternetOptionSet
      read FInternetOptions write SetInternetOptions
      default [wftpIo_No_cache_write];
    property OnStatusChange: TAlWinInetFTPClientStatusChangeEvent
...
begin
  with TALWinInetFTPClient(Context) do
  begin

    { fire the On Status change event }
    if Assigned(FOnStatusChange) then
      FOnStatusChange(TALWinInetFTPClient(Context), InternetStatus,
...
begin
  ...
  begin
    ...
    begin
      ErrMsgln := 0;
      ErrMsg := '';
      if not InternetGetLastResponseInfoA(ErrCode, PAnsiChar(ErrMsg), ErrMsgln)
      then
      begin
        ErrCode := GetLastError;
        if ErrCode = ERROR_INSUFFICIENT_BUFFER then
        begin
...
{ ------------------------------------------- }
  ...
  Begin
    result := 0;
    if wFtpIo_From_Cache in InternetOptions then
      result := result or INTERNET_FLAG_FROM_CACHE;
    if wFtpIo_Offline in InternetOptions then
...
{ ---------------------------------------------- }
  ...
  Begin
    result := 0;
    if wFtpIo_Passive in InternetOptions then
      result := result or INTERNET_FLAG_PASSIVE;
  end;

const
  AccessTypeArr: Array [TALWinInetFTPInternetOpenAccessType]
...
begin
  ...
  { init FInetRoot }
  FInetRoot := InternetOpenA(PAnsiChar(''), AccessTypeArr[FAccessType],
    InternalGetProxyServerName, InternalGetProxyBypass,
  ...
  try
    ...
    begin
      InternetSetStatusCallbackResult := InternetSetStatusCallback(FInetRoot,
        @ALWininetFTPCLientStatusCallback);
      CheckError(InternetSetStatusCallbackResult = Pointer
        (INTERNET_INVALID_STATUS_CALLBACK));
    end;

    { init FInetConnect }
    FInetConnect := InternetConnectA(FInetRoot, PAnsiChar(ServerName),
      ServerPort, PAnsiChar(UserName), PAnsiChar(Password),
      INTERNET_SERVICE_FTP, InternalGetInternetConnectFlags, DWORD_PTR(Self));
  ...
  Try
    ...
    If ProxyParams.ProxyUserName <> '' then
      CheckError(not InternetSetOption(FInetConnect,
        INTERNET_OPTION_PROXY_USERNAME, PAnsiChar(ProxyParams.ProxyUserName),
        Length(ProxyParams.ProxyUserName)));
    If ProxyParams.ProxyPassword <> '' then
      CheckError(not InternetSetOption(FInetConnect,
        INTERNET_OPTION_PROXY_PASSWORD, PAnsiChar(ProxyParams.ProxyPassword),
...
begin
  If Not FConnected then
    raise EALFTPClientException.Create(CALFTPCLient_MsgNotConnected);
...
{ *********************************************************************************** }
function TALWinInetFTPClient.FindMatchingFile
...
begin
  ...
  begin
    while FindData.dwFileAttributes and ExcludeAttr <> 0 do
      if not InternetFindNextFile(F.FindHandle, @F.FindData) then
      begin
        result := GetLastError;
        Exit;
      end;
...
function TALWinInetFTPClient.FindFirst(const Path: AnsiString;
  const Attr: Integer; var F: TALFtpclientSearchRec): Integer;

{ ----------------------------------------------- }
  Function InternalGetFtpFindFirstFileFlags: DWORD;
  Begin
    result := 0;
    if wFtpIo_Hyperlink in InternetOptions then
      result := result or INTERNET_FLAG_HYPERLINK;
    if wFtpIo_Need_file in InternetOptions then
      result := result or INTERNET_FLAG_NEED_FILE;
    if wftpIo_No_cache_write in InternetOptions then
      result := result or INTERNET_FLAG_NO_CACHE_WRITE;
    if wftpIo_Reload in InternetOptions then
      result := result or INTERNET_FLAG_RELOAD;
    if wftpIo_Resynchronize in InternetOptions then
      result := result or INTERNET_FLAG_RESYNCHRONIZE;
  end;

const
  faSpecial = faDirectory;
begin
  If Not FConnected then
    raise EALFTPClientException.Create(CALFTPCLient_MsgNotConnected);
  F.ExcludeAttr := not Attr and faSpecial;
  F.FindHandle := FtpFindFirstFileA(FInetConnect, PAnsiChar(Path), F.FindData,
    InternalGetFtpFindFirstFileFlags, DWORD_PTR(Self));
...
{ ------------------------------------------ }
  ...
  Begin
    result := 0;
    if FTranferType = wFtpTt_ASCII then
      result := result or INTERNET_FLAG_TRANSFER_ASCII
    else
      result := result or INTERNET_FLAG_TRANSFER_BINARY;
    if wFtpIo_Hyperlink in InternetOptions then
      result := result or INTERNET_FLAG_HYPERLINK;
    if wFtpIo_Need_file in InternetOptions then
      result := result or INTERNET_FLAG_NEED_FILE;
    ...
    if wftpIo_No_cache_write in InternetOptions then
      result := result or INTERNET_FLAG_NO_CACHE_WRITE;
    // even if the doc of Ms don't say it, we must set this here
  end;

var
  Size, Downloaded, ContentlengthDownloaded, ContentLength: DWORD;
  S: AnsiString;
  hFile: HINTERNET;
...
begin
  If Not FConnected then
    raise EALFTPClientException.Create(CALFTPCLient_MsgNotConnected);
  hFile := FtpOpenFileA(FInetConnect, PAnsiChar(RemoteFile), GENERIC_READ,
    InternalGetFtpOpenFileFlags, DWORD_PTR(Self));
  ...
  Try
    ...
    repeat
      CheckError(not InternetQueryDataAvailable(hFile, Size, 0, 0));

      if Size > 0 then
      begin
        SetLength(S, Size);
        CheckError(not InternetReadFile(hFile, @S[1], Size, Downloaded));
...
{ ***************************************************************** }
procedure TALWinInetFTPClient.GetFile(const RemoteFile: AnsiString;
  const LocalFile: AnsiString; FailIfExists: Boolean);
...
begin
  If Not FConnected then
    raise EALFTPClientException.Create(CALFTPCLient_MsgNotConnected);
  CheckError(not FtpGetFileA(FInetConnect, PAnsiChar(RemoteFile),
    PAnsiChar(LocalFile), FailIfExists, FILE_ATTRIBUTE_NORMAL,
    InternalGetFtpGetFileFlags, DWORD_PTR(Self)));
end;
...
begin
  If Not FConnected then
    raise EALFTPClientException.Create(CALFTPCLient_MsgNotConnected);
  hFile := FtpOpenFileA(FInetConnect, PAnsiChar(FileName), GENERIC_READ,
    InternalGetFtpOpenFileFlags, DWORD_PTR(Self));
  CheckError(not Assigned(hFile));
  Try
...
begin
  If Not FConnected then
    raise EALFTPClientException.Create(CALFTPCLient_MsgNotConnected);
  hFile := FtpOpenFileA(FInetConnect, PAnsiChar(RemoteFile), GENERIC_WRITE,
    InternalGetFtpOpenFileFlags, DWORD_PTR(Self));
  CheckError(not Assigned(hFile));
  ...
  Try
    ...
    begin
      ...
      if Len = 0 then
        raise EALFTPClientException.Create(CALFtpCLient_MsgInvalidFtpRequest);

      CheckError(not InternetWriteFile(hFile, @Buffer.Memory^, Len, RetVal));
...
begin
  If Not FConnected then
    raise EALFTPClientException.Create(CALFTPCLient_MsgNotConnected);
  CheckError(not FtpPutFileA(FInetConnect, PAnsiChar(LocalFile),
    PAnsiChar(RemoteFile), InternalGetFtpPutFileFlags, DWORD_PTR(Self)));
end;
...
begin
  If Not FConnected then
    raise EALFTPClientException.Create(CALFTPCLient_MsgNotConnected);
  CheckError(not FtpRenameFileA(FInetConnect, PAnsiChar(ExistingFile),
    PAnsiChar(NewFile)));
end;
...
{ ******************************************************************************************** }
procedure TALWinInetFTPClient.SetAccessType(const Value
...
{ ************************************************************************************************* }
procedure TALWinInetFTPClient.SetOnStatusChange(const Value
  : TAlWinInetFTPClientStatusChangeEvent);
begin
  Disconnect;
...
{ *********************************************************************************************** }
procedure TALWinInetFTPClient.OnProxyParamsChange(sender: Tobject;
...
{ ************************************************************************************************** }
procedure TALWinInetFTPClient.SetInternetOptions(const Value
  : TALWinInetFTPClientInternetOptionSet);
begin
  If Value <> FInternetOptions then
  begin
    If (wFtpIo_From_Cache in Value) and
      (not(wFtpIo_From_Cache in FInternetOptions)) or
      (wFtpIo_From_Cache in FInternetOptions) and
      (not(wFtpIo_From_Cache in Value)) or (wFtpIo_Passive in Value) and
      (not(wFtpIo_Passive in FInternetOptions)) or
      ...
      (wFtpIo_Offline in FInternetOptions) and (not(wFtpIo_Offline in Value))
    then
      Disconnect;
