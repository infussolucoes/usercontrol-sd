...
Var
  ALInterlockedCompareExchange64: function(var Destination: LONGLONG;
    Exchange, Comperand: LONGLONG): LONGLONG; stdcall;
  // from Windows Vista / Windows Server 2003

Var
  ALGetTickCount64: function: Int64; stdcall;
  // from Windows Vista / Windows Server 2008

Var
  ALSetProcessWorkingSetSizeEx: function(hProcess: THandle;
    dwMinimumWorkingSetSize, dwMaximumWorkingSetSize:
    {$IF CompilerVersion >= 23}{ Delphi XE2 } Size_T{$ELSE}Cardinal{$IFEND};
    Flags: DWORD): BOOL; stdcall; // from Windows Vista / Windows Server 2003
function ALInterlockedExchange64(var Target: LONGLONG; Value: LONGLONG)
  : LONGLONG;
...
{ *********************************************************************************** }
function ALCreateProcessWithLogonW;
...
begin
  ...
  if not Assigned(ALGetTickCount64) then
    ALGetTickCount64 := ALGetTickCount64XP;
  @ALInterlockedCompareExchange64 := GetProcAddress(kernel32,
    'InterlockedCompareExchange64');
  @ALSetProcessWorkingSetSizeEx := GetProcAddress(kernel32,
...
begin
  ...
  try
    ...
    Result := LookupAccountNameA(nil, PansiChar(aUserName), SID, cbSID,
