...
function _InternetCrackUrlA(lpszUrl: PAnsiChar; dwUrlLength, dwFlags: DWORD;
  var lpUrlComponents: _TURLComponentsA): BOOL; stdcall; external
{$IFDEF MSWINDOWS}'wininet.dll'{$ENDIF}{$IFDEF LINUX}'libwininet.borland.so'{$ENDIF} name 'InternetCrackUrlA';
...
function _HttpSendRequestExA(hRequest: HINTERNET;
  lpBuffersIn: _LP_INTERNET_BUFFERSA; lpBuffersOut: _LP_INTERNET_BUFFERSA;
  dwFlags: DWORD; dwContext: DWORD_PTR): BOOL; stdcall; external
{$IFDEF MSWINDOWS}'wininet.dll'{$ENDIF}{$IFDEF LINUX}'libwininet.borland.so'{$ENDIF} name 'HttpSendRequestExA';
function _HttpEndRequestA(hRequest: HINTERNET;
  lpBuffersOut: _LP_INTERNET_BUFFERSA; dwFlags: DWORD; dwContext: DWORD_PTR)
  : BOOL; stdcall; external
{$IFDEF MSWINDOWS}'wininet.dll'{$ENDIF}{$IFDEF LINUX}'libwininet.borland.so'{$ENDIF} name 'HttpEndRequestA';

type

  { ----------------------------------------------------- }
  TALWinInetHttpInternetOpenAccessType = (wHttpAt_Direct,
    { Resolves all host names locally. }
    wHttpAt_Preconfig,
    { Retrieves the proxy or direct configuration from the registry. }
    wHttpAt_Preconfig_with_no_autoproxy,
  ...
  TAlWininetHttpClientInternetOption = (wHttpIo_Async, { NOT SUPPORTED YET!
    ...
    wHttpIo_Offline,
    { Identical to Hcio_From_Cache. Does not make network requests.
    ...
    wHttpIo_Hyperlink,
    { Forces a reload if there was no Expires time and no LastModified time returned
    ...
    { Disables detection of this special type of redirect. When this flag is used, WinINet functions
  ...
  { -------------------------------------------------------------------------------- }
  TALWininetHttpClientInternetOptionSet = Set of
  ...
  TAlWinInetHTTPClientStatusChangeEvent = procedure(sender: Tobject;
    InternetStatus: DWORD; StatusInformation: Pointer;
    StatusInformationLength: DWORD) of object;
  ...
  private
    ...
    procedure SetInternetOptions(const Value
      : TALWininetHttpClientInternetOptionSet);
    procedure SetOnStatusChange(const Value
      : TAlWinInetHTTPClientStatusChangeEvent);
  protected
    ...
    procedure OnProxyParamsChange(sender: Tobject;
      Const PropertyIndex: Integer); override;
  ...
  public
    ...
    procedure Receive(aContext: DWORD; aResponseContentStream: TStream;
    ...
    property AccessType: TALWinInetHttpInternetOpenAccessType read FAccessType
      write SetAccessType default wHttpAt_Preconfig;
    property InternetOptions: TALWininetHttpClientInternetOptionSet
      read FInternetOptions write SetInternetOptions
      default [wHttpIo_Keep_connection];
    property DisconnectOnError: Boolean read FDisconnectOnError
      write FDisconnectOnError default False;
    // WinInethttp seam to handle internally the disconnection/reconnection !
    property OnStatusChange: TAlWinInetHTTPClientStatusChangeEvent
...
procedure ALWininetHTTPCLientStatusCallback(InternetSession: HINTERNET;
  Context, InternetStatus: DWORD; StatusInformation: Pointer;
  StatusInformationLength: DWORD); stdcall;
var
  NewURL: AnsiString;
begin
  with TALWinInetHTTPClient(Context) do
  begin

    { fire the On Status change event }
    if Assigned(FOnStatusChange) then
      FOnStatusChange(TALWinInetHTTPClient(Context), InternetStatus,
        StatusInformation, StatusInformationLength);

    { fire the On redirect event }
    If (InternetStatus = INTERNET_STATUS_REDIRECT) and Assigned(OnRedirect) then
    begin
...
begin
  ...
  Begin
    ...
    If (FOldURLHost <> FURLHost) or (FoldUrlPort <> FURLPort) then
      Disconnect;
...
begin
  ...
  begin
    ...
    if not(URLComp.nScheme in [INTERNET_SCHEME_HTTP, INTERNET_SCHEME_HTTPS])
    then
      raise EALHTTPClientException.CreateFmt
        (CALHTTPCLient_MsgInvalidURL, [Value]);
    FURLScheme := URLComp.nScheme;
    FURLPort := URLComp.nPort;
    FURLHost := ALCopyStr(Value, URLComp.lpszHostName - P + 1,
      URLComp.dwHostNameLength);
    FURLSite := ALCopyStr(Value, URLComp.lpszUrlPath - P + 1,
...
begin
  IF Password <> PasswordValue then
    Disconnect;
  inherited;
end;
...
{ --------------------------------------------- }
  ...
  Begin
    If (ProxyParams.ProxyServer = '') then
      result := nil
    else
      result := PAnsiChar(ProxyParams.ProxyServer + ':' +
        ALIntToStr(ProxyParams.ProxyPort));
  end;
...
{ ----------------------------------------- }
  ...
  Begin
    ...
    if (ProxyParams.ProxyBypass = '') then
      result := nil
    else
      result := PAnsiChar(ProxyParams.ProxyBypass);
  end;
...
{ ------------------------------------------- }
  ...
  Begin
    result := 0;
    if wHttpIo_From_Cache in InternetOptions then
      result := result or INTERNET_FLAG_FROM_CACHE;
    if wHttpIo_Offline in InternetOptions then
      result := result or INTERNET_FLAG_OFFLINE;
  end;

const
  AccessTypeArr: Array [TALWinInetHttpInternetOpenAccessType]
    of DWORD = (INTERNET_OPEN_TYPE_DIRECT, INTERNET_OPEN_TYPE_PRECONFIG,
    INTERNET_OPEN_TYPE_PRECONFIG_WITH_NO_AUTOPROXY, INTERNET_OPEN_TYPE_PROXY);

var
  InternetSetStatusCallbackResult: PFNInternetStatusCallback;
...
begin
  ...
  if InternetAttemptConnect(0) <> ERROR_SUCCESS then
    ...
    System.{$IFEND}SysUtils.Abort;

  { init FInetRoot }
  FInetRoot := InternetOpenA(PAnsiChar(RequestHeader.UserAgent),
    AccessTypeArr[FAccessType], InternalGetProxyServerName,
    InternalGetProxyBypass, InternalGetInternetOpenFlags);
  CheckError(not Assigned(FInetRoot));

  try

    { Register the callback function }
    if Assigned(OnStatusChange) or Assigned(OnRedirect) then
    begin
      InternetSetStatusCallbackResult := InternetSetStatusCallback(FInetRoot,
        @ALWininetHTTPCLientStatusCallback);
      CheckError(InternetSetStatusCallbackResult = Pointer
        (INTERNET_INVALID_STATUS_CALLBACK));
    end;

    { init FInetConnect }
    FInetConnect := InternetConnectA(FInetRoot, PAnsiChar(FURLHost), FURLPort,
      PAnsiChar(UserName), PAnsiChar(Password), INTERNET_SERVICE_HTTP, 0,
      DWORD_PTR(Self));
...
{ ---------------------------------------------- }
  ...
  Begin
    result := 0;
    if wHttpIo_Cache_if_net_fail in InternetOptions then
      result := result or INTERNET_FLAG_CACHE_IF_NET_FAIL;
    if wHttpIo_Hyperlink in InternetOptions then
    ...
    if wHttpIo_Ignore_redirect_to_http in InternetOptions then
      result := result or INTERNET_FLAG_IGNORE_REDIRECT_TO_HTTP;
    if wHttpIo_Ignore_redirect_to_https in InternetOptions then
      result := result or INTERNET_FLAG_IGNORE_REDIRECT_TO_HTTPS;
    if wHttpIo_Keep_connection in InternetOptions then
      result := result or INTERNET_FLAG_KEEP_CONNECTION;
    if wHttpIo_Need_file in InternetOptions then
      result := result or INTERNET_FLAG_NEED_FILE;
    if wHttpIo_No_auth in InternetOptions then
      result := result or INTERNET_FLAG_NO_AUTH;
    if wHttpIo_No_auto_redirect in InternetOptions then
      result := result or INTERNET_FLAG_NO_AUTO_REDIRECT;
    if wHttpIo_No_cache_write in InternetOptions then
      result := result or INTERNET_FLAG_NO_CACHE_WRITE;
    if wHttpIo_No_cookies in InternetOptions then
      result := result or INTERNET_FLAG_NO_COOKIES;
    if wHttpIo_No_ui in InternetOptions then
      result := result or INTERNET_FLAG_NO_UI;
    if wHttpIo_Pragma_nocache in InternetOptions then
      result := result or INTERNET_FLAG_PRAGMA_NOCACHE;
    if wHttpIo_Reload in InternetOptions then
      result := result or INTERNET_FLAG_RELOAD;
    if wHttpIo_Resynchronize in InternetOptions then
      result := result or INTERNET_FLAG_RESYNCHRONIZE;
    if (wHttpIo_Secure in InternetOptions) or
      (FURLScheme = INTERNET_SCHEME_HTTPS) then
...
{ ------------------------------------------------- }
  ...
  Begin
    If ProtocolVersion = HTTPpv_1_1 then
      result := 'HTTP/1.1'
    else
      result := 'HTTP/1.0'
  end;
...
{ ------------------------------------------------- }
  ...
  Begin
    Case RequestMethod of
      ...
      HTTPmt_Trace:
        result := 'TRACE';
        // <= seam to not work on Wininet ! but work on winhttp
    else
      raise Exception.Create('Unknown Request Method');
    end;
  end;
...
begin
  ...
  Request := nil;
  try
    Request := HttpOpenRequestA(FInetConnect, InternalGetHttpOpenRequestVerb,
      PAnsiChar(FURLSite), InternalGetHttpProtocolVersion,
      PAnsiChar(RequestHeader.Referer), Pointer(AcceptTypes),
      InternalGetHttpOpenRequestFlags, DWORD_PTR(Self));

    CheckError(not Assigned(Request));

    { Timeouts }
    if ConnectTimeout > 0 then
      CheckError(not InternetSetOptionA(Request,
        INTERNET_OPTION_CONNECT_TIMEOUT, Pointer(@ConnectTimeout),
        SizeOf(ConnectTimeout)));
    if SendTimeout > 0 then
      CheckError(not InternetSetOptionA(Request, INTERNET_OPTION_SEND_TIMEOUT,
        Pointer(@SendTimeout), SizeOf(SendTimeout)));
    if ReceiveTimeout > 0 then
      CheckError(not InternetSetOptionA(Request,
        INTERNET_OPTION_RECEIVE_TIMEOUT, Pointer(@ReceiveTimeout),
        SizeOf(ReceiveTimeout)));

    { proxy user name and password }
    If ProxyParams.ProxyUserName <> '' then
      CheckError(not InternetSetOptionA(Request, INTERNET_OPTION_PROXY_USERNAME,
        PAnsiChar(ProxyParams.ProxyUserName),
        Length(ProxyParams.ProxyUserName)));
    If ProxyParams.ProxyPassword <> '' then
      CheckError(not InternetSetOptionA(Request, INTERNET_OPTION_PROXY_PASSWORD,
        PAnsiChar(ProxyParams.ProxyPassword),
        Length(ProxyParams.ProxyPassword)));

    { set the header }
    aHeader := RequestHeader.RawHeaderText;
    HttpAddRequestHeadersA(Request, PAnsiChar(aHeader), Length(aHeader),
      HTTP_ADDREQ_FLAG_REPLACE or HTTP_ADDREQ_FLAG_ADD);

    If Assigned(aRequestDataStream) then
    begin
    ...
    if BuffSize > UploadBufferSize then
    begin
      ...
      try
        ...
        { Start POST }
        CheckError(not _HttpSendRequestExA(Request, @INBuffer, nil,
          HSR_INITIATE or HSR_SYNC, DWORD_PTR(Self)));
        try
          while True do
          begin
            { Calc length of data to send }
            Len := BuffSize - aRequestDataStream.Position;
            if Len > UploadBufferSize then
              Len := UploadBufferSize;
            { Bail out if zip.. }
            if Len = 0 then
              break;
            ...
            if Len = 0 then
              raise EALHTTPClientException.Create
                (CALHTTPCLient_MsgInvalidHTTPRequest);

            CheckError(not InternetWriteFile(Request, @Buffer.Memory^,
              Len, RetVal));
          ...
          CheckError(not _HttpEndRequestA(Request, nil, 0, 0));
        end;
      finally
        Buffer.Free;
      end;
    ...
    begin
      ...
      try
        ...
        CheckError(not HttpSendRequestA(Request, nil, 0, @StrStr.DataString[1],
    ...
    begin
      CheckError(not HttpSendRequestA(Request, nil, 0, nil, 0));
    end;
...
var
  Size, Downloaded, Status, Len, Index, ContentlengthDownloaded,
    ContentLength: DWORD;
  S: AnsiString;
...
begin
  ...
  { read the header }
  If Assigned(aResponseContentHeader) then
  begin
    Size := 4096;
    While True do
    begin
      Index := 0;
      SetLength(S, Size);
      If HttpQueryInfoA(Pointer(aContext), HTTP_QUERY_RAW_HEADERS_CRLF, @S[1],
        Size, Index) then
      begin
        SetLength(S, Size);
  ...
  Index := 0;
  Len := SizeOf(Status);
  if HttpQueryInfoA(Pointer(aContext), HTTP_QUERY_STATUS_CODE or
    HTTP_QUERY_FLAG_NUMBER, @Status, Len, Index) and (Status >= 300) then
  begin
    ...
    begin
      Index := 0;
      SetLength(S, Size);
      if HttpQueryInfoA(Pointer(aContext), HTTP_QUERY_STATUS_TEXT, @S[1], Size,
        Index) then
      begin
        SetLength(S, Size);
        raise EALHTTPClientException.CreateFmt('%s (%d) - ''%s''',
          [S, Status, URL], Status);
      end
      else
        CheckError(GetLastError <> ERROR_INSUFFICIENT_BUFFER);
  ...
  if not HttpQueryInfoA(Pointer(aContext), HTTP_QUERY_CONTENT_LENGTH or
    HTTP_QUERY_FLAG_NUMBER, @ContentLength, Len, Index) then
    ContentLength := 0;
  ...
  repeat
    CheckError(not InternetQueryDataAvailable(Pointer(aContext), Size, 0, 0));

    if Size > 0 then
    begin
      SetLength(S, Size);
      CheckError(not InternetReadFile(Pointer(aContext), @S[1], Size,
        Downloaded));
...
begin
  if URL = '' then
    raise EALHTTPClientException.Create(CALHTTPCLient_MsgEmptyURL);
...
{ ********************************************************************************************** }
procedure TALWinInetHTTPClient.SetAccessType(const Value
  : TALWinInetHttpInternetOpenAccessType);
begin
  If (Value <> AccessType) then
  begin
    Disconnect;
    FAccessType := Value;
  end;
...
{ *************************************************************************************************** }
procedure TALWinInetHTTPClient.SetOnStatusChange(const Value
  : TAlWinInetHTTPClientStatusChangeEvent);
begin
  Disconnect;
...
{ ************************************************************************************ }
procedure TALWinInetHTTPClient.SetOnRedirect(const Value
  : TAlHTTPClientRedirectEvent);
begin
  Disconnect;
  inherited;
end;
...
{ ************************************************************************************************ }
procedure TALWinInetHTTPClient.OnProxyParamsChange(sender: Tobject;
  Const PropertyIndex: Integer);
begin
  if (PropertyIndex = -1) or (PropertyIndex in [0, 1, 2]) then
    Disconnect; // clear, ProxyBypass, ProxyServer, ProxyPort
end;

{ ************************************************************************************************** }
procedure TALWinInetHTTPClient.OnRequestHeaderChange(sender: Tobject;
  Const PropertyIndex: Integer);
begin
  if (PropertyIndex = -1) or (PropertyIndex = 35) then
    Disconnect; // clear, UserAgent
end;

{ **************************************************************************************************** }
procedure TALWinInetHTTPClient.SetInternetOptions
  (const Value: TALWininetHttpClientInternetOptionSet);
begin
  If Value <> FInternetOptions then
  begin
    If (wHttpIo_From_Cache in Value) and
      (not(wHttpIo_From_Cache in FInternetOptions)) or
      (wHttpIo_From_Cache in FInternetOptions) and
      (not(wHttpIo_From_Cache in Value)) or (wHttpIo_Offline in Value) and
      (not(wHttpIo_Offline in FInternetOptions)) or
      (wHttpIo_Offline in FInternetOptions) and (not(wHttpIo_Offline in Value))
    then
      Disconnect;
